...
# -*- coding: utf-8 -*-
#
# BORG OS v25 — Four-Stack Brain Dominant Fusion
# v24 Four-Stack Brain + v23 Deep RL + Mesh + DPI + Full Governors + Game Scanner + FastAPI + Tk GUI
#
# Single-file monolithic organism, designed for Windows gaming-first security.
# Windows 10/11, double-click runnable, gaming-safe, autonomous AI governor.
...
class BorgConfig:
    def __init__(self):
        ...
        self.initial_full_scan_done = False

        # Game protection
        self.game_safe_window_minutes = 5
        self.game_approval_timeout_sec = 30
        self.game_extensions = {".exe", ".bat", ".cmd"}
        self.game_keywords = {
            "steam.exe", "epicgameslauncher.exe", "battle.net.exe",
            "origin.exe", "eaapp.exe", "riotclientservices.exe",
            "valorant.exe", "cs2.exe", "csgo.exe", "fortniteclient-win64-shipping.exe",
            "gta5.exe", "eldenring.exe", "minecraft.exe"
        }

        # Never kill list (hard safety)
        self.never_kill_exact = {
            "steam.exe", "epicgameslauncher.exe", "battle.net.exe",
            "origin.exe", "eaapp.exe", "riotclientservices.exe",
            "explorer.exe", "ShellExperienceHost.exe", "StartMenuExperienceHost.exe",
            "SearchUI.exe", "dwm.exe", "System", "Registry", "csrss.exe",
            "wininit.exe", "winlogon.exe", "services.exe", "lsass.exe",
            "svchost.exe", "MicrosoftEdge.exe", "msedge.exe", "Teams.exe",
            "Copilot.exe", "GameBar.exe"
        }

        # Security / AI modes
        self.security_mode = 2          # 2 = Security-focused
        self.autonomous_mode = 3        # 3 = Autonomous AI

        # RL / persona
        self.persona_names = ["AGGRESSIVE", "DEFENSIVE", "PASSIVE"]
        self.persona_default_weights = {
            "AGGRESSIVE": 0.33,
            "DEFENSIVE": 0.34,
            "PASSIVE": 0.33
        }

        # Stacking layers (8-layer full stack)
        self.layers = [
            "ThreatScoreStack",
            "PersonaStack",
            "RLStack",
...
###############################################################################
# MEMORY STACKING – MULTI-ORGAN (v23)
###############################################################################
...
###############################################################################
# THREAT SIGNATURES + PROCESS REPUTATION (v23)
###############################################################################
...
###############################################################################
# GAME SCANNER + FIND_EXE + MANIFEST WATCHER (v23)
###############################################################################
...
###############################################################################
# GPU GOVERNOR + SYSTEM LOAD GOVERNOR (v23)
###############################################################################
...
###############################################################################
# KERNEL STACKING – MULTI-POLICY (v23)
###############################################################################

class KernelLayerV23:
...
class KernelStackV23:
    def __init__(self, logger=None, event_bus=None):
        self.logger = logger or (lambda msg: None)
        self.event_bus = event_bus

        self.hook_kernel     = KernelLayerV23("HOOK", logger=self.logger, event_bus=self.event_bus)
        self.policy_kernel   = KernelLayerV23("POLICY", logger=self.logger, event_bus=self.event_bus)
        self.behavior_kernel = KernelLayerV23("BEHAVIOR", logger=self.logger, event_bus=self.event_bus)
        self.threat_kernel   = KernelLayerV23("THREAT", logger=self.logger, event_bus=self.event_bus)
        self.kill_kernel     = KernelLayerV23("KILL", logger=self.logger, event_bus=self.event_bus)
...
###############################################################################
# DEEP RL STACKING – MULTI-AGENT (v23)
###############################################################################
...
###############################################################################
# MESH STACKING – MULTI-CONSENSUS (v23)
###############################################################################
...
class MeshStackV23:
    def __init__(self, storage: BorgStorage, logger=None, event_bus=None):
        ...
        self.local_mesh      = MeshConsensus(storage, logger=self.logger, event_bus=self.event_bus, name="LOCAL", quorum=2.0)
...
###############################################################################
# THREAT SCORE STACKING – MULTI-SCORE FUSION (v23)
###############################################################################

class ThreatScoreStackV23:
    def __init__(self, reputation: ProcessReputation, mesh_stack: MeshStackV23, logger=None, accelerator=None):
...
###############################################################################
# PERSONA STACKING – MULTI-PERSONA FUSION (v23)
###############################################################################

class PersonaStackV23:
    def __init__(self, logger=None, event_bus=None):
        self.logger = logger or (lambda msg: None)
        self.event_bus = event_bus
...
###############################################################################
# PROCESS & GAME REGISTRY (v24)
###############################################################################

class ProcessInfo:
    def __init__(self, pid, name, exe, cmdline, create_time):
        self.pid = pid
        self.name = name
        self.exe = exe
...
class GameRegistry:
    def __init__(self, cfg: BorgConfig):
        self.cfg = cfg
        self.lock = threading.Lock()
        self.games = {}          # pid -> ProcessInfo
        self.launchers = {}      # pid -> ProcessInfo
        self.approved_games = set()
        self.safe_window_until = None

    def _is_game_candidate(self, pinfo: ProcessInfo) -> bool:
        name_lower = (pinfo.name or "").lower()
        exe_lower = (pinfo.exe or "").lower()
        if not name_lower and not exe_lower:
            return False
        for kw in self.cfg.game_keywords:
            if kw.lower() in name_lower or kw.lower() in exe_lower:
                return True
        if "steamapps" in exe_lower or "\\games\\" in exe_lower or "\\steam\\" in exe_lower:
            return True
        return False

    def _is_launcher_candidate(self, pinfo: ProcessInfo) -> bool:
        name_lower = (pinfo.name or "").lower()
        exe_lower = (pinfo.exe or "").lower()
        for kw in ["steam.exe", "epicgameslauncher.exe", "battle.net.exe", "origin.exe", "eaapp.exe"]:
            if kw in name_lower or kw in exe_lower:
                return True
        return False

    def register_process(self, pinfo: ProcessInfo):
        with self.lock:
            if self._is_launcher_candidate(pinfo):
                pinfo.is_launcher = True
                pinfo.is_protected = True
                self.launchers[pinfo.pid] = pinfo
            elif self._is_game_candidate(pinfo):
                pinfo.is_game = True
                pinfo.is_protected = True
                self.games[pinfo.pid] = pinfo

    def mark_approved(self, pid: int):
        with self.lock:
            self.approved_games.add(pid)

    def set_safe_window(self, minutes: int):
        with self.lock:
            self.safe_window_until = datetime.utcnow() + timedelta(minutes=minutes)

    def is_in_safe_window(self) -> bool:
        with self.lock:
            if self.safe_window_until is None:
                return False
            return datetime.utcnow() <= self.safe_window_until

    def is_game_protected(self, pid: int, name: str) -> bool:
        with self.lock:
            if name.lower() in (n.lower() for n in self.cfg.never_kill_exact):
                return True
            if pid in self.games or pid in self.launchers:
                return True
            if pid in self.approved_games:
                return True
            if self.is_in_safe_window():
                return True
            return False

###############################################################################
# SAFE WINDOW / APPROVAL MANAGER (v24)
###############################################################################

class SafeWindowManagerV24:
    def __init__(self, cfg: BorgConfig, game_registry: GameRegistry):
        self.cfg = cfg
    ...
    def request_initial_approval(self, root: tk.Tk):
        with self.lock:
            if self.pending_approval:
                return
            self.pending_approval = True
            self.last_prompt_time = datetime.utcnow()

        def ask():
            answer = messagebox.askyesno(
                "BORG OS — Game Session",
                "New game/launcher activity detected.\n\n"
                "Allow a 5-minute SAFE MODE where anything that starts is protected?\n\n"
                "You will only be asked once per session."
            )
            with self.lock:
                if answer:
                    self.game_registry.set_safe_window(self.cfg.game_safe_window_minutes)
                self.pending_approval = False

        root.after(0, ask)

###############################################################################
# THREAT SCORE STACK (v24 simple) — used inside Four-Stack Brain
###############################################################################

class ThreatScoreStackV24:
    def __init__(self, cfg: BorgConfig):
        self.cfg = cfg

    def compute_score(self, proc: psutil.Process) -> float:
        try:
            cpu = proc.cpu_percent(interval=0.0)
            mem = proc.memory_percent()
            score = (cpu / 100.0) * 0.6 + (mem / 100.0) * 0.4
...
###############################################################################
# PERSONA STACK (v24 simple)
###############################################################################

class PersonaStackV24:
    def __init__(self, cfg: BorgConfig):
        self.cfg = cfg
        self.weights = dict(cfg.persona_default_weights)

    def update_from_rl(self, rl_vector: dict):
        for k, v in rl_vector.items():
            if k in self.weights:
                self.weights[k] = max(0.0, min(1.0, self.weights[k] + v))
        total = sum(self.weights.values())
        if total <= 0:
            self.weights = dict(self.cfg.persona_default_weights)
        else:
...
class RLAgentV24:
    ...
    def get_adjustment(self) -> float:
        return (self.value - 0.5) * 0.2

class RLStackV24:
    def __init__(self):
        self.threat_agent = RLAgentV24("Threat")
        self.load_agent = RLAgentV24("Load")
        self.behavior_agent = RLAgentV24("Behavior")

    def update(self, threat_score: float, overload: bool, false_positive: bool):
        reward = 0.0
        reward += (1.0 - threat_score) if not overload else -0.5
        reward += -0.3 if false_positive else 0.1
        self.threat_agent.observe(reward)
        self.load_agent.observe(reward * 0.8)
        self.behavior_agent.observe(reward * 1.2)

    def persona_adjustments(self) -> dict:
        base = self.threat_agent.get_adjustment()
        return {
            "AGGRESSIVE": base,
            "DEFENSIVE": -base * 0.5,
            "PASSIVE": -base * 0.5
...
###############################################################################
# DPI STACK (v24 simple) — we still keep v23 DPI for OS governor
###############################################################################
...
class DPIStackV24:
    ...
    def analyze(self, proc: psutil.Process) -> float:
        try:
            conns = proc.connections(kind='inet')
            score = min(len(conns) / 50.0, 1.0)
            score += random.uniform(-0.05, 0.05)
            return max(0.0, min(1.0, score))
        except Exception:
            return 0.0
...
###############################################################################
# MESH STACK (v24 simple) — local consensus per process
###############################################################################

class MeshStackV24:
    def __init__(self):
        self.votes = deque(maxlen=100)

    def submit_vote(self, pid: int, threat_score: float, dpi_score: float) -> str:
        combined = 0.6 * threat_score + 0.4 * dpi_score
        decision = "THREAT" if combined > 0.7 else "SAFE"
        self.votes.append((pid, combined, decision))
        return decision

###############################################################################
# MEMORY ORGANS (v24 simple)
###############################################################################

class MemoryOrgansV24:
    def __init__(self):
        self.short_term = deque(maxlen=200)
        self.mid_term = deque(maxlen=500)
    ...
    def record_event(self, event_type: str, data: dict):
        ...
        if event_type in ("THREAT", "KILL", "SAFE"):
            self.threat_history.append(entry)
        else:
            self.behavior_history.append(entry)
        if len(self.short_term) > 150:
            self.mid_term.extend(self.short_term)
            self.short_term.clear()
        if len(self.mid_term) > 400:
            self.long_term.extend(self.mid_term)
            self.mid_term.clear()

###############################################################################
# KERNEL STACK (v24 simple) — integrated with v23 kernel
###############################################################################

class KernelStackV24:
    def __init__(self, cfg: BorgConfig, game_registry: GameRegistry, kernel_v23: KernelStackV23):
        self.cfg = cfg
        self.game_registry = game_registry
        self.kernel_v23 = kernel_v23

    def can_kill(self, proc: psutil.Process) -> bool:
        name = (proc.name() or "").lower()
        pid = proc.pid
        if name in (n.lower() for n in self.cfg.never_kill_exact):
            return False
        if self.game_registry.is_game_protected(pid, name):
            return False
        return True

    def kill_process(self, proc: psutil.Process) -> bool:
        if not self.can_kill(proc):
            return False
        try:
            name = proc.name() or ""
            pid = proc.pid
            self.kernel_v23.enforce_threat(pid, name, "kill")
            self.kernel_v23.enforce_kill(pid, name)
            proc.terminate()
            return True
        except Exception:
            return False
...
###############################################################################
# GOVERNOR STACK (v24 simple) — uses v24 stacks + v23 kernel
###############################################################################

class GovernorStackV24:
    def __init__(self, cfg: BorgConfig, kernel_stack: KernelStackV24,
                 memory_stack: MemoryOrgansV24):
        self.cfg = cfg
        self.kernel_stack = kernel_stack
        self.memory_stack = memory_stack

    def decide(self, proc: psutil.Process, threat_score: float,
               dpi_score: float, persona_mode: str, mesh_decision: str) -> str:
        name = (proc.name() or "").lower()
        pid = proc.pid

        if name in (n.lower() for n in self.cfg.never_kill_exact):
            self.memory_stack.record_event("SAFE", {"pid": pid, "name": name, "reason": "never_kill"})
            return "SAFE"
        ...
        aggression_factor = {
            ...
            "DEFENSIVE": 0.8,
            "PASSIVE": 0.5
        }.get(persona_mode, 1.0)

        combined = (0.6 * threat_score + 0.4 * dpi_score) * aggression_factor

        if mesh_decision == "THREAT":
            combined += 0.1
        else:
            combined -= 0.05
        ...
        if combined >= kill_threshold:
            if self.kernel_stack.kill_process(proc):
                self.memory_stack.record_event("KILL", {
                    "pid": pid, "name": name, "score": combined,
                    "persona": persona_mode, "mesh": mesh_decision
                })
                return "KILL"
            else:
                self.memory_stack.record_event("FAIL_KILL", {
                    "pid": pid, "name": name, "score": combined
                })
                return "FAIL_KILL"
        elif combined >= suppress_threshold:
            self.memory_stack.record_event("SUPPRESS", {
                "pid": pid, "name": name, "score": combined,
                "persona": persona_mode, "mesh": mesh_decision
            })
            return "SUPPRESS"
        else:
            self.memory_stack.record_event("SAFE", {
                "pid": pid, "name": name, "score": combined,
                "persona": persona_mode, "mesh": mesh_decision
            })
            return "SAFE"
...
###############################################################################
# BORG STACK — One Cortex (Sensory/Cognitive/Emotional/Executive) (v24)
###############################################################################

class BorgStackV24:
    def __init__(self, name: str, cfg: BorgConfig, game_registry: GameRegistry, kernel_v23: KernelStackV23):
        self.name = name
        self.cfg = cfg
        self.game_registry = game_registry

        self.threat_stack = ThreatScoreStackV24(cfg)
        self.persona_stack = PersonaStackV24(cfg)
        self.rl_stack = RLStackV24()
        self.dpi_stack = DPIStackV24()
        self.mesh_stack = MeshStackV24()
        self.memory_stack = MemoryOrgansV24()
        self.kernel_stack = KernelStackV24(cfg, game_registry, kernel_v23)
        self.governor_stack = GovernorStackV24(cfg, self.kernel_stack, self.memory_stack)

    def process_tick(self, proc: psutil.Process) -> dict:
        pid = proc.pid
        name = (proc.name() or "").lower()

        threat_score = self.threat_stack.compute_score(proc)
        dpi_score = self.dpi_stack.analyze(proc)
        mesh_decision = self.mesh_stack.submit_vote(pid, threat_score, dpi_score)

        self.rl_stack.update(threat_score, overload=False, false_positive=False)
        persona_delta = self.rl_stack.persona_adjustments()
        self.persona_stack.update_from_rl(persona_delta)
        persona_mode = self.persona_stack.get_mode()

        decision = self.governor_stack.decide(
            proc, threat_score, dpi_score, persona_mode, mesh_decision
        )
...
###############################################################################
# NEURAL BUS — Four-Stack Brain Coordination (v24)
###############################################################################
...
class NeuralBus:
    ...
    def publish(self, stack_name: str, event: dict):
        with self.lock:
            self.state[stack_name] = event

    def get_brain_view(self) -> dict:
        with self.lock:
            return dict(self.state)

###############################################################################
# BORG BRAIN — Four Stacks Acting as One (v24 dominant)
###############################################################################
...
class BorgBrain:
    def __init__(self, cfg: BorgConfig, game_registry: GameRegistry, kernel_v23: KernelStackV23):
        ...
        self.stack_sensory = BorgStackV24("Sensory", cfg, game_registry, kernel_v23)
        self.stack_cognitive = BorgStackV24("Cognitive", cfg, game_registry, kernel_v23)
        self.stack_emotional = BorgStackV24("Emotional", cfg, game_registry, kernel_v23)
        self.stack_executive = BorgStackV24("Executive", cfg, game_registry, kernel_v23)

    def tick_process(self, proc: psutil.Process) -> dict:
        sensory_event = self.stack_sensory.process_tick(proc)
        cognitive_event = self.stack_cognitive.process_tick(proc)
        emotional_event = self.stack_emotional.process_tick(proc)
        executive_event = self.stack_executive.process_tick(proc)

        self.bus.publish("Sensory", sensory_event)
        self.bus.publish("Cognitive", cognitive_event)
        self.bus.publish("Emotional", emotional_event)
        self.bus.publish("Executive", executive_event)

        decisions = [
            sensory_event["decision"],
            cognitive_event["decision"],
            emotional_event["decision"],
            executive_event["decision"]
        ]
        fused_decision = self._majority_vote(decisions)

        return {
            "pid": proc.pid,
            "name": (proc.name() or "").lower(),
            "sensory": sensory_event,
            "cognitive": cognitive_event,
            "emotional": emotional_event,
            "executive": executive_event,
            "fused_decision": fused_decision
        }

    def _majority_vote(self, decisions):
...
###############################################################################
# PROCESS SCANNER — One-Time Full Scan + Incremental Detection (v24)
###############################################################################
...
class ProcessScanner:
    ...
    def initial_scan(self):
        with self.lock:
            ...
            self.cfg.initial_full_scan_done = True
    ...
    def _register_proc(self, proc: psutil.Process):
        try:
        ...
        self.seen_pids.add(pinfo.pid)
        self.game_registry.register_process(pinfo)

###############################################################################
# BORG CORE (v23) — Memory Cortex, driven alongside Four-Stack Brain
###############################################################################

class BorgCoreV23:
    def __init__(self, memory_stack: MemoryStackV23, logger=None, storage=None, governor=None, event_bus=None, persona=BorgPersona.DEFENSIVE):
        self.memory_stack = memory_stack
...
class BorgWatchdog:
    def __init__(self, core: BorgCoreV23, timeout=5.0, event_bus=None):
        self.core = core
        ...
        while self.running:
            try:
                if time.time() - self.last_tick > self.timeout:
                    ...
                    self.core.memory_stack = MemoryStackV23()
...
###############################################################################
# OS GOVERNOR – FULL STACK (v23) — used as OS-level governor
###############################################################################
...
class ThreatGovernor:
    def __init__(self, threat_stack: ThreatScoreStackV23, logger=None):
...
class KillGovernor:
    def __init__(self, kernel_stack: KernelStackV23, rl_stack: DeepRLStack, logger=None, event_bus=None):
        self.kernel_stack = kernel_stack
    ...
    def kill(self, p, score, gpu_load, sigs, overloaded):
        try:
            name = p.info.get('name') or ""
            lname = name.lower()
            ...
            reason = f"threat score={score:.1f}, gpu={gpu_load:.1f}, sigs={sig_str}"
...
class GameGovernor:
    def __init__(self, reputation: ProcessReputation, mesh_stack: MeshStackV23, logger=None):
        self.reputation = reputation
        self.mesh_stack = mesh_stack
        self.logger = logger or (lambda msg: None)
...
class BorgOSGovernor:
    def __init__(self, logger=None, storage=None, event_bus=None,
        ...
        self.persona = persona
        self.accelerator = accelerator
        self.rl_stack = rl_stack or DeepRLStack(logger=self.logger, event_bus=self.event_bus, accelerator=self.accelerator)
        self.kernel_stack = kernel_stack or KernelStackV23(logger=self.logger, event_bus=self.event_bus)
        self.mesh_stack = mesh_stack or MeshStackV23(storage=storage, logger=self.logger, event_bus=self.event_bus)
        self.dpi_stack = DPIStackV23(logger=self.logger, event_bus=self.event_bus, deep_rl=self.rl_stack)
        ...
        self.threat_stack = ThreatScoreStackV23(self.reputation, self.mesh_stack, logger=self.logger, accelerator=self.accelerator)
        self.persona_stack = PersonaStackV23(logger=self.logger, event_bus=self.event_bus)
                    ...
                    for p in procs[start:end]:
                        try:
                            ...
                            if state == BorgState.DOMINANT:
                                ...
                                if threat:
                                    self._kill_process(p, score, gpu_load, hard_protected, learned_safe, sigs, gpu_mem, overloaded)
    ...
    def _kill_process(self, p, score, gpu_load, hard_protected, learned_safe, sigs, gpu_mem, overloaded):
        killed = self.kill_governor.kill(p, score, gpu_load, sigs, overloaded)
        if killed:
...
###############################################################################
# GLOBAL INSTANCES (v23) + Four-Stack Brain (v24)
###############################################################################
...
rl_stack_os = DeepRLStack(logger=lambda msg: print(msg), event_bus=event_bus, accelerator=accelerator)
kernel_stack_v23 = KernelStackV23(logger=lambda msg: print(msg), event_bus=event_bus)
mesh_stack_v23 = MeshStackV23(storage=storage, logger=lambda msg: print(msg), event_bus=event_bus)

governor_os = BorgOSGovernor(logger=lambda msg: print(msg), storage=storage, event_bus=event_bus,
                             persona=BorgPersona.DEFENSIVE, rl_stack=rl_stack_os,
                             kernel_stack=kernel_stack_v23, mesh_stack=mesh_stack_v23,
...
for exe in detected_games:
    name = os.path.basename(exe)
    governor_os.reputation.safe_names.add(name)
    storage.update_safe_process(name, delta_rep=10.0)
    mesh_stack_v23.vote_safe(name, weight=2.0)
...
governor_os.refresh_game_cache()

manifest_watcher = GameManifestWatcher(game_scanner, governor_os, mesh_stack_v23.local_mesh, storage,
                                       logger=lambda msg: print(msg), interval=30.0)

memory_stack_v23 = MemoryStackV23(size_short=10, size_mid=10, size_long=10, size_threat=10, size_behavior=10)
core_v23 = BorgCoreV23(memory_stack_v23, logger=lambda msg: print(msg), storage=storage, governor=governor_os,
                       event_bus=event_bus, persona=BorgPersona.DEFENSIVE)
watchdog = BorgWatchdog(core_v23, event_bus=event_bus)
core_v23.attach_watchdog(watchdog)

class BorgRunnerV23:
    def __init__(self, core: BorgCoreV23, interval=0.5):
        self.core = core
        self.interval = interval
        self.running = True
        t = threading.Thread(target=self.loop, daemon=True)
...
runner_v23 = BorgRunnerV23(core_v23, interval=0.5)

###############################################################################
# Four-Stack Brain v24 instances
###############################################################################

cfg = BorgConfig()
game_registry = GameRegistry(cfg)
scanner = ProcessScanner(cfg, game_registry)
brain = BorgBrain(cfg, game_registry, kernel_stack_v23)
safe_window_mgr_v24 = SafeWindowManagerV24(cfg, game_registry)

###############################################################################
# SWARM + MESH INTEL (v23)
###############################################################################
...
###############################################################################
# FASTAPI SERVICE (v23) — uses BorgCore + OS metrics
###############################################################################

if FASTAPI_AVAILABLE:
    app = FastAPI(title="Borg OS Node", version="25.0.0")
    ...
    @app.get("/states")
    def get_states():
        organ_states_stack = core_v23.get_states()
        return {
            "organs": [[s.name for s in organ_states] for organ_states in organ_states_stack],
            "os": OS_NAME,
            "tick": core_v23.tick_counter,
            "persona": core_v23.persona.name,
    ...
    def sync_pull(req: SyncPushRequest):
        ...
        for organ_idx, organ_states in enumerate(organs):
            try:
                organ = core_v23.memory_stack.organs()[organ_idx]
...
###############################################################################
# TKINTER GUI – Command Center (v23) + Four-Stack Brain view (v24)
###############################################################################

class BorgGUI:
    def __init__(self, root, core: BorgCoreV23, brain: BorgBrain):
        self.root = root
        self.core = core
        self.brain = brain

        root.title("BORG OS NODE v25 – Four-Stack Brain + Full Stacking Organism")
    ...
    def build_ui(self):
        ...
        frame_brain = tk.Frame(self.root, bg="#111111")
        frame_brain.pack(padx=20, pady=5, fill="x")
        self.brain_label = tk.Label(frame_brain, text="Brain View: (waiting)", bg="#111111", fg="#00ffcc", font=("Consolas", 10))
        self.brain_label.pack(side="left", padx=5)

    def on_persona_change(self, event):
        name = self.persona_combo.get()
        try:
            new_persona = BorgPersona[name]
            core_v23.persona = new_persona
            governor_os.persona = new_persona
        except KeyError:
            pass
    ...
    def update_display(self):
        ...
        self.info_label.configure(
            text=f"Tick: {core_v23.tick_counter} | OS: {OS_NAME}"
        )
        self.persona_label.configure(
            text=f"Persona: {core_v23.persona.name}"
        )
        self.accel_var.set(f"Accelerator: {accelerator.mode.name}")

        brain_view = brain.bus.get_brain_view()
        self.brain_label.configure(text=f"Brain View: {json.dumps(brain_view, default=str)[:200]}...")
...
def run_cli():
    print("BORG OS NODE v25 (CLI MODE, Four-Stack Brain + Full Stacking Organism)")
    print(f"OS: {OS_NAME}")
    ...
    try:
        while True:
            ...
            print(f"STATE: {line} | TICK: {core_v23.tick_counter} | Persona: {core_v23.persona.name} | Accelerator: {accelerator.mode.name}")
            time.sleep(0.5)
    except KeyboardInterrupt:
        print("\nCLI Borg node terminated.")

###############################################################################
# SCAN + TICK LOOPS FOR Four-Stack Brain (v24)
###############################################################################

def start_brain_loops(root: tk.Tk):
    def scan_loop():
        scanner.initial_scan()
        while True:
            time.sleep(cfg.tick_interval_sec)
            scanner.incremental_scan()
            if not game_registry.is_in_safe_window():
                safe_window_mgr_v24.request_initial_approval(root)
...
def main():
    ...
    root = tk.Tk()
    gui = BorgGUI(root, core_v23, brain)
    start_brain_loops(root)
    root.mainloop()
