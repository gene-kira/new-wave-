#!/usr/bin/env python3
# BORG EVOLUTION ENGINE — SERVER + PC GAMING SENSORS + WEB STATUS
...
PING_TARGET = "8.8.8.8"  # can change to your game/server IP
...
def measure_latency(host=PING_TARGET):
    try:
        start = time.time()
        psutil.net_connections()  # cheap call to ensure psutil is alive
        # simple TCP connect latency
        import socket
...
def collect_external_sensors():
    """
    Gaming + server oriented sensors:
    - GPU usage
    - game process CPU/RAM
    - server process CPU/RAM
    - network latency to target
    - placeholder FPS (can be wired to real overlay later)
    """
    gpu = get_gpu_usage()
    ...
    # placeholder FPS: you can wire in real data via shared memory / file / API
    fps = 0.0
...
def append_system_data():
    ...
    # target: simple load metric; you can change to FPS, latency, etc.
    target = features["cpu"] + features["ram"] + features.get("ext_gpu_usage", 0.0)
...
def web_status_loop(port=5000):
    print(f"[WEB] Status endpoint on port {port} at /status")
...
def start_threads():
    ...
    t_web.start()
    return [t_stream, t_teacher, *swarm_threads, t_snapshot, t_port, t_web]
...
def watchdog_loop():
    print("=== BORG GAMING/SERVER ENGINE — WATCHDOG MODE ===")
    while True:
        try:
