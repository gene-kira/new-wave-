#!/usr/bin/env python
# game_guardian_ultra_ai_cluster_mic_supervisor_plus_audio_intel_v6.py
#
# Focus of V6:
# - Performance tightening:
#   - Less blocking work in audio callback
#   - Optional ping/GeoIP throttling
#   - Configurable feature tiers (LIGHT / FULL)
#   - Reduced log spam + batched mesh updates
# - Deeper AI Operator Mode:
#   - “Voice console” command set (audio, network, mic, profiles)
#   - Operator state + feedback in GUI
#   - STT + operator routing cleaned up
#
...
DEFAULT_CONFIG = {
    ...
    # RNNoise / deep / EQ / latency / macros
    ...
    # V5 features
    "stt_macros_enabled": True,
    "stt_backend": "speech_recognition",
    "rl_eq_enabled": False,
    "rl_eq_learning_rate": 0.01,
    "rl_eq_gamma": 0.95,
    "ai_operator_enabled": False,
    "audio_mesh_enabled": False,
    "audio_mesh_peers": [],
    # V6 performance + operator
    "feature_tier": "FULL",  # LIGHT / FULL
    "ping_enabled": True,
    "geoip_enabled": True,
    "mesh_update_interval": 5.0,
}
...
BLUE = "#0055FF"
GAME_HINTS = ["steam", "epic", "fortnite", "cs2", "apex", "valorant", "gta", "warzone"]
CURRENT_GAME_NAME = ""
...
def dns_hijack_status() -> Dict[str, Any]:
    ...
    return {"servers": servers, "hosts_suspicious": hosts_suspicious}
...
# -----------------------------
# Deep-learning enhancement stub
# -----------------------------
class DeepEnhancer:
    def __init__(self):
        self.model = None
...
# -----------------------------
# Adaptive EQ via RL (stub)
# -----------------------------
class RLEQAgent:
    def __init__(self):
        self.enabled = bool(config.get("rl_eq_enabled", False))
        self.lr = float(config.get("rl_eq_learning_rate", 0.01))
        self.gamma = float(config.get("rl_eq_gamma", 0.95))
        self.state = np.zeros(3, dtype=np.float32)
        self.last_reward = 0.0

    def select_action(self, clarity: float, clip_ratio: float, energy: float) -> Dict[str, float]:
        if not self.enabled:
            return {"low": 0.0, "mid": 0.0, "high": 0.0}
        delta = (1.0 - clarity)
        self.state[1] += self.lr * delta
        self.state[2] += self.lr * delta
        self.state = np.clip(self.state, -6.0, 6.0)
        return {"low": float(self.state[0]), "mid": float(self.state[1]), "high": float(self.state[2])}

    def update_reward(self, clarity: float):
        if not self.enabled:
            return
        self.last_reward = clarity
...
# STT Macro Engine
# -----------------------------
class STTMacroEngine:
    def __init__(self, macro_engine: VoiceMacroEngine, operator_callback: Callable[[str], None]):
        self.enabled = bool(config.get("stt_macros_enabled", True))
        self.backend = config.get("stt_backend", "speech_recognition")
    ...
    def loop(self):
        ...
        if not STT_AVAILABLE:
            print("[STT] Backend not available, running stub.")
            return
        ...
        while self.running:
            try:
                ...
                log_event({"type": "stt_text", "text": text, "timestamp": time.time()})
                # macros
                for key in config.get("voice_macros", {}).keys():
                    if key in text:
                        self.macro_engine.trigger_macro(key)
                # operator
                if config.get("ai_operator_enabled", False):
                    self.operator_callback(text)
            except Exception:
                continue
...
class VoiceFingerprintDB:
    ...
    def enroll(self, speaker_id: str, frame: np.ndarray, sr: int):
        ...
        self.profiles[speaker_id] = {"energy": energy, "centroid": centroid}
...
# -----------------------------
# VOICE MONITOR (performance-aware)
# -----------------------------
class VoiceMonitor:
    def __init__(self):
    ...
    def _maybe_apply_eq(self, frame: np.ndarray) -> np.ndarray:
        base_eq = get_eq_profile_for_game(CURRENT_GAME_NAME)
        rl_eq = self.rl_eq_agent.select_action(self.last_clarity_score, self.last_clip_ratio, self.last_energy)
        eq = {
            "low": base_eq.get("low", 0.0) + rl_eq.get("low", 0.0),
            "mid": base_eq.get("mid", 0.0) + rl_eq.get("mid", 0.0),
            "high": base_eq.get("high", 0.0) + rl_eq.get("high", 0.0),
        }
    ...
    def _audio_callback(self, indata, frames, time_info, status):
        ...
        try:
            ...
            self.rl_eq_agent.update_reward(clarity)

            if config.get("voice_fingerprint_enabled", False) and self.is_speaking and self.feature_tier == "FULL":
...
# -----------------------------
# AI Operator Mode (voice console)
# -----------------------------
class AIOperatorConsole:
    """
    Voice console:
    - Audio: clarity, noise, EQ, tier
    - Network: ping/geo toggles, optimize
    - Mic: self-heal toggles
    - Profiles: per-game EQ presets
    """
    def __init__(self):
        self.last_command = ""
        self.last_feedback = ""
    ...
    def _set_feedback(self, cmd: str, msg: str):
        self.last_command = cmd
        self.last_feedback = msg
        self.last_timestamp = time.time()
        log_event({"type": "ai_operator", "cmd": cmd, "feedback": msg, "timestamp": self.last_timestamp})

    def handle(self, text: str):
        t = text.lower()

        # audio clarity / noise
        if "increase clarity" in t:
            config["clarity_optimizer_enabled"] = True
            save_config(config)
            self._set_feedback("increase_clarity", "Clarity optimizer enabled.")
            return
        if "reduce noise" in t or "more noise suppression" in t:
            config["noise_suppression_enabled"] = True
            save_config(config)
            self._set_feedback("reduce_noise", "Noise suppression enabled.")
            return
        if "disable noise suppression" in t:
            config["noise_suppression_enabled"] = False
            save_config(config)
            self._set_feedback("disable_noise", "Noise suppression disabled.")
            return

        # EQ / bass / treble
        if "boost bass" in t:
            eq = config.get("eq_profiles", {})
            for k in eq.keys():
                eq[k]["low"] = eq[k].get("low", 0.0) + 2.0
            config["eq_profiles"] = eq
            save_config(config)
            self._set_feedback("boost_bass", "Bass boosted by +2 dB on all profiles.")
            return
        if "reduce treble" in t:
            eq = config.get("eq_profiles", {})
            for k in eq.keys():
                eq[k]["high"] = eq[k].get("high", 0.0) - 2.0
            config["eq_profiles"] = eq
            save_config(config)
            self._set_feedback("reduce_treble", "Treble reduced by -2 dB on all profiles.")
            return

        # feature tier
        if "performance light mode" in t or "light mode" in t:
            config["feature_tier"] = "LIGHT"
            save_config(config)
            self._set_feedback("feature_tier_light", "Feature tier set to LIGHT (reduced CPU).")
            return
        if "performance full mode" in t or "full mode" in t:
            config["feature_tier"] = "FULL"
            save_config(config)
            self._set_feedback("feature_tier_full", "Feature tier set to FULL.")
            return

        # network
        if "disable ping" in t:
            config["ping_enabled"] = False
            save_config(config)
            self._set_feedback("disable_ping", "Ping disabled for performance.")
            return
        if "enable ping" in t:
            config["ping_enabled"] = True
            save_config(config)
            self._set_feedback("enable_ping", "Ping enabled.")
            return
        if "disable geoip" in t:
            config["geoip_enabled"] = False
            save_config(config)
            self._set_feedback("disable_geoip", "GeoIP disabled for performance.")
            return
        if "enable geoip" in t:
            config["geoip_enabled"] = True
            save_config(config)
            self._set_feedback("enable_geoip", "GeoIP enabled.")
            return
        if "optimize network" in t:
            ok = optimize_network()
            msg = "Network optimization executed." if ok else "Network optimization failed."
            self._set_feedback("optimize_network", msg)
            return

        # mic / self-heal toggles
        if "disable mic watchdog" in t:
            config["mic_usb_watchdog_enabled"] = False
            save_config(config)
            self._set_feedback("disable_mic_watchdog", "Mic USB watchdog disabled.")
            return
        if "enable mic watchdog" in t:
            config["mic_usb_watchdog_enabled"] = True
            save_config(config)
            self._set_feedback("enable_mic_watchdog", "Mic USB watchdog enabled.")
            return

        # generic
        if "status report" in t or "audio status" in t:
            self._set_feedback("status_report", "Status report requested.")
            return

        # fallback
        self._set_feedback("unknown", f"No mapped operator command for: {text}")

# -----------------------------
# Distributed Audio Mesh
# -----------------------------
def audio_mesh_broadcast_state(state: Dict[str, Any]):
    if not config.get("audio_mesh_enabled", False):
        return
    peers = config.get("audio_mesh_peers", [])
    for peer in peers:
        try:
            requests.post(peer.rstrip("/") + "/audio_mesh_state", json=state, timeout=2)
        except Exception:
            continue

class AudioMeshHandler(BaseHTTPRequestHandler):
    def _send_json(self, obj: Any, code: int = 200):
        data = json.dumps(obj).encode("utf-8")
        self.send_response(code)
        self.send_header("Content-Type", "application/json")
    ...
    def do_POST(self):
        if self.path == "/audio_mesh_state":
            ...
            try:
                ...
                self._send_json({"status": "error", "reason": "invalid json"}, 400)
                return
            log_event({"type": "audio_mesh_state", "state": state, "timestamp": time.time()})
            self._send_json({"status": "ok"})
        else:
            self._send_json({"status": "not_found"}, 404)

def start_audio_mesh_server():
    if not config.get("audio_mesh_enabled", False):
        return
    port = int(config.get("ti_port", 8099)) + 1
    def run():
        try:
            server = HTTPServer(("0.0.0.0", port), AudioMeshHandler)
            server.serve_forever()
        except Exception:
            pass
    threading.Thread(target=run, daemon=True).start()
...
# -----------------------------
EXCLUSIVE_APPS = [
    "discord.exe", "teams.exe", "zoom.exe", "obs64.exe",
    "obs32.exe", "slobs.exe", "skype.exe"
]
VOICE_APPS = ["discord.exe", "steam.exe"]
...
class GameGuardianGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("System Network + Audio AI Helper")
        ...
        load_plugins()
        start_ti_server()
        start_audio_mesh_server()

        self.voice_monitor = VoiceMonitor()
        threading.Thread(target=self.voice_monitor.loop, daemon=True).start()

        self.operator_console = AIOperatorConsole()
        self.macro_engine = self.voice_monitor.macro_engine
        self.stt_engine = STTMacroEngine(self.macro_engine, self.operator_console.handle)
        threading.Thread(target=self.stt_engine.loop, daemon=True).start()
        ...
        self.block_selected_btn = tk.Button(top_frame, text="Block Selected IP (Admin)", command=self.block_selected_ip, bg=self.btn_bg, fg=self.btn_fg)
        self.block_selected_btn.pack(side=tk.LEFT, padx=5)

        self.replay_btn = tk.Button(top_frame, text="Replay Log", command=self.replay_log, bg=self.btn_bg, fg=self.btn_fg)
        self.replay_btn.pack(side=tk.LEFT, padx=5)

        self.dns_btn = tk.Button(top_frame, text="Check DNS Hijack", command=self.check_dns_hijack, bg=self.btn_bg, fg=self.btn_fg)
        self.dns_btn.pack(side=tk.LEFT, padx=5)

        self.opt_btn = tk.Button(top_frame, text="Optimize Network", command=self.run_optimize_network, bg=self.btn_bg, fg=self.btn_fg)
        self.opt_btn.pack(side=tk.LEFT, padx=5)

        self.settings_btn = tk.Button(top_frame, text="Settings", command=self.open_settings, bg=self.btn_bg, fg=self.btn_fg)
        self.settings_btn.pack(side=tk.LEFT, padx=5)

        self.dark_toggle_btn = tk.Button(top_frame, text="Toggle Dark Mode", command=self.toggle_dark_mode, bg=self.btn_bg, fg=self.btn_fg)
        self.dark_toggle_btn.pack(side=tk.LEFT, padx=5)

        self.minimize_tray_btn = tk.Button(top_frame, text="Minimize to Tray", command=self.minimize_to_tray, bg=self.btn_bg, fg=self.btn_fg)
        self.minimize_tray_btn.pack(side=tk.LEFT, padx=5)

        self.playback_btn = tk.Button(top_frame, text="Threat Playback", command=self.play_threat_animation, bg=self.btn_bg, fg=self.btn_fg)
        self.playback_btn.pack(side=tk.LEFT, padx=5)

        self.update_btn = tk.Button(top_frame, text="Check Update", command=self.check_update_button, bg=self.btn_bg, fg=self.btn_fg)
        self.update_btn.pack(side=tk.LEFT, padx=5)

        self.cloud_btn = tk.Button(top_frame, text="Cloud Sync Now", command=self.cloud_sync_now, bg=self.btn_bg, fg=self.btn_fg)
        self.cloud_btn.pack(side=tk.LEFT, padx=5)

        self.train_btn = tk.Button(top_frame, text="Train ML Model", command=self.train_ml_button, bg=self.btn_bg, fg=self.btn_fg)
        self.train_btn.pack(side=tk.LEFT, padx=5)

        self.audio_map_btn = tk.Button(top_frame, text="Audio Routing Map", command=self.show_audio_map, bg=self.btn_bg, fg=self.btn_fg)
        self.audio_map_btn.pack(side=tk.LEFT, padx=5)

        self.audio_graph_btn = tk.Button(top_frame, text="Audio Graph View", command=self.show_audio_graph_view, bg=self.btn_bg, fg=self.btn_fg)
        ...
        self.summary_text = tk.Text(side_frame, width=32, height=6, bg="#101010", fg="#FFFFFF", wrap="word")
        self.summary_text.pack(pady=5)
        self.summary_text.insert("1.0", "Threat and anomaly summaries will appear here.")
        self.summary_text.config(state="disabled")

        tk.Label(side_frame, text="Audio Intelligence (Voice / Clarity / RL EQ)", bg=self.bg_color, fg=self.fg_color).pack(pady=5)
        self.audio_text = tk.Text(side_frame, width=32, height=7, bg="#101010", fg="#FFFFFF", wrap="word")
        self.audio_text.pack(pady=5)
        self.audio_text.insert("1.0", "Voice/VAD/clarity/RL EQ status will appear here.")
        self.audio_text.config(state="disabled")

        tk.Label(side_frame, text="AI Operator Console (Voice Commands)", bg=self.bg_color, fg=self.fg_color).pack(pady=5)
        self.operator_text = tk.Text(side_frame, width=32, height=5, bg="#101010", fg="#00FFAA", wrap="word")
        self.operator_text.pack(pady=5)
        self.operator_text.insert("1.0", "Last operator command + feedback will appear here.")
        self.operator_text.config(state="disabled")
    ...
    def open_settings(self):
        ...
        tk.Checkbutton(win, text="Enable Voice Macros", variable=macro_var,
                       bg=self.bg_color, fg=self.fg_color, selectcolor=self.bg_color).pack(pady=5)

        stt_var = tk.BooleanVar(value=config.get("stt_macros_enabled", True))
        tk.Checkbutton(win, text="Enable STT Macro Engine", variable=stt_var,
                       bg=self.bg_color, fg=self.fg_color, selectcolor=self.bg_color).pack(pady=5)

        rl_var = tk.BooleanVar(value=config.get("rl_eq_enabled", False))
        tk.Checkbutton(win, text="Enable RL Adaptive EQ", variable=rl_var,
                       bg=self.bg_color, fg=self.fg_color, selectcolor=self.bg_color).pack(pady=5)

        aiop_var = tk.BooleanVar(value=config.get("ai_operator_enabled", False))
        tk.Checkbutton(win, text="Enable AI Operator Mode", variable=aiop_var,
                       bg=self.bg_color, fg=self.fg_color, selectcolor=self.bg_color).pack(pady=5)

        mesh_var = tk.BooleanVar(value=config.get("audio_mesh_enabled", False))
        tk.Checkbutton(win, text="Enable Audio Mesh", variable=mesh_var,
                       bg=self.bg_color, fg=self.fg_color, selectcolor=self.bg_color).pack(pady=5)

        tk.Label(win, text="Audio Mesh Peers (comma-separated URLs):", bg=self.bg_color, fg=self.fg_color).pack(pady=5)
        mesh_peers_var = tk.StringVar(value=",".join(config.get("audio_mesh_peers", [])))
        tk.Entry(win, textvariable=mesh_peers_var).pack(pady=5)

        tk.Label(win, text="Feature Tier (LIGHT/FULL):", bg=self.bg_color, fg=self.fg_color).pack(pady=5)
        tier_var = tk.StringVar(value=config.get("feature_tier", "FULL"))
        tk.Entry(win, textvariable=tier_var).pack(pady=5)

        ping_en_var = tk.BooleanVar(value=config.get("ping_enabled", True))
        tk.Checkbutton(win, text="Enable Ping", variable=ping_en_var,
                       bg=self.bg_color, fg=self.fg_color, selectcolor=self.bg_color).pack(pady=5)

        geo_en_var = tk.BooleanVar(value=config.get("geoip_enabled", True))
        tk.Checkbutton(win, text="Enable GeoIP", variable=geo_en_var,
                       bg=self.bg_color, fg=self.fg_color, selectcolor=self.bg_color).pack(pady=5)

        tk.Label(win, text="Mesh Update Interval (s):", bg=self.bg_color, fg=self.fg_color).pack(pady=5)
        mesh_int_var = tk.DoubleVar(value=config.get("mesh_update_interval", 5.0))
        tk.Entry(win, textvariable=mesh_int_var).pack(pady=5)
        ...
        def save_and_close():
            ...
            config["stt_macros_enabled"] = bool(stt_var.get())
            config["rl_eq_enabled"] = bool(rl_var.get())
            config["ai_operator_enabled"] = bool(aiop_var.get())
            config["audio_mesh_enabled"] = bool(mesh_var.get())
            mesh_str = mesh_peers_var.get().strip()
            config["audio_mesh_peers"] = [p.strip() for p in mesh_str.split(",") if p.strip()]
            config["feature_tier"] = tier_var.get().strip().upper() or "FULL"
            config["ping_enabled"] = bool(ping_en_var.get())
            config["geoip_enabled"] = bool(geo_en_var.get())
            config["mesh_update_interval"] = float(mesh_int_var.get())
            save_config(config)
    ...
    def update_loop(self):
        ...
        while self.running:
            ...
            now = time.time()
            if now - last_mesh_push >= config.get("mesh_update_interval", 5.0):
                last_mesh_push = now
                audio_mesh_broadcast_state({
                    "timestamp": now,
                    "clarity": self.voice_monitor.last_clarity_score,
                    "latency_samples": self.last_latencies[-5:],
                })
    ...
    def update_voice_panel(self):
        ...
        self.audio_text.insert(
            ...
            f"RL EQ: {'ON' if config.get('rl_eq_enabled', False) else 'OFF'}\n"
            f"AI Operator: {'ON' if config.get('ai_operator_enabled', False) else 'OFF'}\n"
            f"STT Macros: {'ON' if config.get('stt_macros_enabled', True) else 'OFF'}\n"
            f"Audio Mesh: {'ON' if config.get('audio_mesh_enabled', False) else 'OFF'}\n"
            f"Feature Tier: {config.get('feature_tier', 'FULL')}\n"
        )
        self.audio_text.config(state="disabled")

    def update_operator_panel(self):
        op = self.operator_console
        self.operator_text.config(state="normal")
        self.operator_text.delete("1.0", "end")
        if op.last_timestamp:
            ts = datetime.fromtimestamp(op.last_timestamp).strftime("%H:%M:%S")
            self.operator_text.insert(
                "1.0",
                f"[{ts}] Command: {op.last_command}\nFeedback: {op.last_feedback}"
            )
        else:
            self.operator_text.insert("1.0", "No operator commands yet.")
        self.operator_text.config(state="disabled")
    ...
    def update_table(self):
        ...
        for proc in games:
            ...
            for c in conns:
                ...
                ev_conn = {"type": "connection", "timestamp": time.time(), "values": values}
                log_event(ev_conn)
                run_plugins(ev_conn)
