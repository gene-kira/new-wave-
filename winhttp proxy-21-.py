...
# ============================================================
# DEFAULT CONFIG (SCHEMA v5 / HybridBrain v3 + Hybrid Generator)
# ============================================================
...
DEFAULT_CONFIG = {
    ...
    # Hybrid proxy generator
    "proxy_gen_enabled": True,
    "proxy_gen_local_count": 20,
    "proxy_gen_local_base_port": 50000,
    "proxy_gen_public_sources": [
        "https://example.com/public_proxies.txt"
    ],
    "proxy_gen_cloud_url": "",
    "proxy_gen_interval": 900,
}
...
def push_node_state():
    ...
    if not url:
        return
    try:
        payload = json.dumps({"node": desc}, indent=2).encode("utf-8")
...
# ============================================================
# HYBRID PROXY GENERATOR (LOCAL + PUBLIC + CLOUD)
# ============================================================

def generate_local_proxies():
    with config_lock:
        base_port = config.get("proxy_gen_local_base_port", 50000)
        count = config.get("proxy_gen_local_count", 20)
        proxies = config.get("proxies", [])
    new_proxies = []
    for i in range(count):
        p = f"127.0.0.1:{base_port + i}"
        if p not in proxies:
    ...
    with config_lock:
        ...
        config["config_version"] = config.get("config_version", 1) + 1
        save_config(config)
    logging.info(f"[PROXY_GEN] Local synthetic proxies added: {new_proxies}")

def fetch_public_proxies():
    with config_lock:
        sources = config.get("proxy_gen_public_sources", [])
        proxies = config.get("proxies", [])
    new_proxies = []
    for url in sources:
        try:
            ...
            for line in data.splitlines():
                ...
                if not line:
                    continue
                if ":" in line and line not in proxies and line not in new_proxies:
                    new_proxies.append(line)
        except Exception as e:
            logging.warning(f"[PROXY_GEN] Failed to fetch public proxies from {url}: {e}")
    with config_lock:
        config["proxies"].extend(new_proxies)
        config["config_version"] = config.get("config_version", 1) + 1
        save_config(config)
    logging.info(f"[PROXY_GEN] Public proxies added: {len(new_proxies)}")

def fetch_cloud_proxies():
    with config_lock:
        url = config.get("proxy_gen_cloud_url", "").strip()
        proxies = config.get("proxies", [])
    if not url:
        return
    new_proxies = []
    try:
        ...
        cloud_list = payload.get("proxies", [])
        for p in cloud_list:
            if isinstance(p, str) and ":" in p and p not in proxies and p not in new_proxies:
                new_proxies.append(p)
    except Exception as e:
        logging.warning(f"[PROXY_GEN] Failed to fetch cloud proxies: {e}")
        return
    with config_lock:
        config["proxies"].extend(new_proxies)
        config["config_version"] = config.get("config_version", 1) + 1
        save_config(config)
    logging.info(f"[PROXY_GEN] Cloud proxies added: {len(new_proxies)}")

def proxy_generator_thread():
    while True:
        try:
            touch_heartbeat("proxy_generator")
            with config_lock:
                enabled = config.get("proxy_gen_enabled", True)
                interval = config.get("proxy_gen_interval", 900)
            if not enabled:
                time.sleep(30)
                continue
            generate_local_proxies()
            fetch_public_proxies()
            fetch_cloud_proxies()
            publish_event("proxy_changed")
            time.sleep(max(120, interval))
        except Exception as e:
            logging.error(f"[PROXY_GEN] thread error: {e}")
            time.sleep(60)

# ============================================================
# FUSION / PREDICTIVE / AI GOVERNOR / EMERGENT / SELF-TUNE
# ============================================================
...
def refresh():
    try:
        ...
        with config_lock:
            ...
            proxy_gen_enabled = config.get("proxy_gen_enabled", True)
        if history:
            avg_health = sum(history) / len(history)
        else:
            avg_health = 100.0
        ...
        status.set(
            ...
            f"Proxy Generator: {'ON' if proxy_gen_enabled else 'OFF'}\n"
...
root = tk.Tk()
root.title("Proxy Controller Pro - HybridBrain v3 FULL FUSION")
root.geometry("900x780")
root.configure(bg="#202020")

tk.Label(
    root,
    text="Proxy Controller Pro - HybridBrain v3 (Full Fusion + Hybrid Generator)",
    fg="lime",
    bg="#202020",
...
tk.Button(
    btn_frame,
    text="APPLY BEST PROXY",
    width=20,
    command=apply_best_proxy
).grid(row=2, column=1, padx=5, pady=5)

tk.Button(
    btn_frame,
    text="RUN PROXY GENERATOR NOW",
    width=25,
    command=lambda: threading.Thread(target=proxy_generator_thread, daemon=True).start()
...
with config_lock:
    ...
    proxy_gen_var = tk.BooleanVar(value=config.get("proxy_gen_enabled", True))
...
def on_toggle(key, var):
    with config_lock:
        ...
        config["config_version"] = config.get("config_version", 1) + 1
        save_config(config)
    logging.info(f"[UI] {key} set to {config[key]}")
    refresh()
...
tk.Checkbutton(
    ...
    command=lambda: on_toggle("auto_rotate", auto_rotate_var),
    fg="white",
    bg="#202020",
    selectcolor="#202020",
...
tk.Checkbutton(
    ...
    command=lambda: on_toggle("borg_ai", borg_ai_var),
    fg="lime",
    bg="#202020",
    selectcolor="#202020",
    activebackground="#202020"
...
tk.Checkbutton(
    ...
    command=lambda: (on_toggle("startup", startup_var), sync_startup_from_config()),
    fg="white",
    bg="#202020",
    selectcolor="#202020",
    activebackground="#202020"
...
tk.Checkbutton(
    ...
    command=lambda: (on_toggle("stealth_mode", stealth_var), enable_proxy_threaded()),
    fg="orange",
    bg="#202020",
    selectcolor="#202020",
    activebackground="#202020"
...
tk.Checkbutton(
    ...
    command=lambda: on_toggle("cluster_sync", cluster_var),
    fg="cyan",
    bg="#202020",
    selectcolor="#202020",
    activebackground="#202020"
...
tk.Checkbutton(
    ...
    command=lambda: on_toggle("predictive_enabled", predictive_var),
    fg="magenta",
    bg="#202020",
    selectcolor="#202020",
    activebackground="#202020"
...
tk.Checkbutton(
    ...
    text="Fusion Engine",
    variable=fusion_var,
    command=lambda: on_toggle("fusion_enabled", fusion_var),
    fg="yellow",
    bg="#202020",
    selectcolor="#202020",
    activebackground="#202020"
...
tk.Checkbutton(
    ...
    text="AI Governor",
    variable=ai_gov_var,
    command=lambda: on_toggle("ai_governor_enabled", ai_gov_var),
    fg="red",
    bg="#202020",
    selectcolor="#202020",
    activebackground="#202020"
...
tk.Checkbutton(
    ...
    command=lambda: on_toggle("emergent_enabled", emergent_var),
    fg="cyan",
    bg="#202020",
    selectcolor="#202020",
    activebackground="#202020"
...
tk.Checkbutton(
    ...
    command=lambda: on_toggle("self_tune_enabled", self_tune_var),
    fg="white",
    bg="#202020",
    selectcolor="#202020",
    activebackground="#202020"
...
tk.Checkbutton(
    ...
    text="RL Agent",
    variable=rl_var,
    command=lambda: on_toggle("rl_enabled", rl_var),
    fg="magenta",
    bg="#202020",
    selectcolor="#202020",
    activebackground="#202020"
...
tk.Checkbutton(
    ...
    text="ML Routing",
    variable=ml_var,
    command=lambda: on_toggle("ml_routing_enabled", ml_var),
    fg="red",
    bg="#202020",
    selectcolor="#202020",
    activebackground="#202020"
...
tk.Checkbutton(
    options_frame,
    text="Hybrid Proxy Generator",
    variable=proxy_gen_var,
    command=lambda: on_toggle("proxy_gen_enabled", proxy_gen_var),
    fg="lime",
    bg="#202020",
    selectcolor="#202020",
    activebackground="#202020"
...
cluster_url_entry.grid(row=4, column=1, padx=10)
with config_lock:
...
threading.Thread(target=proxy_generator_thread, daemon=True).start()
