...
+ RT Browser VPN Saving + ASN/ISP Caching + Multi‑Provider Geo Backoff
"""
...
ASN_CACHE_PATH = "asn_cache.json"
ASN_CACHE_LOCK = threading.Lock()
ASN_CACHE = {
    "entries": {},
    "ttl": 6 * 3600,  # 6 hours
}
ASN_BACKOFF_UNTIL = 0.0
...
def _load_asn_cache():
    global ASN_CACHE
    if os.path.exists(ASN_CACHE_PATH):
        try:
            with open(ASN_CACHE_PATH, "r", encoding="utf-8") as f:
                data = json.load(f)
            if isinstance(data, dict):
                ASN_CACHE.update(data)
        except Exception as e:
            log(f"[ASN] Failed to load ASN cache: {e}")

def _save_asn_cache():
    try:
        with open(ASN_CACHE_PATH, "w", encoding="utf-8") as f:
            json.dump(ASN_CACHE, f, indent=2)
    except Exception as e:
        log(f"[ASN] Failed to save ASN cache: {e}")
...
def _fetch_ipinfo(ip):
    """
    ASN/ISP lookup with caching, multi-provider fallback and 429 backoff.
    """
    global ASN_BACKOFF_UNTIL
    now = time.time()
    with ASN_CACHE_LOCK:
        if not ASN_CACHE["entries"]:
            _load_asn_cache()
        entry = ASN_CACHE["entries"].get(ip)
        if entry:
            if now - entry.get("ts", 0) < ASN_CACHE.get("ttl", 6 * 3600):
                return entry.get("data", {})
    providers = []
    providers.append(("ipinfo", f"https://ipinfo.io/{ip}/json"))
    providers.append(("ip.sb", f"https://api.ip.sb/geoip/{ip}"))
    providers.append(("ipapi", f"https://ipapi.co/{ip}/json/"))
    providers.append(("ipwhois", f"https://ipwho.is/{ip}"))
    import urllib.request
    import urllib.error
    result = {}
    for name, url in providers:
        if name == "ipinfo" and now < ASN_BACKOFF_UNTIL:
            continue
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "BorgASN/1.0"})
            with urllib.request.urlopen(req, timeout=5) as resp:
                raw = resp.read().decode("utf-8")
            data = json.loads(raw)
            if name == "ipinfo":
                result = _parse_ipinfo(data)
            elif name == "ip.sb":
                result = _parse_ip_sb(data)
            elif name == "ipapi":
                result = _parse_ipapi(data)
            elif name == "ipwhois":
                result = _parse_ipwhois(data)
            if result.get("asn") or result.get("isp"):
                with ASN_CACHE_LOCK:
                    ASN_CACHE["entries"][ip] = {"ts": now, "data": result}
                    _save_asn_cache()
                return result
        except urllib.error.HTTPError as e:
            if name == "ipinfo" and e.code == 429:
                backoff = 900 if ASN_BACKOFF_UNTIL <= now else (ASN_BACKOFF_UNTIL - now) * 2
                ASN_BACKOFF_UNTIL = now + backoff
                log(f"[PUBIP] ipinfo 429 for {ip}, backoff {int(backoff)}s")
            else:
                log(f"[PUBIP] {name} lookup failed for {ip}: {e}")
        except Exception as e:
            log(f"[PUBIP] {name} lookup failed for {ip}: {e}")
    return {}
...
def public_ip_worker():
    while True:
        ...
        with PUBLIC_IP_LOCK:
            ...
            prev_ip = PUBLIC_IP_STATE["current_ip"]
            ...
            if ip and (ip != prev_ip):
                need_geo = True
            elif now - PUBLIC_IP_STATE.get("last_geo_update", 0) > 1800:
                need_geo = True
            if ip and need_geo:
                info = _fetch_ipinfo(ip)
                ...
                PUBLIC_IP_STATE["last_geo_update"] = now
...
class BorgGUI:
    ...
    def _update_system_dns_status(self):
        ...
        if dns_set and dns_value == "127.0.0.1":
            ...
            if err:
                self.system_dns_status_label.config(
                    text=f"System DNS: {adapter} (ERR: {err})",
                    foreground="red"
                )
            else:
                self.system_dns_status_label.config(
                    text=f"System DNS: {adapter} (DNS={dns_value})",
                    foreground="orange"
                )
...
def main():
    log("=== Borg VPN Overmind starting (GPU Threat + Swarm Telemetry + LLM Anomaly + RT Browser VPN Saving + ASN Cache) ===")
