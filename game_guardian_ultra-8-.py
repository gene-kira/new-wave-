#!/usr/bin/env python
# game_guardian_ultra_ai_cluster_mic_supervisor_plus_audio_intel.py
#
# Game Guardian ULTRA AI CLUSTER + MIC / AUDIO INTEL:
...
DEFAULT_CONFIG = {
    ...
    # Game-specific voice optimization profiles
    # Example: "cs2": {"mic_device": "HyperX", "sample_rate": 48000}
    "voice_profiles": {
...
# -----------------------------
# VOICE MONITOR (AI-ish quality + VAD)
# -----------------------------
class VoiceMonitor:
    def __init__(self):
    ...
    def _audio_callback(self, indata, frames, time_info, status):
        try:
            ...
            # crude quality score: high energy, low clipping
            quality = 0.0
            if self.is_speaking:
                quality = max(0.0, 1.0 - clip_ratio)
...
class MicSelfHealer:
    ...
    def restart_audio_service(self):
        ...
        end_ts = time.time()
        # Audio driver crash forensics log
        ev = {
    ...
    def usb_watchdog_reset(self):
        if not config.get("mic_usb_watchdog_enabled", True):
            return
        # Kernel-style USB reset hook via PowerShell / devcon-like call
        # This assumes you have appropriate tools installed; errors are ignored.
        try:
            cmd = [
                "powershell",
                "-Command",
...
class GameGuardianGUI:
    ...
    def update_voice_panel(self):
        ...
        self.voice_label.config(
            text=f"Voice: {speaking}, quality={quality:.2f}, clip={clip:.2f}",
            fg=("green" if quality > 0.7 else BLUE)
        )
    ...
    def update_table(self):
        ...
        for proc in games:
            ...
            CURRENT_GAME_NAME = exe  # update global for voice profiles
