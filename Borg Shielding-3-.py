...
# =========================
# OS helpers (hybrid capture + firewall + process mapping)
# =========================
...
def parse_connections_with_pid(netstat_output):
    """
    Try to parse IP:port and PID from netstat/ss output.
    Returns list of dicts: {"ip_port": "...", "pid": "..."}
    """
    system = platform.system().lower()
    results = []
    ...
    if "windows" in system:
        ...
        for line in netstat_output.splitlines():
            ...
            if len(parts) >= 5 and ("TCP" in parts[0] or "UDP" in parts[0]):
                foreign = parts[2] if "windows" in system else parts[1]
                pid = parts[-1]
                if ":" in foreign and not foreign.startswith("127.") and not foreign.startswith("::1"):
                    results.append({"ip_port": foreign, "pid": pid})
    else:
        # Linux/macOS: netstat -anp or ss -ntp: look for ip:port and pid/program
        for line in netstat_output.splitlines():
            line = line.strip()
            if not line:
                continue
        ...
        for item in conns:
            ...
            if self.baseline.is_learning_phase():
                ...
                if is_baseline:
                    ...
                    if role == "ADVERSARY":
                        level = THREAT_HIGH
                        desc = f"NEW adversary {ip_port} ({country}) pid={pid} proc={proc_name}"
                    else:
                        level = THREAT_HIGH
                        desc = f"NEW non-baseline {ip_port} ({country}) pid={pid} proc={proc_name}"
            ...
            if IP in pkt:
                ...
                if self.baseline.is_learning_phase():
                    ...
                    if is_baseline:
                        ...
                        if role == "ADVERSARY":
                            level = THREAT_HIGH
                            desc = f"[PCAP] NEW adversary packet to {dst} ({country})"
                        else:
                            level = THREAT_HIGH
                            desc = f"[PCAP] NEW non-baseline packet to {dst} ({country})"
...
# =========================
# Borg Queen (decision core + firewall control)
# =========================
...
class BorgShieldGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Borg Shield Command (Hybrid, GeoIP, Baseline, Firewall, Per-Process)")
