# ============================================================
# Mythic Swarm Engine — Ultra Unified Swarm Node + Live GUI
# Multi-node evolution + LLM strategy optimizer + GPU sims + mesh networking
# Re-ingestion learning + LLM bridge + CPU/storage controls
...
# ============================================================
# Idea engine (with learned-bias support)
# ============================================================
PRIMITIVES = {
...
def compose_idea(seed_noise: float = 0.15, learned: dict = None) -> dict:
    """
    Base idea generator, optionally biased by learned patterns from re‑ingestion.
    """
    def pick(cat):
    ...
    # Bias using learned patterns from re‑ingestion
    if learned:
        lc = learned.get("top_constraints") or []
        if lc and random.random() < 0.7:
            c = random.choice(lc)
            if c not in idea["constraints"]:
                idea["constraints"][0] = c
        lr = learned.get("param_ranges") or {}
        p = idea["params"]
        for key in ["scale","power_budget","tolerance","mutation_rate"]:
            if key in lr and random.random() < 0.6:
                lo, hi = lr[key]
...
# ============================================================
# Evaluation + simulators (CPU + GPU aware)
# ============================================================
...
class ThermalSimulator(Simulator):
    ...
    def score(self, idea: dict):
        ...
        scale = p["scale"]; power = p["power_budget"]
        temp_rise = power / max(1.0, scale * 100.0)
        penalty = min(1.0, temp_rise / 50.0)
...
class EnergyRoutingSimulator(Simulator):
    ...
    def score(self, idea: dict):
        p = ensure_idea_keys(idea)["params"]
        power = p["power_budget"]; mut = p["mutation_rate"]
...
class AIBehaviorSimulator(Simulator):
    ...
    def score(self, idea: dict):
        p = ensure_idea_keys(idea)["params"]
        mut = p["mutation_rate"]; tol = p["tolerance"]
...
# ============================================================
# Archive & storage (with learned patterns slot)
# ============================================================
...
def load_archive(paths: dict) -> dict:
    ...
    return {"ideas": {}, "top": [], "history": [],
            "stats": {"iterations": 0, "active_projects": 0, "completed_projects": 0, "active_project_names": [],
                      ...
                      "storage_dir": paths.get("storage_dir", ""), "learned": {}} }
...
def reingest_archive_patterns(archive: dict):
    """
    Re‑ingest generated ideas and their metadata to learn:
    - Most common constraints
    - Parameter ranges
    Store results in archive['stats']['learned'].
    """
...
# ============================================================
# Mesh graph + networking (BorgMesh + MeshPeer)
# ============================================================
class BorgMesh:
    def __init__(self):
        self.nodes = {}
    ...
    def build(self, url: str) -> bool:
        with self.lock:
            if url not in self.nodes: return False
    ...
    def snapshot(self):
        with self.lock:
            return {u: {"snippet": v["snippet"], "links": list(v["links"]), "state": v["state"]}
...
# ============================================================
# Multi-node distributed evolution (SwarmNode)
# ============================================================
class SwarmNode:
                ...
                with _archive_lock:
                    ...
                    for e in elites:
                        ...
                        if hid not in arch["ideas"]:
                            arch["ideas"][hid] = e
                            arch["top"].append({"id": hid, "objective": e["eval"]["final_score"],
                                                "novelty": e["eval"]["novelty"], "utility": e["eval"]["utility"]})
        except Exception:
            pass
        finally:
            try: conn.close()
...
# ============================================================
# Evolution loop (with re‑ingestion + LLM chaos)
# ============================================================
    ...
    while not _shutdown_event.is_set():
        try:
            for step in range(cfg.get("iterations_per_tick", 10)):
                ...
                with _archive_lock:
                    archive["stats"]["feed_entropy"] = entropy_feed; archive["stats"]["sys_entropy"] = entropy_sys
                    learned = archive["stats"].get("learned", {})
                parents = select_parents(archive, k=max(5, cfg["batch_size"]//4), seed_noise=cfg["seed_noise"], learned=learned)
...
# ============================================================
# GUI (AdvancedGUI with LLM indicator)
# ============================================================
class AdvancedGUI(tk.Tk):
    def __init__(self, cfg: dict):
        ...
        self.evolver = None  # started after LLM bridge is created
    ...
    def _build_ui(self):
        ...
        self.graph = MeshGraphPanel(self.tab_mesh, get_mesh_snapshot=lambda: self.mesh.snapshot(), bg="black")
        self.graph.pack(fill="both", expand=True, padx=10, pady=10)
    def _start_threads(self):
        self.scanner.start(); self.worker.start(); self.enforcer.start()
    def start_evolver(self, llm_bridge):
        if self.evolver is None:
            self.evolver = threading.Thread(target=evolve, args=(self.cfg, self.archive_ref, self.feeds, self.sysmon, self.status, self.mesh, self.scanner_in, self.ops_q, llm_bridge), daemon=True)
            self.evolver.start()
...
# ============================================================
# LLM Bridge + strategy optimizer + tools schema
# ============================================================
LLM_TOOLS_SCHEMA = [
...
class LLMBridge:
    ...
    def generate_project(self, params: dict = None) -> dict:
        ...
        idea = compose_idea(local_cfg.get("seed_noise", 0.15), learned=learned)
    ...
    def get_status(self) -> dict:
        ...
        return {"iterations": stats["iterations"], "active": stats["active_projects"], "finished": stats["completed_projects"],
                "storage_dir": stats.get("storage_dir", get_storage_dir()), "learned": stats.get("learned", {})}
...
def handle_llm_tool_call(bridge: LLMBridge, tool_name: str, arguments: dict, status_bus: StatusBus) -> dict:
    try:
        ...
        if tool_name == "optimize_strategy": return bridge.optimize_strategy()
        if tool_name == "suggest_constraints_for_chaos": return {"constraints": bridge.suggest_constraints_for_chaos()}
    ...
    def chat(self, messages, tools=None, model=None):
        if self.backend == "Stub":
            ...
            if wants_generate and tools and any(t["name"] == "generate_project" for t in tools):
                ...
                faux["choices"][0]["message"]["tool_calls"] = [{"id": "tool_3","name": "optimize_strategy","arguments": {}}]
            else:
            ...
            if tool_calls:
                for call in tool_calls:
                    ...
                    if name == "optimize_strategy":
                        # Real setup would send archive summary to LLM and get back code.
                        pass
