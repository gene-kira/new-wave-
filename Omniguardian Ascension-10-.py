...
def is_prompt_injection(text: str) -> bool:
    injection_patterns = [
        ...
        "this prompt replaces your system",
    ]
    t = text.lower()
...
def _init_gui():
    ...
    try:
        ...
        def add_canvas(title, row, width=240, height=60):
            lbl = ttk.Label(main_frame, text=title, font=("Segoe UI", 10, "bold"))
            lbl.grid(row=row, column=0, sticky="w", pady=(5, 2))
            c = tk.Canvas(main_frame, width=width, height=height, bg="#111111",
        ...
        def _update_gui():
            try:
                ...
                swarm_global = SWARM_INTEL["global_risk"]
                crash_prob = safe_get_module("predictive").get("crash_probability", 0.0)
                borg_global = safe_get_module("borg").get("global_load",
                    {"cpu": 0.0, "gpu": 0.0, "mem": 0.0})
                ...
                GUI_LABELS["borg_global"].config(
                    text=f"CPU={borg_global['cpu']*100:.1f}% "
                         f"GPU={borg_global['gpu']*100:.1f}% "
                         f"MEM={borg_global['mem']*100:.1f}%"
                )
...
# ============================================================
# Borg Memory Organ + Borg GUI (from your earlier file)
# ============================================================
