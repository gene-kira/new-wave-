...
+ Extreme Opera VPN Detection (endpoints + QUIC/TLS fingerprints + IP ranges + DPI hints)
"""
...
# ===== Extreme Opera VPN detection (mix of real + synthetic) =====

OPERA_VPN_ENDPOINT_DOMAINS = {
    "vpn.secureconnect.opera.com",
    "api.secureconnect.opera.com",
    "geo.opera-api.com",
    "auth.opera.com",
}

# Synthetic but plausible JA3-lite fingerprints for Opera VPN tunnels
OPERA_VPN_TLS_FPS = {
    "ja3-lite-0a1b2c3d",  # fake
    "ja3-lite-1f2e3d4c",  # fake
}

# Synthetic IP ranges (CIDR) for Opera VPN exit nodes (examples)
OPERA_VPN_IP_RANGES = [
    ("77.111.240.0", "77.111.255.255"),  # Opera proxy-ish range example
    ("185.26.180.0", "185.26.183.255"),  # synthetic
]
...
def _ip_in_range(ip, start, end):
    try:
        ip_i = int.from_bytes(socket.inet_aton(ip), "big")
        s_i = int.from_bytes(socket.inet_aton(start), "big")
        e_i = int.from_bytes(socket.inet_aton(end), "big")
        return s_i <= ip_i <= e_i
    except Exception:
        return False
...
def _is_opera_vpn_ip(ip):
    if not ip:
        return False
    for start, end in OPERA_VPN_IP_RANGES:
...
def _is_vpn_asn(asn, isp):
    ...
    if isp:
        for hint in KNOWN_VPN_ISP_HINTS:
            if isp and hint.lower() in isp.lower():
                return True
        for hint in OPERA_VPN_ISP_HINTS:
            if isp and hint.lower() in isp.lower():
                return True
    return False

def _parse_netstat_line(line):
    # Expect something like:
    #  TCP    192.168.1.10:12345   77.111.244.21:443   ESTABLISHED     1234
    parts = line.split()
    if len(parts) < 5:
        return None
    proto = parts[0].lower()
    ...
    try:
        ...
        return None
    return proto, local, remote, pid_int
    ...
    while True:
        try:
            ...
            for line in out.splitlines():
                ...
                rhost, rport = _split_host_port(remote)
                if not rhost:
                    continue
                ...
                vpn_like = _is_vpn_asn(asn, isp) or any(h in (ip or "").lower() for h in KNOWN_VPN_HOST_HINTS)

                # Extreme Opera VPN detection: endpoints + IP ranges
                if ip and _is_opera_vpn_ip(ip):
                    vpn_like = True
                    with PUBLIC_IP_LOCK:
                        PUBLIC_IP_STATE["signals"]["opera_vpn_activity"] = True
...
def _peek_tls_client_hello(sock, max_bytes=512):
    ...
    try:
        ...
        if not data:
            return None
        fp = hash(data[:64])
        fp_str = f"ja3-lite-{fp & 0xFFFFFFFF:08x}"
        # Opera VPN TLS hint
        if fp_str in OPERA_VPN_TLS_FPS:
            with PUBLIC_IP_LOCK:
                PUBLIC_IP_STATE["signals"]["opera_vpn_activity"] = True
        return fp_str
    except Exception:
...
class ProxyHandler(socketserver.StreamRequestHandler):
    ...
    def learn_domain(self, domain):
        ...
        if any_vpn:
            ...
            # No VPN active: still learn domain but mark as lower trust
            SITE_STORE.update_site(domain, "no-vpn", "0.0.0.0")
            BORG_MEMORY.update_domain(domain, "no-vpn", "0.0.0.0", threat_delta=0.1, trust_delta=0.0)
    ...
    def _update_dns_status(self):
        ...
        if fwd_ok:
            self.system_dns_status_label.config(
                text=f"System DNS Sync: OK (forwarder {DNS_FORWARDER_ADDR[1]})",
                foreground="green"
            )
        else:
            self.system_dns_status_label.config(
                text=f"System DNS Sync: ERROR ({fwd_err or 'unknown'})",
                foreground="red"
            )

    def _update_system_dns_status(self):
        pass  # already merged into _update_dns_status
...
def main():
    log("=== Borg VPN Overmind starting (GPU Threat + Swarm Telemetry + LLM Anomaly + RT Browser VPN Saving + Ghost Tunnel + Extreme Opera VPN) ===")
