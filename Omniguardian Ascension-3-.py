...
# ============================================================
# Codex Sentinel Forklift Node v10 – Omniguardian Apex
#
# Focus:
# - Precision detection (reduced false positives)
# - Deep context awareness (role-based interpretation)
# - Adaptive self-tuning (risk-aware scan cadence & thresholds)
#
# Modes:
# - LLM Engine (RPC + CLI)
...
def train_policy_net_step(sys_tel, latency_ms):
    ...
    # Adaptive scan cadence: faster when risk is high, slower when calm
...
def _score_process_anomaly(pid: int, name: str, rss: int, threads: int, read_bytes: int, write_bytes: int) -> float:
    ...
    role = _infer_context_role(lname)
...
def _predict_performance_risk():
    ...
    risk = "low"
    if cpu_spike or gpu_spike or background_heavy:
        risk = "medium"
    if (cpu_spike and gpu_spike) or len(background_heavy) > 3:
        risk = "high"
