#!/usr/bin/env python3
# ============================================================
# HybridBrain Unified Memory Daemon v8
...
#   - High-rate in-memory random reads/writes stressor
# ============================================================
...
class UnifiedMemoryEngine:
    def __init__(self, os_layer, swarm_client=None, ai_mode=DEFAULT_AI_MODE):
        ...
        # RAM cache is the primary in-memory store for reads/writes
        self.ram_cache = bytearray(16 * 1024 * 1024)
        ...
        # In-memory key/value map for random reads/writes
    ...
    def _choose_tier(self, key: str, data: bytes):
        ...
        # Random + sensor workloads prefer RAM strongly
    ...
    def _tier_migration_pass(self):
        ...
        for key, meta in list(self.residency_map.items()):
            ...
            if idle > 60 and tier == "ram" and priority <= PRIORITY_NETWORK:
                meta["tier"] = "disk"
            if idle > 300 and tier in ("ram", "disk") and priority <= PRIORITY_FILE:
                meta["tier"] = "swarm"
    ...
    # Core write: always keep a copy in memory_map for fast random reads
    ...
    # Core read: prefer memory_map (pure in-memory) for random access
            ...
            with self.lock:
                if key in self.memory_map:
                    ...
                    if tier == "ram":
                        ...
                        start2 = time.time()
                        self.disk_mmap.seek(0)
                        data = self.disk_mmap.read(size or self.disk_size)
                        elapsed = time.time() - start2
                        self.disk_bw_bytes += len(data)
...
# ============================================================
# REAL WORKLOAD MANAGER (Files + Network + Sensors + Random)
# ============================================================
...
class WorkloadManager:
    ...
    # Random in-memory stressor: pure random keys, random sizes, random reads
    def _random_workload_loop(self, rid):
        while self.running:
            try:
                ...
                # Random read: pick a random key from memory_map if available
        ...
        while self.running:
            try:
                if not self.engine.heartbeat_ok():
                    log_error("[WATCHDOG] Engine heartbeat lost → forcing safe mode + tier reset")
...
def main():
    log_info("=== HybridBrain Unified Memory Daemon v8 (AI Governor + Random In-Memory) ===")
