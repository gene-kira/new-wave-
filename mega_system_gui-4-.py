...
class RuntimeEnv:
    ...
    def _init_libs(self):
        # Light-touch: try import, no pip here (pip via ensure_lib when needed)
...
class ModelTierManager:
    ...
    def _init_llm_onnx(self):
        ...
        if not onnxruntime or not transformers:
            return
        try:
            model_id = "gpt2"  # example; you can change to your ONNX-exported model
            LOGGER.info("[LLM-ONNX] Initializing ONNX LLM...")
    ...
    def _init_gguf_llm(self):
        ...
        try:
            ...
            self.gguf_llm = llama_cpp.Llama(
                ...
                n_ctx=4096,
                n_gpu_layers=35 if RUNTIME.has_cuda else 0,
            )
            self.llm_backend = "gguf"
            self.llm_name = "llama-3-8b.gguf"
    ...
    def _init_vit_onnx(self):
        ...
        if not onnxruntime:
            return
        try:
            LOGGER.info("[ViT-ONNX] Initializing ONNX ViT...")
            sess_options = onnxruntime.SessionOptions()
            self.vit_onnx_session = onnxruntime.InferenceSession(
                "vit.onnx",  # path to your ONNX ViT
                providers=RUNTIME.onnx_providers,
                sess_options=sess_options,
            )
            self.vit_backend = "onnx"
            if not self.vit_name:
                self.vit_name = "vit.onnx"
    ...
    def _init_whisper_onnx(self):
        ...
        if not onnxruntime:
            return
        try:
            LOGGER.info("[Whisper-ONNX] Initializing ONNX Whisper...")
            sess_options = onnxruntime.SessionOptions()
            self.whisper_onnx_session = onnxruntime.InferenceSession(
                "whisper.onnx",  # path to your ONNX Whisper
                providers=RUNTIME.onnx_providers,
                sess_options=sess_options,
            )
            self.whisper_backend = "onnx"
            if not self.whisper_name:
                self.whisper_name = "whisper.onnx"
    ...
    def run_vit(self, image_path: str) -> Dict[str, Any]:
        ...
        if self.vit_backend == "onnx" and self.vit_onnx_session:
            ...
            outputs = self.vit_onnx_session.run(None, {"input": img_arr})
    ...
    def run_whisper(self, audio_path: str) -> Dict[str, Any]:
        ...
        if self.whisper_backend == "onnx" and self.whisper_onnx_session:
            ...
            # Minimal example: assume mel spectrogram precomputed externally
            # Here we just stub; you can wire real preprocessing.
            outputs = self.whisper_onnx_session.run(None, {"input": [[0.0]]})
            transcript = "ONNX-Whisper transcript (stub)"
...
class MegaSystemApp(tk.Tk):
    def __init__(self):
        ...
        # Preload heavy libs in background to avoid blocking UI
    ...
    def _build_layout(self):
        ...
        self.canvas = tk.Canvas(anim_frame, bg="#050510", highlightthickness=0, height=220)
    ...
    def _run_pipeline_worker(self, text: str):
        ...
        try:
            result = self.orchestrator.run_full_pipeline(text)
    ...
    def update_swarm_visual(self, result: Dict[str, Any]):
        ...
        cx, cy = self.nodes.get("swarm", (620, 60))
        radius = 50
        n = len(agents)
