...
MODEL_PATHS = {
    "llm_onnx_model_id": "gpt2",  # tokenizer id (local or HF-style, but we force local_files_only)
...
def lazy_faiss():
    global faiss
    if faiss is None:
        faiss = ensure_lib("faiss", "faiss-cpu")
    return faiss
...
class VectorMemoryStore:
    """
    Simple FAISS-like vector store stub for long-term memory.
    If faiss is not available, falls back to in-memory brute force.
    """
    ...
    def _init_index(self):
        ...
        if faiss:
            ...
            LOGGER.info("[VectorMemory] Using FAISS index.")
        else:
            LOGGER.info("[VectorMemory] FAISS not available, using brute-force memory.")
    ...
    def search(self, query: List[float], k: int = 5) -> List[Dict[str, Any]]:
        ...
        if self.index:
            import numpy as _np
            q_arr = _np.array([q], dtype="float32")
            D, I = self.index.search(q_arr, min(k, len(self.embeddings)))
            results = []
            for idx in I[0]:
            ...
            scored = [(dist(q, v), i) for i, v in enumerate(self.embeddings)]
...
class ModelTierManager:
    ...
    - All heavy models are loaded ONLY when user presses GUI buttons.
    - HF is used only with local_files_only=True or local directories.
    """
    ...
    def __init__(self):
        ...
        # DO NOT auto-load heavy models here.
        self._init_gguf_stub()
    ...
    def _init_gguf_stub(self):
        # Just keep config; actual load is manual via GUI button.
        if GGUF_CONFIG.get("enabled", False):
            LOGGER.info("[LLM-GGUF] GGUF backend available but not loaded (manual load only).")

    def load_gguf_llm(self, model_path: Optional[str] = None) -> str:
        try:
            import llama_cpp
        except Exception as e:
            LOGGER.warning(f"[LLM-GGUF] llama_cpp not available: {e}")
            return "GGUF backend not available (missing llama_cpp)."

        path = model_path or GGUF_CONFIG["model_path"]
        if not os.path.isfile(path):
            msg = f"GGUF model file not found: {path}"
            LOGGER.warning(msg)
            return msg

        LOGGER.info("[LLM-GGUF] Initializing GGUF LLM from local file...")
        ...
        try:
            ...
            self.llm_name = os.path.basename(path)
            LOGGER.info("[LLM-GGUF] GGUF LLM ready.")
            return f"GGUF LLM loaded from {path}"
        except Exception as e:
            LOGGER.warning(f"[LLM-GGUF] Failed to init GGUF LLM: {e}")
            return f"Failed to load GGUF LLM: {e}"

    # ---------- LLM ONNX (MANUAL LOAD) ----------

    def load_llm_onnx(self, onnx_path: Optional[str] = None, tokenizer_id: Optional[str] = None) -> str:
        lazy_onnxruntime()
        lazy_transformers()
        if not onnxruntime or not transformers:
            msg = "ONNXRuntime or transformers not available."
            LOGGER.warning(f"[LLM-ONNX] {msg}")
            return msg

        path = onnx_path or MODEL_PATHS["llm_onnx_path"]
        if not os.path.isfile(path):
            msg = f"LLM ONNX file not found: {path}"
            LOGGER.warning(msg)
            return msg

        model_id = tokenizer_id or MODEL_PATHS["llm_onnx_model_id"]
        LOGGER.info(f"[LLM-ONNX] Initializing ONNX LLM from {path} (tokenizer={model_id})...")
        try:
            # Force local-only tokenizer load
            self.llm_onnx_tokenizer = transformers.AutoTokenizer.from_pretrained(
                model_id,
                local_files_only=True,
            )
        except Exception as e:
            LOGGER.warning(f"[LLM-ONNX] Failed to load tokenizer locally: {e}")
            return f"Failed to load tokenizer locally: {e}"
        ...
        try:
            ...
            if not self.llm_name:
                self.llm_name = f"{os.path.basename(path)}"
            LOGGER.info("[LLM-ONNX] ONNX LLM ready.")
            return f"ONNX LLM loaded from {path}"
        except Exception as e:
            LOGGER.warning(f"[LLM-ONNX] Failed to init ONNX LLM: {e}")
            return f"Failed to init ONNX LLM: {e}"

    # ---------- ViT ONNX (MANUAL LOAD) ----------

    def load_vit_onnx(self, onnx_path: Optional[str] = None) -> str:
        lazy_onnxruntime()
        if not onnxruntime:
            msg = "ONNXRuntime not available."
            LOGGER.warning(f"[ViT-ONNX] {msg}")
            return msg

        path = onnx_path or MODEL_PATHS["vit_onnx_path"]
        if not os.path.isfile(path):
            msg = f"ViT ONNX file not found: {path}"
            LOGGER.warning(msg)
            return msg
        ...
        try:
            ...
            self.vit_name = os.path.basename(path)
            LOGGER.info("[ViT-ONNX] ONNX ViT ready.")
            return f"ONNX ViT loaded from {path}"
        except Exception as e:
            LOGGER.warning(f"[ViT-ONNX] Failed to init ONNX ViT: {e}")
            return f"Failed to init ONNX ViT: {e}"

    # ---------- Whisper ONNX (MANUAL LOAD) ----------

    def load_whisper_onnx(self, onnx_path: Optional[str] = None) -> str:
        lazy_onnxruntime()
        if not onnxruntime:
            msg = "ONNXRuntime not available."
            LOGGER.warning(f"[Whisper-ONNX] {msg}")
            return msg

        path = onnx_path or MODEL_PATHS["whisper_onnx_path"]
        if not os.path.isfile(path):
            msg = f"Whisper ONNX file not found: {path}"
            LOGGER.warning(msg)
            return msg
        ...
        try:
            ...
            self.whisper_name = os.path.basename(path)
            LOGGER.info("[Whisper-ONNX] ONNX Whisper ready.")
            return f"ONNX Whisper loaded from {path}"
        except Exception as e:
            LOGGER.warning(f"[Whisper-ONNX] Failed to init ONNX Whisper: {e}")
            return f"Failed to init ONNX Whisper: {e}"
    ...
    def run_llm(self, text: str) -> Dict[str, Any]:
        ...
        if self.llm_backend == "gguf" and self.gguf_llm:
            ...
            try:
                ...
                LOGGER.warning(f"[LLM] GGUF inference failed: {e}")
        ...
        if self.llm_backend == "onnx" and self.llm_onnx_session and self.llm_onnx_tokenizer:
            ...
            np_mod = lazy_np()
            if not np_mod:
                return {"model": self.llm_name or "LLM-ONNX", "backend": "onnx", "output": text}
            try:
                ...
                LOGGER.warning(f"[LLM-ONNX] Inference failed: {e}")

        LOGGER.info("[LLM] Using stub backend.")
        return {"model": "LLM-stub", "backend": "stub", "output": f"Stub response: {text[:80]}"}
    ...
    def run_vit(self, image_path: str) -> Dict[str, Any]:
        if self.vit_backend == "onnx" and self.vit_onnx_session:
            ...
            try:
                ...
                np_mod = lazy_np()
                if not np_mod:
                    return {"model": self.vit_name, "backend": "onnx", "embedding": []}
                ...
                LOGGER.warning(f"[ViT-ONNX] Inference failed: {e}")
    ...
    def run_whisper(self, audio_path: str) -> Dict[str, Any]:
        if self.whisper_backend == "onnx" and self.whisper_onnx_session:
            ...
            try:
                ...
                LOGGER.warning(f"[Whisper-ONNX] Inference failed: {e}")
...
# =========================
# DISTRIBUTED SWARM NODES (NETWORK STUB)
# =========================

class DistributedSwarmNode:
    """
    Very lightweight UDP-based swarm node stub.
    Not secure, not production-ready — just enough to simulate real networking.
    """
...
class EmotionalGradientEngine:
    """
    Tracks emotional axes over time:
    - arousal (low/high)
    - valence (negative/positive)
    - focus (scattered/locked)
    """
...
class CorticalColumn:
    """
    Tiny cortical column simulation:
    - receives signals
...
# =========================
# ORGANS (WITH NEURAL PLASTICITY + SELF-HEALING)
# =========================
...
# =========================
# HYBRID BRAIN (META-STATES, DREAM MODE, HALLUCINATION)
# =========================
...
# =========================
# ORCHESTRATOR (ASYNC + HYBRID BRAIN + DREAMS)
# =========================
...
class MegaSystemApp(tk.Tk):
    ...
    def _build_nerve_center(self):
        ...
        # ---------- MODEL LOAD BUTTONS ----------
        ttk.Label(control_frame, text="Model Load Controls:", style="Cyber.TLabel").pack(anchor="w", pady=(6, 2))

        llm_gguf_btn = ttk.Button(
            control_frame,
            text="Load LLM (GGUF, local)",
            style="Cyber.TButton",
            command=self.gui_load_llm_gguf,
        )
        llm_gguf_btn.pack(fill=tk.X, pady=1)

        llm_onnx_btn = ttk.Button(
            control_frame,
            text="Load LLM (ONNX, local)",
            style="Cyber.TButton",
            command=self.gui_load_llm_onnx,
        )
        llm_onnx_btn.pack(fill=tk.X, pady=1)

        vit_onnx_btn = ttk.Button(
            control_frame,
            text="Load ViT (ONNX, local)",
            style="Cyber.TButton",
            command=self.gui_load_vit_onnx,
        )
        vit_onnx_btn.pack(fill=tk.X, pady=1)

        whisper_onnx_btn = ttk.Button(
            control_frame,
            text="Load Whisper (ONNX, local)",
            style="Cyber.TButton",
            command=self.gui_load_whisper_onnx,
        )
        whisper_onnx_btn.pack(fill=tk.X, pady=1)
        # ---------------------------------------
    ...
    def update_altered_states_view(self, result: Dict[str, Any]):
        brain_state = result.get("brain_state", {}) or {}
        best_guess = result.get("best_guess", "") or ""
        hallucination = result.get("hallucination", "") or ""
        cortical_state = result.get("cortical_state", {}) or {}
        dreams = (result.get("dreams", {}) or {}).get("dreams", [])
        ...
        organs = brain_state.get("organs", {}) or {}
        heatmap = brain_state.get("heatmap", {}) or {}
        emotional = brain_state.get("emotional", {}) or {}
    ...
    def gui_load_llm_gguf(self):
        path = filedialog.askopenfilename(
            title="Select GGUF LLM model",
            filetypes=[("GGUF files", "*.gguf"), ("All files", "*.*")],
        )
        if not path:
            return
        msg = self.tier_manager.load_gguf_llm(path)
        self._log(f"[GUI] {msg}")

    def gui_load_llm_onnx(self):
        path = filedialog.askopenfilename(
            title="Select LLM ONNX model",
            filetypes=[("ONNX files", "*.onnx"), ("All files", "*.*")],
        )
        if not path:
            return
        # Tokenizer id: user can keep default or change code to point to local dir
        msg = self.tier_manager.load_llm_onnx(path, tokenizer_id=MODEL_PATHS["llm_onnx_model_id"])
        self._log(f"[GUI] {msg}")

    def gui_load_vit_onnx(self):
        path = filedialog.askopenfilename(
            title="Select ViT ONNX model",
            filetypes=[("ONNX files", "*.onnx"), ("All files", "*.*")],
        )
        if not path:
            return
        msg = self.tier_manager.load_vit_onnx(path)
        self._log(f"[GUI] {msg}")

    def gui_load_whisper_onnx(self):
        path = filedialog.askopenfilename(
        ...
        if not path:
            return
        msg = self.tier_manager.load_whisper_onnx(path)
        self._log(f"[GUI] {msg}")
    ...
    def run_full_pipeline_async(self):
        ...
        def schedule():
            try:
                ...
                self._log(f"[ERROR] Failed to schedule pipeline: {e}")
    ...
    def update_swarm_visual(self, result: Dict[str, Any]):
        swarm_agents = result.get("swarm_agents") or {}
