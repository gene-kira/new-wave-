#!/usr/bin/env python3
# ============================================================
# Codex Input Firewall — OBLIVION BUILD (REFINED)
# - Tuned thresholds (less noisy)
# - Learning mode (observe-only, no kills)
...
STATE = {
    ...
    "learning_mode": True,  # NEW: observe-only by default
}
...
# ============================================================
# 12. PROCESS MONITORING & AI-STYLE MODEL
# ============================================================
...
def process_behavior_profile(proc: psutil.Process) -> dict:
    ...
    # NEW: tuned threshold: less noisy, require higher score
    stats["suspicious"] = score >= 20.0
    stats["features"] = features

    BEHAVIOR_DB[pid] = stats

    # NEW: per-process feature log (compact)
    write_log(
        f"[PROFILE] PID {pid} score={score:.2f} "
                ...
                if fw_on:
                    for proc in psutil.process_iter(['pid', 'name', 'exe', 'cmdline']):
                        try:
                            ...
                            # NEW: tuned decision logic
                            should_flag = (
                                bad_sig or
                                (bad_markers and ai_score >= 16.0) or
                                (bad_behavior and ai_score >= 20.0)
                            )
...
class CodexCockpitGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Codex Input Firewall — OBLIVION Cockpit (Refined)")
