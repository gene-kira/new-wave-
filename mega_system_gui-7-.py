...
PLUGIN_DIR = "plugins"  # directory for plugin discovery
...
# =========================
# MODEL TIER MANAGER + PLUGIN DISCOVERY
# =========================
...
class ModelTierManager:
    ...
    Also supports ONNX + GGUF backends and plugin registration.
    """
    ...
    # -------- Whisper RUN (REAL MEL-SPECTROGRAM) --------
    ...
    def run_whisper(self, audio_path: str) -> Dict[str, Any]:
        if self.whisper_backend == "onnx" and self.whisper_onnx_session:
            ...
            transcript = "ONNX-Whisper transcript (mel-based stub decode)"
...
class MegaSystemOrchestrator:
    ...
    async def run_full_pipeline_async(self, text: str, image_path: Optional[str] = None, audio_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        vit_task = None
        if image_path:
            vit_task = loop.run_in_executor(None, self.models.vision_transformer, image_path)

        whisper_task = None
        if audio_path:
            whisper_task = loop.run_in_executor(None, self.models.speech_recognition, audio_path)
...
# =========================
# TKINTER GUI (CYBERPUNK + REAL-TIME SWARM + ASYNC)
# =========================
