#!/usr/bin/env python
# game_guardian_ultra_ai_cluster_mic_supervisor.py
#
# Game Guardian ULTRA AI CLUSTER + MIC SUPERVISOR:
...
#   * USB port watchdog (stubbed auto-reset hook)
#   * Exclusive-mode killer (kills apps that may lock mic)
#   * Discord/Steam voice auto-reconnect (process restart)
#   * Audio device priority enforcement (preferred input device)
#
...
DEFAULT_CONFIG = {
    ...
    "ti_peers": [],
    # Mic subsystem
    "mic_preferred_device_name": "",  # partial name match
...
class MicSelfHealer:
    ...
    def usb_watchdog_reset(self):
        if not config.get("mic_usb_watchdog_enabled", True):
            return
        # Stub: here you could call devcon or PowerShell to reset USB root hub
        # For now we just log the intent.
        print("USB watchdog: would reset USB audio root hub here")
...
class GameGuardianGUI:
    def __init__(self):
        ...
        self.mic_healer = MicSelfHealer(gui_callback=self.update_mic_status_text)
