...
for p in [STORAGE_ROOT, LOG_ROOT, BACKUP_ROOT, REPLICA_ROOT, STATE_ROOT]:
...
ENABLE_ELEVATION = False  # flip to True if you really want admin prompt
...
# ============================================================
# SYNTHETIC DEVICES (CPU / DISK / NET / GPU / SENSOR)
# ============================================================
...
class SyntheticGPU:
    def __init__(self, name, vram_gb=8):
        self.name = name
...
# ============================================================
# NETWORK LAYER (TCP HUB + NODE CLIENTS)
# ============================================================

class NetworkHub:
    def __init__(self, host, port, bus):
        self.host = host
        self.port = port
        self.bus = bus
        self.clients = []
    ...
    def _server_loop(self):
        ...
        s.listen(10)
        self.bus.publish("system.net", {
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "status": f"hub_listening {self.host}:{self.port}"
        })
        while True:
            conn, addr = s.accept()
            self.bus.publish("system.net", {
                "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "status": f"node_connected {addr}"
            })
            with self.lock:
            ...
            while True:
                ...
                while b"\n" in buf:
                    ...
                    try:
                        ...
                        topic = msg.get("topic", "node.event")
...
class WorkerBase(threading.Thread):
    def __init__(self, name, bus, node_id, devices, queen_ref=None, net_client=None):
    ...
    def heartbeat(self, extra=None):
        ...
        if self.net_client:
            self.net_client.send("worker.status", msg)
...
class GPUWorker(WorkerBase):
    def run(self):
        ...
        while self.running:
            gpu.load = round(random.uniform(0, 100), 2)
            gpu.temp_c = round(40 + gpu.load * 0.4 + random.uniform(-2, 2), 1)
            self.heartbeat({"gpu_load": gpu.load, "gpu_temp": gpu.temp_c})
            time.sleep(2)

class PurgeWorker(WorkerBase):
    def __init__(self, name, bus, node_id, devices, purge_shell, queen_ref=None, net_client=None):
        super().__init__(name, bus, node_id, devices, queen_ref, net_client)
...
class LogWorker(threading.Thread):
    def __init__(self, bus):
        ...
        topics = [
            ...
            "system.api", "worker.restart", "system.net", "config.update"
        ]
        for t in topics:
...
# ============================================================
# AI-STYLE ANOMALY DETECTION (Z-SCORE)
# ============================================================

class AnomalyWorker(threading.Thread):
    def __init__(self, bus, window_size, z_threshold):
        super().__init__(daemon=True)
        self.bus = bus
    ...
    def on_status(self, msg):
        metrics = {}
        for k in ("cpu_percent", "disk_percent", "tx", "rx", "mem_percent", "gpu_load", "gpu_temp"):
            if k in msg and isinstance(msg[k], (int, float)):
                metrics[k] = float(msg[k])
        if metrics:
    ...
    def run(self):
        while True:
            if len(self.window) >= 5:
                ...
                keys = latest.keys()
                for k in keys:
                    vals = [m.get(k, 0.0) for m in self.window]
...
# ============================================================
# STATE SNAPSHOTTER (PERSISTENCE)
# ============================================================

class StateSnapshotter(threading.Thread):
    def __init__(self, bus, interval_sec):
        super().__init__(daemon=True)
        self.bus = bus
    ...
    def on_status(self, msg):
        node = msg["node"]
        worker = msg["worker"]
        self.latest_state.setdefault(node, {})
        self.latest_state[node][worker] = msg

    def run(self):
        while True:
            try:
                path = os.path.join(STATE_ROOT, "snapshot.json")
                with open(path, "w", encoding="utf-8") as f:
                    json.dump(self.latest_state, f, indent=2)
            except:
                traceback.print_exc()
            time.sleep(self.interval)
...
class SwarmNode:
    def __init__(self, node_id, bus, purge_shell, queen_ref=None, net_host=None, net_port=None):
        self.node_id = node_id
        self.bus = bus
        self.purge_shell = purge_shell
        self.queen_ref = queen_ref
        self.net_client = NodeClient(node_id, net_host, net_port) if net_host and net_port else None
        self.devices = {
            ...
            "gpu": SyntheticGPU(f"GPU-{node_id}", vram_gb=8),
        }
        self.workers = []
    ...
    def start(self):
        self.workers = [
            ComputeWorker("ComputeWorker", self.bus, self.node_id, self.devices, self.queen_ref, self.net_client),
            TelemetryWorker("TelemetryWorker", self.bus, self.node_id, self.devices, self.queen_ref, self.net_client),
            StorageWorker("StorageWorker", self.bus, self.node_id, self.devices, self.queen_ref, self.net_client),
            NetWorker("NetWorker", self.bus, self.node_id, self.devices, self.queen_ref, self.net_client),
            GPUWorker("GPUWorker", self.bus, self.node_id, self.devices, self.queen_ref, self.net_client),
            PurgeWorker("PurgeWorker", self.bus, self.node_id, self.devices, self.purge_shell, self.queen_ref, self.net_client),
        ]
        self.pod_worker = PodWorker("PodWorker", self.bus, self.node_id, self.devices, self.queen_ref, self.net_client)
        self.workers.append(self.pod_worker)
        for w in self.workers:
            w.start()
        for svc in ("compute", "storage", "network", "telemetry", "pods", "gpu"):
...
class Queen:
    def __init__(self, bus, node_count=3, net_host=None, net_port=None):
        self.bus = bus
    ...
    def start(self):
        ...
        for i in range(self.node_count):
            node = SwarmNode(f"Node-{i+1}", self.bus, self.purge_shell, queen_ref=self,
                             net_host=self.net_host, net_port=self.net_port)
                    ...
                    for w in list(node.workers):
                        ...
                        if now - last > 20:
                            ...
                            if isinstance(w, PurgeWorker):
                                new_worker = PurgeWorker(w.name, self.bus, node.node_id, node.devices,
                                                         node.purge_shell, self, node.net_client)
                            elif isinstance(w, PodWorker):
                                new_worker = PodWorker(w.name, self.bus, node.node_id, node.devices,
                                                       self, node.net_client)
                                node.pod_worker = new_worker
                            else:
                                new_worker = type(w)(w.name, self.bus, node.node_id, node.devices,
                                                     self, node.net_client)
    ...
    def handle_status(self):
        ...
        for node in q.nodes:
            ninfo = {
                ...
                "gpu_load": node.devices["gpu"].load,
                "gpu_temp": node.devices["gpu"].temp_c,
            }
            nodes.append(ninfo)
...
class BorgGUI:
    def __init__(self, bus, queen, node_count):
        self.bus = bus
        self.queen = queen
        self.node_count = node_count
        self.node_state = {}
        ...
        self.root = tk.Tk()
        self.root.title("Borg Datacenter — Monolith (Swarm + Network + AI + GPU + Persistence)")
        ...
        self.anomaly_frame = tk.Frame(self.notebook, bg="black")
        ...
        self.notebook.add(self.anomaly_frame, text="Anomalies")

        self.canvas = tk.Canvas(self.overview_frame, bg="black")
        self.canvas.pack(fill="both", expand=True)

        self.workers_text = tk.Text(self.workers_frame, bg="black", fg="cyan", font=("Consolas", 9))
        self.workers_text.pack(fill="both", expand=True)

        self.devices_text = tk.Text(self.devices_frame, bg="black", fg="yellow", font=("Consolas", 9))
        ...
        self.purge_text = tk.Text(self.purge_frame, bg="black", fg="magenta", font=("Consolas", 9))
        ...
        self.anomaly_text = tk.Text(self.anomaly_frame, bg="black", fg="magenta", font=("Consolas", 9))
        ...
        bus.subscribe("system.net", self.on_net_status)
    ...
    def _log_anomaly(self, line):
        self.anomaly_text.insert("end", line + "\n")
        self.anomaly_text.see("end")
    ...
    def on_worker_status(self, msg):
        node = msg["node"]
        self.node_state.setdefault(node, {})
        self.node_state[node].update(msg)
        line = f"[{msg['timestamp']}] {node}::{msg['worker']} — {json.dumps(msg)}"
        self._log_workers(line)
        self._log_event(line)
        if any(k in msg for k in ("load", "storage_used", "tx", "rx", "cpu_percent", "disk_percent", "mem_percent", "gpu_load", "gpu_temp")):
    ...
    def on_anomaly(self, msg):
        line = f"[{msg['timestamp']}] ANOMALY — z={msg['z_scores']} metrics={msg['metrics']}"
        self._log_anomaly(line)
        self._log_event(line)
    ...
    def on_net_status(self, msg):
        line = f"[{msg['timestamp']}] NET — {msg['status']}"
        self._log_event(line)
    ...
    def redraw(self):
        ...
        margin = 40
        node_w = (w - 2 * margin) / max(1, self.node_count)
        node_h = h - 2 * margin

        nodes_sorted = sorted(self.node_state.keys())
        if not nodes_sorted:
            self.canvas.create_text(w/2, h/2, text="Waiting for nodes...", fill="gray", font=("Consolas", 16))
        else:
            for idx, node in enumerate(nodes_sorted):
                x0 = margin + idx * node_w
                y0 = margin
                x1 = x0 + node_w - 10
                y1 = y0 + node_h

                self.canvas.create_rectangle(x0, y0, x1, y1, outline="white", width=2)
                self.canvas.create_text((x0+x1)/2, y0+15, text=node, fill="white", font=("Consolas", 12, "bold"))

                state = self.node_state.get(node, {})

                cpu = float(state.get("cpu_percent", 0.0))
                self._draw_bar(x0+20, y0+40, x1-20, y0+80, cpu, 100, "CPU %", "lime")

                disk_used = float(state.get("storage_used", 0.0) or state.get("disk_used_gb", 0.0))
                disk_cap = 128.0
                self._draw_bar(x0+20, y0+90, x1-20, y0+130, disk_used, disk_cap, "Disk GB", "yellow")

                tx = float(state.get("tx", 0.0))
                rx = float(state.get("rx", 0.0))
                net = tx + rx
                self._draw_bar(x0+20, y0+140, x1-20, y0+180, net, 100, "Net Mbps", "cyan")

                gpu_load = float(state.get("gpu_load", 0.0))
                self._draw_bar(x0+20, y0+190, x1-20, y0+230, gpu_load, 100, "GPU Load", "orange")

                gpu_temp = float(state.get("gpu_temp", 40.0))
                self._draw_bar(x0+20, y0+240, x1-20, y0+280, gpu_temp, 100, "GPU Temp", "red")

        self.root.after(500, self.redraw)

    def _draw_bar(self, x0, y0, x1, y1, value, max_value, label, color):
        self.canvas.create_rectangle(x0, y0, x1, y1, outline="gray", width=1)
        if max_value <= 0:
            return
        ratio = max(0.0, min(1.0, value / max_value))
        fill_w = x0 + ratio * (x1 - x0)
        self.canvas.create_rectangle(x0, y0, fill_w, y1, fill=color, width=0)
        text = f"{label}: {value:.1f}"
        self.canvas.create_text((x0+x1)/2, (y0+y1)/2, text=text, fill="black", font=("Consolas", 9, "bold"))
...
if __name__ == "__main__":
    try:
        ...
        # Network hub
        hub = NetworkHub(CONFIG["network_host"], CONFIG["network_port"], bus)
        hub.start()

        queen = Queen(bus, node_count=CONFIG["node_count"],
                      net_host=CONFIG["network_host"], net_port=CONFIG["network_port"])
        queen.start()
        ...
        anomaly_worker = AnomalyWorker(bus, window_size=CONFIG["anomaly_window"],
        ...
        snapshotter = StateSnapshotter(bus, CONFIG["snapshot_interval_sec"])
        snapshotter.start()

        start_api(bus, queen, scheduler_worker, host=CONFIG["api_host"], port=CONFIG["api_port"])

        gui = BorgGUI(bus, queen, node_count=CONFIG["node_count"])
        gui.run()
