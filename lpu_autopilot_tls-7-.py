...
# -*- coding: utf-8 -*-
"""
lpu_autopilot_kernel_ultimate.py
...
- Multi-node cluster mode (swarm peers + event bus)
"""
...
# =============================================================================
# JSON-safe helper (fixes VehicleMode serialization everywhere)
# =============================================================================
...
def validate_autopilot_message(
    ...
    if msg_type == "NavigationCommand":
        ...
        if current_mode == VehicleMode.MANUAL and mode == VehicleMode.AUTONOMOUS:
            return False, "Cannot issue autonomous navigation while in MANUAL mode"

        if current_mode == VehicleMode.EMERGENCY_STOP and priority < 9:
            return False, "Non-emergency navigation blocked during EMERGENCY_STOP"
    ...
    if msg_type == "SystemQuery":
        ...
        if not qtype:
            return False, "SystemQuery requires 'query_type'"
        return True, "ok"

    if msg_type == "SystemEvent":
        etype = payload.get("event_type", "")
        if not etype:
            return False, "SystemEvent requires 'event_type'"
        return True, "ok"

    return False, f"Unknown message type: {msg_type}"

# =============================================================================
# Plugin system with sandbox + hot-reload
# =============================================================================
...
class MQTTBridge:
    ...
    async def _handle_command(self, text: str):
        ...
        out = json.dumps({"ok": True, "result": result}, default=json_safe)
        try:
            self.client.publish(MQTT_TOPIC_OUT, out)
        except Exception as e:
