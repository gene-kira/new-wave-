...
"""
BORG OS v23 – FULL STACKING ORGANISM
...
class MemoryStack:
    """
    7) Memory Stacking:
    - short_term: fast, volatile
    - mid_term: medium horizon
    - long_term: slow, stable
    - threat_mem: threat-specific
    - behavior_mem: behavior-specific
    """
...
class ThreatScoreStack:
    ...
    - Mesh score
    """
    def __init__(self, reputation: ProcessReputation, mesh_stack: MeshStack, logger=None, accelerator=None):
...
class BorgOSGovernor:
    def __init__(self, logger=None, storage=None, event_bus=None,
        ...
        self.mesh_stack = mesh_stack or MeshStack(storage=storage, logger=self.logger, event_bus=self.event_bus)
        ...
        self.threat_stack = ThreatScoreStack(self.reputation, self.mesh_stack, logger=self.logger, accelerator=self.accelerator)
        self.persona_stack = PersonaStack(logger=self.logger, event_bus=self.event_bus)
                ...
                for i, state in enumerate(states):
                    ...
                    for p in procs[start:end]:
                        try:
                            ...
                            dpi_flags = False  # DPI stack already logs anomalies globally
...
rl_stack = DeepRLStack(logger=lambda msg: print(msg), event_bus=event_bus, accelerator=accelerator)
kernel_stack = KernelStack(logger=lambda msg: print(msg), event_bus=event_bus)
mesh_stack = MeshStack(storage=storage, logger=lambda msg: print(msg), event_bus=event_bus)
...
if FASTAPI_AVAILABLE:
    app = FastAPI(title="Borg OS Node", version="23.0.0")
...
class BorgGUI:
    def __init__(self, root, core: BorgCore):
        ...
        root.title("BORG OS NODE v23 – Full Stacking Organism (Security + Autonomous AI + Games + Safe Window)")
...
def run_cli():
    print("BORG OS NODE v23 (CLI MODE, Full Stacking Organism – Security + Deep RL + Mesh + DPI + Games + Safe Window)")
    print(f"OS: {OS_NAME}")
