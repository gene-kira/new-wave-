...
# -*- coding: utf-8 -*-
"""
AI Security Bridge (GUI-first + Auto-elevation + Auto WAN/LAN detect + Auto-start bridge + Crash-proof subsystems)
...
def detect_interfaces():
    ...
    try:
        ...
        # Heuristic:
        # If we have at least one public and one private -> WAN mode
        # Else -> LAN mode
        if public_ifaces and private_ifaces:
            BRIDGE_MODE = "WAN"
            WAN_IN_IFACE = public_ifaces[0][0]
            WAN_OUT_IFACE = private_ifaces[0][0]
            # For completeness, LAN_IN/LAN_OUT can mirror WAN_OUT or other private
            ...
            if len(private_ifaces) >= 2:
                ...
                # Force mode fallback: pick any up interface
        ...
        # Force mode: if anything is still None, pick first available
        stats_all = psutil.net_if_stats()
        ...
        # Force mode fallback: generic names
