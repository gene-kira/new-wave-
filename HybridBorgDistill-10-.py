#!/usr/bin/env python3
# BORG ALTERED STATES v3 — PERFORMANCE GOVERNOR + MODES + PHYSICS + LSTM + RL + AUTO-TUNING
# Server + PC gaming, real FPS ingestion (file-based), reinforcement learning, auto-tuning, process throttling, AI-driven optimization.
...
def mode_personality(mode):
    if mode == "Flow":
        return {"lr_factor": 1.2, "mutation_rate": 0.03, "physics_sensitivity": 1.2, "control_aggressiveness": 0.8}
    if mode == "DeepWork":
...
# ---------------------------------------------------------
# RL POLICY (AUTO-TUNING + PROCESS THROTTLING)
# ---------------------------------------------------------
def init_rl_policy():
    global RL_POLICY
...
def rl_apply_action(action_idx, ext, mode):
    actions_taken = []
    # 0: no-op, 1: throttle background server processes, 2: throttle non-game processes more aggressively
    if action_idx == 1:
...
def rl_compute_reward(prev_perf, new_perf, anomaly_score):
    # reward: improvement in perf, penalize anomalies
    ...
    while not STOP_FLAG:
        ...
        try:
            ...
            with CONTROL_LOCK:
                ...
                dump(report, "borg_altered_states_rl_report.joblib")
...
def start_threads():
    ...
    return [t_stream, t_teacher, *swarm_threads, t_snapshot, t_port, t_web, t_gov, t_anom, t_lstm, t_rl]

def reset_state():
    global DATA_QUEUE, STOP_FLAG, TEACHER, SHADOW_TEACHER, SWARM_STUDENTS, IN_FEATURES, STATE_HISTORY
    global ANOMALY_MODEL, LSTM_MODEL, RL_POLICY
    with CONTROL_LOCK:
        DATA_QUEUE = queue.Queue(maxsize=4000)
        STOP_FLAG = False
        TEACHER = None
...
def watchdog_loop():
    print("=== BORG ALTERED STATES + LSTM + RL — WATCHDOG MODE ===")
    while True:
        try:
