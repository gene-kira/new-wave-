...
def select_best_proxy():
    with config_lock:
        ...
        if not proxies:
            return None
        # choose lowest latency_avg, fallback to first
        best = None
...
def apply_best_proxy():
    ...
    with config_lock:
        config["proxy"] = best
        config["proxy_index"] = max(0, config.get("proxies", []).index(best))
        save_config(config)
...
def heartbeat_wrapper(name, target):
    def run():
        while True:
            try:
                ...
                logging.error(f"{name} wrapper error: {e}")
                time.sleep(10)
...
def on_stealth_toggle():
    ...
    enable_proxy_threaded()  # rebuild PAC in new location
    refresh()
...
heartbeat_wrapper("auto_rotate", auto_rotate_thread)
heartbeat_wrapper("borg_ai", borg_ai_thread)
heartbeat_wrapper("registry", registry_monitor_thread)
heartbeat_wrapper("logs", auto_refresh_logs_thread)
heartbeat_wrapper("cloud", cloud_sync_thread)
