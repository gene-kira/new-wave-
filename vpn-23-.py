...
EXTREME MODE:
- Treat Opera VPN as a REAL VPN (full trust, full routing, full saving)
- Always-on: DNS + Proxy + VPN detection + Swarm + REST + GUI
- Browser VPN (Opera) integrated into Borg memory, ghost tunnel, and GUI
"""
...
# Opera VPN IP ranges (observed + generic prefixes)
OPERA_VPN_IP_PREFIXES = [
    "77.111.246.",
    "77.111.247.",
    "77.111.244.",
    "185.26.182.",
...
def _is_opera_vpn_ip(ip):
    if not ip:
        return False
    for prefix in OPERA_VPN_IP_PREFIXES:
    ...
    while True:
        ...
        with PUBLIC_IP_LOCK:
            ...
            if sig["quic_vpn_activity"]:
                confidence += 0.6
            if sig["opera_vpn_activity"]:
                confidence += 0.6

            confidence = min(1.0, confidence)
            PUBLIC_IP_STATE["confidence"] = confidence

            # Treat Opera VPN as a REAL VPN: if opera_vpn_activity is on, force browser_vpn_active
            PUBLIC_IP_STATE["browser_vpn_active"] = confidence >= 0.5 or sig["opera_vpn_activity"]
...
# =========================
# Process-socket mapping + QUIC detection + Opera VPN integration
# =========================
...
def _lookup_ipinfo_for_proc(ip):
    if not ip:
        return None, None
    if _is_opera_vpn_ip(ip):
        return "OPERA", "Opera VPN"
    info = _fetch_ipinfo(ip)
    return info.get("asn"), info.get("isp")

def _is_vpn_asn(asn, isp):
    if not asn and not isp:
        return False
    if asn:
        for prefix in KNOWN_VPN_ASN_PREFIXES:
    ...
    while True:
        try:
            ...
            for line in out.splitlines():
                ...
                if remote.startswith("0.0.0.0") or remote.startswith("127.0.0.1") or remote.startswith("[::"):
                    continue
                ...
                asn, isp = _lookup_ipinfo_for_proc(ip)
                ...
                vpn_like = _is_vpn_asn(asn, isp) or any(h in (isp or "").lower() for h in KNOWN_VPN_HOST_HINTS) or _is_opera_vpn_ip(ip)
            ...
            with PUBLIC_IP_LOCK:
                ...
                # Opera VPN: if any hit is Opera VPN IP, mark opera_vpn_activity and force browser_vpn_active
                opera_hit = any(_is_opera_vpn_ip(h["ip"]) for h in browser_hits)
                if opera_hit:
                    PUBLIC_IP_STATE["signals"]["opera_vpn_activity"] = True
                    PUBLIC_IP_STATE["browser_vpn_active"] = True
                    PUBLIC_IP_STATE["confidence"] = max(PUBLIC_IP_STATE["confidence"], 1.0)
...
class VPNStatusProvider:
    ...
    def get_current_vpn_server_name(self):
        ...
        st = get_public_ip_state()
        if st["signals"].get("opera_vpn_activity", False):
            return "Opera VPN"
        with GHOST_TUNNEL_LOCK:
    ...
    def get_current_exit_ip(self):
        ...
        st = get_public_ip_state()
        if st["signals"].get("opera_vpn_activity", False) and st.get("current_ip"):
            return st["current_ip"]
        with GHOST_TUNNEL_LOCK:
            if GHOST_TUNNEL_STATE["active"]:
    ...
    def get_exit_ip_for_server(self, server_name: str):
        ...
        if self.last_adapter_name == server_name:
            return self.last_exit_ip
        st = get_public_ip_state()
        if server_name == "Opera VPN" and st.get("current_ip"):
            return st["current_ip"]
        with GHOST_TUNNEL_LOCK:
...
class ProxyHandler(socketserver.StreamRequestHandler):
    ...
    def learn_domain(self, domain):
        ...
        realtime_browser_vpn = (
            ...
            browser_state["signals"].get("quic_vpn_activity", False) or
            browser_state["signals"].get("opera_vpn_activity", False)
        )
        ...
        if any_vpn:
            if system_vpn:
                ...
                if browser_state["signals"].get("opera_vpn_activity", False):
                    server_name = "Opera VPN"
                else:
                    server_name = "browser-vpn"
                exit_ip = browser_state.get("current_ip") or "0.0.0.0"

            SITE_STORE.update_site(domain, server_name, exit_ip)
            BORG_MEMORY.update_domain(domain, server_name, exit_ip, threat_delta=0.0, trust_delta=0.05)
            BORG_MEMORY.update_server(server_name, exit_ip, reliability_delta=0.05)
            FIREWALL.ensure_block_rule_for_ip(exit_ip)

    def _apply_fingerprint_headers(self, domain, headers_list):
        fp = BORG_MEMORY.get_fingerprint(domain) if domain else None
        if not fp and domain:
...
class BorgGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Borg VPN Overmind - GPU Threat + Swarm Telemetry + LLM Anomaly")
    ...
    def _build_layout(self):
        ...
        columns = ("domain", "server", "pref_server", "ip", "visits", "trust", "threat", "anomaly", "decision", "override")
    ...
    def _update_vpn_status(self):
        ...
        if browser_active:
            label_text = f"Browser VPN: ACTIVE (confidence={confidence:.2f})"
            if browser_state["signals"].get("opera_vpn_activity", False):
                label_text = f"Opera VPN: ACTIVE (confidence={confidence:.2f})"
    ...
    def _update_dns_status(self):
        ...
        if ok and port:
            self.dns_status_label.config(
                text=f"DNS: OK (127.0.0.1:{port})",
                foreground="green"
            )
        else:
            self.dns_status_label.config(
                text=f"DNS: ERROR ({err or 'unknown'})",
                foreground="red"
            )

        if fwd_ok:
            self.system_dns_status_label.config(
                text=f"DNS Forwarder: OK (127.0.0.1:{DNS_FORWARDER_ADDR[1]})",
                foreground="green"
            )
        else:
            self.system_dns_status_label.config(
                text=f"DNS Forwarder: ERROR ({fwd_err or 'unknown'})",
                foreground="red"
            )
    ...
    def _update_system_dns_status(self):
        ...
        dns_set = st.get("dns_set", False)
        dns_value = st.get("dns_value") or "-"
        err = st.get("last_error")

        if dns_set:
            txt = f"System DNS: {adapter} → {dns_value}"
            color = "green"
        else:
            txt = f"System DNS: {adapter} (current={dns_value})"
            color = "orange"
            if err:
                txt += f" ERR={err}"
        self.system_dns_status_label.config(text=txt, foreground=color)
...
def main():
    log("=== Borg VPN Overmind starting (GPU Threat + Swarm Telemetry + LLM Anomaly + RT Browser VPN Saving + Ghost Tunnel + Opera VPN Real Mode) ===")
