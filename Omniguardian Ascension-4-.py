...
# ============================================================
# Codex Sentinel Forklift Node v11 – Neural Swarm Guardian
#
# Upgrades:
# - OS-style event hooks (process/connection change detection)
# - Tkinter GUI dashboard for live telemetry visualization
# - ML-like anomaly scoring with learned baselines
# - Persistent learning (historical thresholds refinement)
# - Swarm sync (distributed threat intelligence sharing)
#
# Modes:
# - LLM Engine (RPC + CLI)
# - System Guardian (process + network + disk + memory)
# - Real-time reactive + predictive + explanatory
...
# ============================================================
# RPC server with auto-port fallback + swarm sync
# ============================================================
...
# ============================================================
# Tkinter GUI dashboard (live telemetry)
# ============================================================
...
def _init_gui():
    global GUI_ROOT, GUI_LABELS
    try:
        import tkinter as tk
        from tkinter import ttk

        GUI_ROOT = tk.Tk()
        GUI_ROOT.title("Neural Swarm Guardian Dashboard")
        ...
        labels["cpu"] = add_section("CPU Load", 0)
        labels["gpu"] = add_section("GPU Load", 1)
        labels["game_crash_risk"] = add_section("Game Crash Risk", 2)
        labels["rat_risk"] = add_section("RAT Risk", 3)
        labels["telemetry_risk"] = add_section("Telemetry Spike Risk", 4)
        labels["perf_risk"] = add_section("Performance Risk", 5)
        labels["proc_events"] = add_section("Process Events", 6)
        labels["conn_events"] = add_section("Connection Events", 7)
        labels["explanations"] = add_section("Explanations (top)", 8)
        ...
        def _update_gui():
            try:
                ...
                GUI_LABELS["game_crash_risk"].config(text=am.get("game_crash_risk", "low"))
                GUI_LABELS["rat_risk"].config(text=am.get("rat_risk", "low"))
                GUI_LABELS["telemetry_risk"].config(text=am.get("telemetry_spike_risk", "low"))
                GUI_LABELS["perf_risk"].config(text=am.get("performance_risk", "low"))
