# Project: biodesign-nonlinear_coupling-spiral_tower-mobility
# Name: Gaea-harbor-x-f55d60
# Constraints: zero_emissions, modular, scalable, self_healing, fail_safe, low_mass, silent
# Params: scale=942.2395, power_budget=39.1917, tol=0.003, mutation_rate=0.8
# Scores: final=0.7629 novelty=0.7477 utility=0.6554 impact=1.0000 curiosity=0.5517
...
class Config:
    def __init__(self):
        self.name = 'Gaea-harbor-x-f55d60'
        self.title = 'biodesign-nonlinear_coupling-spiral_tower-mobility'
        self.seed = 42
        self.iterations = 200
...
def main():
    ...
    p = Parameters(scale=942.2395, power_budget=39.1917, tolerance=0.003, mutation_rate=0.8)
    errs = validate_params(p)
    if errs:
        logging.error('Validation errors: %s', errs); sys.exit(1)
    core = ProjectCore(p, 'biodesign-nonlinear_coupling-spiral_tower-mobility', ['zero_emissions', 'modular', 'scalable', 'self_healing', 'fail_safe', 'low_mass', 'silent'], 'mobility')
