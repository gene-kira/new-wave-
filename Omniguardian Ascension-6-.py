...
# ============================================================
# Codex Sentinel Forklift Node v13 – Neural Swarm Overmind++
#
...
# - Real ML anomaly classifier (PyTorch-based model for process anomalies)
# - GUI evolution (graphs, risk heatmap, event timeline)
...
class UniversalOSLoader:
    ...
    def _normalize_env(self):
        ...
        learn_dir = self.base_path / "learning"
        learn_dir.mkdir(exist_ok=True)
        self.env["LEARN_DIR"] = str(learn_dir)

        swarm_dir = self.base_path / "swarm"
        swarm_dir.mkdir(exist_ok=True)
        self.env["SWARM_DIR"] = str(swarm_dir)

        replay_dir = self.base_path / "replay"
        replay_dir.mkdir(exist_ok=True)
        self.env["REPLAY_DIR"] = str(replay_dir)

        trust_dir = self.base_path / "trust"
        trust_dir.mkdir(exist_ok=True)
        self.env["TRUST_DIR"] = str(trust_dir)
...
LEARN_FILE = os.path.join(OSLOADER.env["LEARN_DIR"], "guardian_learning.json")
...
# ============================================================
# System telemetry + adaptive behavior
# ============================================================
...
# ============================================================
# System Guardian + ML-like anomaly scoring + OS-style events
# + Temporal intelligence + Threat classification + Swarm consensus
# + Swarm trust scoring + Predictive crash modeling + Real ML classifier
# ============================================================
...
# ============================================================
# Real ML anomaly classifier (PyTorch)
# ============================================================

class AnomalyClassifier(nn.Module):
    def __init__(self, input_dim: int = 5, hidden_dim: int = 32, num_classes: int = 4):
        super().__init__()
...
ANOMALY_MODEL = AnomalyClassifier().to(DEFAULT_DEVICE)
...
def _process_features(proc_record: Dict) -> torch.Tensor:
    ...
    return torch.tensor([[rss, threads, read_bytes, write_bytes, cpu_pct]], dtype=torch.float32, device=DEFAULT_DEVICE)
...
def process_module_scan():
    ...
    SYSTEM_GUARDIAN_STATS["context_module"]["process_roles"] = context_roles
    SYSTEM_GUARDIAN_STATS["events"]["process_started"] = events_started
    SYSTEM_GUARDIAN_STATS["events"]["process_stopped"] = events_stopped
    SYSTEM_GUARDIAN_STATS["classification"]["process_labels"] = process_labels
...
def network_module_scan():
    ...
    SYSTEM_GUARDIAN_STATS["events"]["connection_opened"] = events_opened
    SYSTEM_GUARDIAN_STATS["events"]["connection_closed"] = events_closed
    SYSTEM_GUARDIAN_STATS["classification"]["connection_labels"] = connection_labels
...
def _predict_performance_risk():
    ...
    SYSTEM_GUARDIAN_STATS.setdefault("anomaly_module", {})
    SYSTEM_GUARDIAN_STATS["anomaly_module"]["performance_risk"] = risk

def _update_temporal_history():
    tel = get_system_telemetry()
    am = SYSTEM_GUARDIAN_STATS.get("anomaly_module", {})
    tmod = SYSTEM_GUARDIAN_STATS["temporal"]
...
def _predict_crash_probability():
    hist = list(SYSTEM_GUARDIAN_STATS["temporal"]["crash_risk_history"])
    if not hist:
        SYSTEM_GUARDIAN_STATS["predictive"]["crash_probability"] = 0.0
        return

    weights = {"low": 0.1, "medium": 0.5, "high": 0.9}
    recent = hist[-32:]
    score = sum(weights.get(entry["value"], 0.1) for entry in recent) / len(recent)
    SYSTEM_GUARDIAN_STATS["predictive"]["crash_probability"] = float(score)
...
def _record_replay_snapshot():
    snapshot = {
        ...
        "events": SYSTEM_GUARDIAN_STATS.get("events", {}),
        "predictive": SYSTEM_GUARDIAN_STATS.get("predictive", {}),
    }
...
def _explain_guardian_state():
    ...
    nm = SYSTEM_GUARDIAN_STATS.get("network_module", {})
    bm = SYSTEM_GUARDIAN_STATS.get("bad_site_module", {})
    ctx = SYSTEM_GUARDIAN_STATS.get("context_module", {})
    ev = SYSTEM_GUARDIAN_STATS.get("events", {})
    swarm_global = SWARM_INTEL["global_risk"]
    crash_prob = SYSTEM_GUARDIAN_STATS["predictive"]["crash_probability"]
...
def _build_export_snapshot():
    SYSTEM_GUARDIAN_STATS["export_snapshot"] = {
        ...
        "context_module": SYSTEM_GUARDIAN_STATS.get("context_module", {}),
        "events": SYSTEM_GUARDIAN_STATS.get("events", {}),
        "explanations": SYSTEM_GUARDIAN_STATS.get("explanations", []),
        "temporal": SYSTEM_GUARDIAN_STATS.get("temporal", {}),
        "classification": SYSTEM_GUARDIAN_STATS.get("classification", {}),
...
# ============================================================
# RPC server with auto-port fallback + swarm sync + replay access
# ============================================================
            ...
            while b"\n" in buf:
                ...
                try:
                    ...
                    if mode == "guardian_snapshot":
                        resp = {"snapshot": SYSTEM_GUARDIAN_STATS["export_snapshot"]}
...
# ============================================================
# Tkinter GUI dashboard (graphs, risk heatmap, event timeline)
# ============================================================
...
def _init_gui():
    ...
    try:
        ...
        GUI_ROOT = tk.Tk()
        GUI_ROOT.title("Neural Swarm Overmind++ Dashboard")
        ...
        labels["cpu"] = add_label("CPU Load", 0)
        labels["gpu"] = add_label("GPU Load", 1)
        labels["game_crash_risk"] = add_label("Game Crash Risk (local/swarm)", 2)
        labels["rat_risk"] = add_label("RAT Risk (local/swarm)", 3)
        labels["telemetry_risk"] = add_label("Telemetry Risk (local/swarm)", 4)
        labels["perf_risk"] = add_label("Performance Risk (local/swarm)", 5)
        labels["proc_events"] = add_label("Process Events (+/-)", 6)
        labels["conn_events"] = add_label("Connection Events (+/-)", 7)
        labels["explanations"] = add_label("Explanations (top)", 8)
        labels["crash_prob"] = add_label("Predictive Crash Probability", 9)

        canvases["cpu_graph"] = add_canvas("CPU History", 10)
        canvases["perf_heatmap"] = add_canvas("Performance Risk Heatmap", 11)
        canvases["event_timeline"] = add_canvas("Event Timeline", 12)
        ...
        def _draw_cpu_history():
            ...
            c.delete("all")
            hist = list(SYSTEM_GUARDIAN_STATS["temporal"]["cpu_history"])
            if not hist:
                return
            w = int(c["width"])
            h = int(c["height"])
        ...
        def _draw_perf_heatmap():
            ...
            c.delete("all")
            hist = list(SYSTEM_GUARDIAN_STATS["temporal"]["perf_risk_history"])
            if not hist:
                return
            w = int(c["width"])
            h = int(c["height"])
...
def main():
    ...
    if not args.headless_node:
        _init_gui()
        if GUI_ROOT is not None:
            threading.Thread(target=run_gui_loop, daemon=True).start()

    if args.headless_node:
        print(f"[Node] Running in headless RPC mode on {OSLOADER.os}, profile={args.game_profile}, node_id={args.node_id}.")
        try:
            while True:
                time.sleep(3600)
        except KeyboardInterrupt:
