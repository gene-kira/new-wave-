#!/usr/bin/env python3
"""
UNIFIED_MEMORY_SYSTEM_BORG_NEURAL_OS_GOVERNOR_WITH_HUD_v2.py

Upgrades:
- Improved NeuralMemoryBrain (richer features + small online ML model)
- Improved RLMemoryBrain (better reward shaping, state-aware)
- Simple real ML model (online linear regressor)
- Improved cluster sharding (consistent hashing + latency-aware scoring)
- HUD state indicators via shared JSON file
- Performance optimizations (intervals, reduced I/O)
- HUD text for RAM/VRAM/CPU is black (bars still colored)
"""
...
# -----------------------------
# Simple online ML model
# -----------------------------

class OnlineLinearModel:
    """
    Simple "real ML" model: y = w^T x
...
class RLMemoryBrain:
    ...
    def select_action(self, state):
        # Slight bias: in PANIC/ALERT, explore more
        eps = self.epsilon
        if state in ("PANIC", "ALERT"):
            eps = min(0.3, self.epsilon * 2.0)
    ...
    def update(self, ram_ratio, vram_ratio, frag_score, state):
        ...
        # Reward: reduce pressure + reduce frag, penalize PANIC/ALERT
        reward = 0.0
...
class NeuralMemoryBrain:
    def __init__(self, lr=0.01):
        self.lr = lr
        self.dim = 10
        self.model_gpu = OnlineLinearModel(self.dim, lr=lr)
        self.model_cpu = OnlineLinearModel(self.dim, lr=lr)
        self.last_pressure = None
    ...
    def decide_and_learn(self, state, hints, buffer_meta, ram_ratio, vram_ratio, frag_score):
        ...
        for i, meta in enumerate(buffer_meta):
            ...
            score_gpu = self.model_gpu.predict(x)
            score_cpu = self.model_cpu.predict(x)
            ...
            if target is not None:
                ...
                target_val_gpu = 1.0 if target != "cpu" else 0.0
                target_val_cpu = 1.0 if target == "cpu" else 0.0

                self.model_gpu.update(x, target_val_gpu - 0.1 * grad_sign)
                self.model_cpu.update(x, target_val_cpu - 0.1 * grad_sign)
...
class MiniMemoryHUD:
    ...
    def update(self):
        ...
        if hud_state:
            self.state_label.config(text=f"State: {hud_state.get('state', 'N/A')}")
            self.profile_label.config(text=f"Profile: {hud_state.get('profile', 'N/A')}")
