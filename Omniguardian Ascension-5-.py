...
# ============================================================
# Codex Sentinel Forklift Node v12 – Neural Swarm Overmind
#
...
def _ml_like_process_score(proc_record: Dict) -> float:
    score = 0.0
...
def _classify_process(proc_record: Dict, score: float) -> str:
    if score < 1.0:
        return "benign"
    if 1.0 <= score < 2.0:
        return "suspicious"
...
def process_module_scan():
    ...
    for proc in psutil.process_iter(attrs=["pid", "name", "memory_info", "num_threads", "io_counters", "cpu_percent"]):
        try:
            ...
            anomaly_score = _ml_like_process_score(proc_record)
            label = _classify_process(proc_record, anomaly_score)
            process_labels[info["pid"]] = label
...
def _update_swarm_intel_from_snapshot():
    ...
    for key in ["rat_risk", "telemetry_spike_risk", "performance_risk", "game_crash_risk"]:
        val = am.get(key, "low")
        SWARM_INTEL["votes"].setdefault(key, []).append(val)
        if len(SWARM_INTEL["votes"][key]) > 128:
            SWARM_INTEL["votes"][key] = SWARM_INTEL["votes"][key][-128:]
...
def _compute_swarm_consensus():
    def consensus(votes: List[str]) -> str:
        ...
        for v in votes:
            if v in counts:
                counts[v] += 1
        if counts["high"] >= max(counts["medium"], counts["low"]):
            return "high"
        if counts["medium"] >= max(counts["high"], counts["low"]):
            return "medium"
        return "low"

    SWARM_INTEL["global_risk"]["rat_risk"] = consensus(SWARM_INTEL["votes"].get("rat_risk", []))
    SWARM_INTEL["global_risk"]["telemetry_risk"] = consensus(SWARM_INTEL["votes"].get("telemetry_spike_risk", []))
    SWARM_INTEL["global_risk"]["perf_risk"] = consensus(SWARM_INTEL["votes"].get("performance_risk", []))
    SWARM_INTEL["global_risk"]["crash_risk"] = consensus(SWARM_INTEL["votes"].get("game_crash_risk", []))
...
def _explain_guardian_state():
    ...
    explanations.append(
        f"Swarm consensus: crash={swarm_global['crash_risk']}, rat={swarm_global['rat_risk']}, "
...
def _init_gui():
    ...
    try:
        ...
        canvases["cpu_graph"] = add_canvas("CPU History", 9)
        canvases["perf_heatmap"] = add_canvas("Performance Risk Heatmap", 10)
        canvases["event_timeline"] = add_canvas("Event Timeline", 11)
