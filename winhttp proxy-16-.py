...
DEFAULT = {
    ...
    "health_window": 10,  # number of samples
    "health_history": [],  # last N health scores
...
# ============================================================
# EVENT BUS (HybridBrain-style internal messaging)
# ============================================================
...
def event_bus_dispatcher():
    while True:
        try:
            ...
            # Simple routing: extend as needed
            if etype == "proxy_changed":
                ui_queue.put(("refresh_status", None))
            elif etype == "health_updated":
                # could be used for future UI or logging
                pass
            elif etype == "profile_changed":
                ui_queue.put(("refresh_status", None))
            elif etype == "policy_action":
                # already applied by policy engine
                pass
        except Exception as e:
            logging.error(f"Event bus dispatcher error: {e}")
        time.sleep(0.05)
...
def validate_config(data):
    ...
    if not isinstance(data.get("profiles_routing"), dict):
        data["profiles_routing"] = DEFAULT["profiles_routing"]
        changed = True
...
def health_monitor_thread():
    while True:
        try:
            ...
            logging.error(f"Health monitor thread error: {e}")
            time.sleep(30)

# ============================================================
# POLICY ENGINE (simple rule interpreter)
# ============================================================
...
def policy_engine_thread():
    while True:
        try:
            ...
            logging.error(f"Policy engine thread error: {e}")
            time.sleep(30)
...
# ============================================================
# CLUSTER-SYNCED CONFIG / PROXY LISTS (multi-node swarm-ish)
# ============================================================
