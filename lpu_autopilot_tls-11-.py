...
# -*- coding: utf-8 -*-
"""
LPU Autopilot Kernel – Ultimate Unified Version (GUI fixed)

Everything-in-one:
- Autopilot core (state machine, safety, obstacle distance)
- LPU (NLP → validated autopilot commands)
- Hardware abstraction layer + async hardware polling
- CAN bus + GPS/IMU integration (stubs)
- Navigation manager + obstacle avoidance
- Swarm formation coordinator (stub)
- Mission scripting engine (JSON/YAML loader + runtime)
...
- Codex Purge Shell hooks
- Swarm sync propagation
- Daemonized watchdog
"""
...
import socket
import datetime
import yaml  # for YAML mission loader (PyPI: pyyaml)
...
SAFETY_INTERLOCKS_ENABLED = CONFIG.get("safety_interlocks_enabled", True)
WATCHDOGS_ENABLED = CONFIG.get("watchdogs_enabled", True)

USE_TLS = False  # can be toggled later if you want TLS IPC
...
class AutopilotCore:
    ...
    def get_state(self) -> VehicleState:
        with self._lock:
            return VehicleState(
                ...
                obstacle_distance_m=self.state.obstacle_distance_m,
            )
    ...
    def update_obstacle_distance(self, dist: float):
        with self._lock:
            self.state.obstacle_distance_m = dist
...
class NavigationManager:
    ...
    def compute_control(self, dt: float) -> Dict[str, float]:
        if not self.current_target:
            return {"throttle": 0.0, "steer": 0.0}
        return {"throttle": 0.5, "steer": 0.0}

class LidarInterface:
    def read_scan(self) -> List[float]:
        return [5.0, 4.0, 3.5, 10.0]

class ObstacleDetector:
    def compute_min_distance(self, scan: List[float]) -> float:
        if not scan:
            return 999.0
        return min(scan)
...
class ObstacleAvoidance:
    ...
    def modify_control_for_obstacles(self, control: Dict[str, float], min_dist: float) -> Dict[str, float]:
        if min_dist < 3.0:
            logger.warning(f"[ObstacleAvoidance] Obstacle too close ({min_dist:.2f} m) → braking.")
            control["throttle"] = min(control.get("throttle", 0.0), 0.0)
        return control
...
class FormationController:
    def __init__(self):
        self.mode = FormationMode.SOLO
        self.leader_id = None
        logger.info("[FormationController] Initialized.")

    def set_mode(self, mode: FormationMode, leader_id: Optional[str] = None):
        self.mode = mode
        self.leader_id = leader_id

    def compute_formation_offset(self, node_id: str) -> Tuple[float, float]:
        if self.mode == FormationMode.SOLO:
            return (0.0, 0.0)
        if self.mode == FormationMode.LEADER:
            return (0.0, 0.0)
        if self.mode == FormationMode.FOLLOWER:
            return (0.0, -5.0)
        return (0.0, 0.0)
    ...
    async def run(self):
        while True:
            ...
            if state.battery_level < 5.0:
                logger.warning("[Safety] Battery critically low → EMERGENCY_STOP.")
                self.autopilot_core.apply_system_event({"event_type": "stop_requested"})
            if state.obstacle_distance_m < 1.0:
                logger.warning("[Safety] Obstacle extremely close → EMERGENCY_STOP.")
                self.autopilot_core.apply_system_event({"event_type": "stop_requested"})
    ...
    async def run(self):
        while True:
            ...
            if time.time() - self.last_heartbeat > 10.0:
                logger.error("[Watchdog] No heartbeat for 10s → potential hang.")
                # In real system, trigger reset or safe state.

WATCHDOG = WatchdogManager()

# =============================================================================
# Async Hardware wrapper (full control path)
# =============================================================================

class AsyncHardwareWrapper:
    def __init__(self, autopilot_core: AutopilotCore, lidar: LidarInterface,
                 obstacle_detector: ObstacleDetector, nav: NavigationManager,
                 obstacle_avoidance: ObstacleAvoidance, interval: float = 0.1):
        self.autopilot_core = autopilot_core
        self.lidar = lidar
    ...
    async def run(self):
        while True:
            try:
                ...
                lidar_scan = self.lidar.read_scan()
                min_dist = self.obstacle_detector.compute_min_distance(lidar_scan)
                self.autopilot_core.update_obstacle_distance(min_dist)
                await EVENT_BUS.publish("lidar_scan", {"min_dist": min_dist})

                now = time.time()
                dt = now - self._last_time
                self._last_time = now

                control = self.nav.compute_control(dt)
                control = self.obstacle_avoidance.modify_control_for_obstacles(control, min_dist)
                HARDWARE.apply_control(control)
...
if PySide6:
    class LPUClientGUI(QtWidgets.QMainWindow):
        ...
        def set_mode(self):
            ...
            asyncio.run_coroutine_threadsafe(self._set_mode_async(mode), self.loop)
        ...
        def send_text(self):
            ...
            asyncio.run_coroutine_threadsafe(self._send_text_async(text, stream), self.loop)
...
# =============================================================================
# Server entry (single asyncio loop with tasks) + GUI integration
# =============================================================================

async def main_async_server(autopilot_core: AutopilotCore,
    ...
    obstacle_avoidance = ObstacleAvoidance(autopilot_core)
    async_hw = AsyncHardwareWrapper(autopilot_core, lidar, obstacle_detector, nav, obstacle_avoidance)

    server = LPUIPCServer(lpu)
    rest_server = RESTServer(autopilot_core, lpu)
    mqtt_bridge = MQTTBridge(lpu)
    copilot = AICopilot(lpu.backend, autopilot_core)
    ...
    tasks = [
        ...
        WATCHDOG.run() if WATCHDOGS_ENABLED else asyncio.sleep(0),
    ]

    if safety_manager:
        tasks.append(safety_manager.run())

    await asyncio.gather(*tasks)

def run_server_in_background(loop: asyncio.AbstractEventLoop,
                             autopilot_core: AutopilotCore,
                             nav: NavigationManager,
                             mission_engine: MissionEngine,
                             lpu: LanguageProcessingUnit,
    ...
    loop.run_until_complete(main_async_server(autopilot_core, nav, mission_engine, lpu, safety_manager))

def main():
    ensure_certs()

    autopilot_core = AutopilotCore()
    nav = NavigationManager(autopilot_core)
    mission_engine = MissionEngine(autopilot_core, nav)
    ...
    safety_manager = SafetyManager(autopilot_core) if SAFETY_INTERLOCKS_ENABLED else None

    loop = asyncio.new_event_loop()
    server_thread = threading.Thread(
        target=run_server_in_background,
        args=(loop, autopilot_core, nav, mission_engine, lpu, safety_manager),
        daemon=True,
    )
    server_thread.start()
    ...
    if PySide6:
        ...
        logger.info("PySide6 not available, running headless server only.")
        try:
            server_thread.join()
