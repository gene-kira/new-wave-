#!/usr/bin/env python
# game_guardian_ultra_ai.py
#
# Game Guardian ULTRA AI:
...
# - Real ML model integration hook (optional gg_ml_model.pkl)
# - Server reputation scoring (good/bad encounters per IP)
# - AI-style threat summaries (text explanation of risk)
# - Auto-block suggestions (manual-only actual blocking)
...
# - Live latency graph (per-scan latency bars, deep blue mid-level)
...
REQUIRED_LIBS = [
    ...
    "pystray",
    "Pillow",
    "scikit-learn",  # for optional real ML model integration
]
...
BLUE = "#0055FF"  # deep blue for mid-level visualization
...
ip_reputation: Dict[str, Dict[str, int]] = {}  # {"ip": {"good": x, "bad": y}}
...
def ml_model_predict_adjust(ip: str, base_score: int, latency_ms: Optional[float], loss_pct: Optional[float]) -> int:
    # If a real ML model is present, use it; otherwise, simple heuristic adjust.
    if ml_model is not None:
        try:
            ...
            prob = ml_model.predict_proba(features)[0][1]  # probability of "bad"
            adjust = int(prob * 20)  # up to +20
...
def create_tray_icon(on_restore):
    ...
    icon = pystray.Icon("GGUltraAI", img, "Game Guardian", menu)
    return icon
...
class GameGuardianGUI:
    def __init__(self):
        ...
        tk.Label(top_frame, text=f"Game Guardian ULTRA AI v{CURRENT_VERSION}", bg=self.bg_color, fg=self.fg_color).pack(side=tk.LEFT, padx=5)
        ...
        tk.Label(side_frame, text="Threat Summary (AI-style)", bg=self.bg_color, fg=self.fg_color).pack(pady=5)
        self.summary_text = tk.Text(side_frame, width=32, height=10, bg="#101010", fg="#FFFFFF", wrap="word")
        self.summary_text.pack(pady=5)
        self.summary_text.insert("1.0", "Threat summaries will appear here when high-risk events are detected.")
    ...
    def update_threat_graph(self):
        ...
        for i, score in enumerate(scores):
            ...
            if score >= 80:
                ...
                color = BLUE  # deep blue mid-level
            elif score >= 20:
                color = BLUE  # also blue for lower mid-level
    ...
    def update_latency_graph(self):
        ...
        for i, lat in enumerate(latencies):
            ...
            if lat > 150:
                ...
                color = BLUE  # deep blue lower mid-level
