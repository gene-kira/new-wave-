...
def morph_profiles_based_on_health():
    ...
    # Simple morphing: adjust thresholds based on health
    with config_lock:
...
def elect_leader(nodes):
    ...
    # simple heuristic: highest health, then highest config_version
    best = None
    for n in nodes:
    ...
    while True:
        try:
            ...
            if role == "leader":
                ...
                if remote_cfg:
                    # optional leader election if remote_cfg contains nodes
                    ...
                    if nodes:
                        ...
                        if leader and leader.get("node_id") == config.get("node_id"):
                            with config_lock:
                                ...
                                save_config(config)
                            logging.info("[CLUSTER] Promoted to leader via election")
                        else:
...
class FluidLatencyField(ProbabilisticField):
    """
    Fluid-physics style latency field:
    mean ~ pressure, var ~ turbulence, velocity ~ trend.
    """
    def __init__(self, mean: float, var: float):
        super().__init__(mean, var)
        self.velocity = 0.0  # trend
        self.viscosity = 1.0  # stability
    ...
    def update(self, observation: float, weight: float = 1.0):
        ...
        # viscosity: lower when turbulence high
...
def fusion_update_from_stats():
    ...
    with fusion_lock:
        ...
        # anomaly heatmap: simple buckets
        heatmap = []
        if lat is not None:
            if lat > 1000:
...
def emergent_behavior_thread():
    ...
    Uses fusion_state + history to morph profiles and thresholds.
    """
    while True:
        try:
...
def self_tuning_thread():
    ...
    Adjusts latency_threshold_ms and predictive_sensitivity based on performance.
    """
    while True:
        try:
            ...
            failures = ps.get("failures", 0)
            # simple tuning: if failures high, lower threshold and increase sensitivity
            with config_lock:
...
# ============================================================
# DAEMON WATCHDOG (SIMULATED EXTERNAL)
# ============================================================
...
root = tk.Tk()
root.title("Proxy Controller Pro - HybridBrain v2")
root.geometry("900x760")
root.configure(bg="#202020")

tk.Label(
    root,
    text="Proxy Controller Pro - HybridBrain v2",
    fg="lime",
    bg="#202020",
...
cluster_url_entry.grid(row=3, column=1, padx=10)
with config_lock:
