#!/usr/bin/env python3
# ============================================================
# HybridBrain Unified Memory Daemon v7
    ...
    def _apply_ai_mode_bias(self, adjustment, cpu, ram_ratio, disk_lat):
        if self.ai_mode == AI_MODE_AGGRESSIVE:
            ...
            if cpu > 80 or disk_lat > 8.0:
                self.target_cache_percent -= 4.0
        # balanced: no extra bias beyond heuristics
        return adjustment
            ...
            try:
                ...
                if self.swarm_client:
                    ...
                    if swarm_score < 0.4:
                        ...
                        TIER_COST_SWARM = 10.0  # local bias; not perfect but pushes decisions
...
class WorkloadManager:
    def __init__(self, engine: UnifiedMemoryEngine):
        ...
        log_info("[WORKLOAD] Manager started (files + network + sensors)")
    ...
    def _update_engine_state(self):
        combined_intensity = max(self.file_intensity, self.net_intensity, self.sensor_intensity)
        if self.sensor_intensity >= self.file_intensity and self.sensor_intensity >= self.net_intensity:
            wtype = "sensor"
...
def main():
    log_info("=== HybridBrain Unified Memory Daemon v7 (AI Governor) ===")
