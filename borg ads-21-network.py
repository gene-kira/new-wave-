#!/usr/bin/env python3
"""
ULTRABORG V8.0 — GAMING-FIRST FULL SYSTEM GOVERNOR
(RL + DEEP RL + WATCHDOG + ANOMALY + ADBLOCK + GUI + EVOLUTION + SUPERVISOR +
 ...
 AUTO GAME MODE + ADVANCED NETWORK SCANNER (AUTO CIDR, ARP, ICMP, TCP, WAMI FULL-PORT) +
 ...
 WADI (Wide Area Data Imagery) FUSION ENGINE + AI-GOVERNED FIREWALL POLICY ENGINE)
"""
...
DEFAULT_CONFIG = {
    ...
    "network": {
        ...
        "full_port_scan": True,          # WAMI-style: enable 1–65535 awareness
        "full_port_sample_limit": 256    # adaptive sampling per host
    },
    "patch": {
...
def load_config() -> Dict[str, Any]:
    ...
    print("[CONFIG] Using embedded default config (gaming-first, V8.0)")
...
def get_proc_info(proc):
    ...
    for old_pid, old_info in list(PROCESS_CACHE.items()):
        if old_info.get("name") == name and old_info.get("path") == path:
            resurrected = True
...
def cleanup_cache(current_pids: List[int]):
    ...
    for pid in list(PROCESS_CACHE.keys()):
        if pid not in current_pids:
            info = PROCESS_CACHE.pop(pid, None)
            if info and now - info.get("last_seen", now) > 60.0:
                LOGGER.log("proc_exit", info)
...
# ============================================================
# NETWORK: AUTO CIDR + ARP + ICMP + TCP + WAMI FULL-PORT
# ============================================================
...
class NetworkScanner:
    """
    WAMI-style scanner: instead of peeking at a few ports, we build a wide-area
    “image” of the LAN — ICMP + ARP + TCP, with adaptive full-port sampling.
    """
    def __init__(self, cfg: Dict[str, Any]):
        self.cfg = cfg
        ...
        self.full_port_scan = cfg.get("full_port_scan", False)
        self.full_port_sample_limit = cfg.get("full_port_sample_limit", 256)
        self.running = True
        self.thread = threading.Thread(target=self.loop, daemon=True)
    ...
    def start(self):
        print("[NET] NetworkScanner starting (WAMI mode)")
        self.thread.start()
    ...
    def _adaptive_full_port_sample(self, ip: str) -> List[int]:
        """
        WAMI-style: instead of literally scanning 1–65535 every interval,
        we sample a subset but across the whole range, giving a “wide-area”
        port image without killing the machine.
        """
        ports = set(self.tcp_ports)
        sample_limit = self.full_port_sample_limit
        # Spread samples across full range
        while len(ports) < sample_limit:
            p = random.randint(1, 65535)
            ports.add(p)
        open_ports = []
        for p in ports:
            if self._probe_tcp(ip, p, timeout=0.15):
                open_ports.append(p)
        ...
        if alive:
            ...
            for p in self.tcp_ports:
                if self._probe_tcp(ip, p):
                    open_ports.append(p)
            # WAMI full-port sampling if enabled
            if self.full_port_scan:
                extra = self._adaptive_full_port_sample(ip)
                # merge
...
class WADIFusionEngine:
    """
    WADI: fuse *everything* into a single frame:
    - sensors
    - network WAMI snapshot
    - RL action + reward
    - anomaly flag
    """
    def __init__(self):
    ...
    def build_frame(self,
                    ...
                    action: str,
                    reward: float,
                    net_info: Dict[str, float]) -> Dict[str, Any]:
        devices = NETWORK_SCANNER.get_snapshot()
        anomalous = ANOMALY.is_anomalous(snap)
        frame = {
            "time": time.time(),
            "version": CONFIG.get("version", "8.0.0"),
            "sensors": {
                "cpu": snap.cpu_usage,
                "ram": snap.ram_usage,
                "gpu": snap.gpu_usage,
            ...
            "network": {
                "health": net_info.get("net_health", 1.0),
                "raw_health": net_info.get("net_health_raw", 1.0),
                "lan_stability": net_info.get("lan_stability", 1.0),
                "fw_penalty": net_info.get("fw_penalty", 0.0),
...
# ============================================================
# MOBILE REMOTE STUB (telemetry + WADI frame)
# ============================================================
...
class MobileRemoteHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/telemetry":
            ...
            # full fused frame
            frame = WADI_ENGINE.last_frame or {}
...
class BorgGUI:
    ...
    def _build_ui(self):
        ...
        self.net_tree.column("ports", width=260, anchor="w")
...
def borg_main_loop():
    ...
    while True:
        try:
            ...
            # WADI fusion frame
            WADI_ENGINE.build_frame(snap, action, reward, net_info)
