...
# - ForkliftLinear LLM wrapper + TinyFallback (Ultra Swarm Hybrid Unified version)
...
numpy_pkg = autoload("numpy", "numpy")
...
# ============================================================
# TORCH / TRANSFORMERS + FORKLIFT LAYER (Ultra Swarm Hybrid Unified)
# ============================================================
...
class DataIngestion:
    ...
    def ingest_all(self) -> Dict[str, Dict[str, float]]:
        ...
        iot_state = self.iot.sample_iot_state()
        self._training_buffer.append(system)
        if len(self._training_buffer) >= 50:
            self.ml_backend_train_local_sklearn(self._training_buffer)
    ...
    def ml_backend_train_local_sklearn(self, samples: List[Dict[str, float]]):
        if sklearn is None or numpy_pkg is None or joblib is None:
            print("[MLBackend] sklearn/numpy/joblib not available, skipping training.")
            return
        try:
            ...
            for s in samples:
                ...
                X.append(v)
                label = 1 if (v[0] + v[1] + v[2]) > 1.8 else 0
                y.append(label)
            if not X:
                return
            X = np.array(X)
            ...
            clf.fit(X, y)
            joblib.dump(clf, "ml_backend_model.joblib")
            print("[MLBackend] Trained and saved sklearn model from real telemetry.")
        except Exception as e:
            print(f"[MLBackend] sklearn training error: {e}")
    def stop(self):
        self.fs.stop()
...
# ============================================================
# LLM TELEMETRY + GENERATION + RPC + CLI (Ultra Swarm Hybrid Unified)
# ============================================================
