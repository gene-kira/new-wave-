# ===========================================================
# BORG OS v33 — System Autopilot, Game-Aware Governor, Insight Export
...
PROFILE_PATH = os.path.join(BASE_DIR, "intelligence_profile_v33.json")
...
def log(msg: str):
    ...
    print(line)
    try:
        with open(os.path.join(LOG_DIR, "borg_v33.log"), "a", encoding="utf-8") as fp:
            fp.write(line + "\n")
    except Exception:
        pass
...
def _subscribe_memory_graph():
    for et in [
        ...
        "autopilot.profile_applied", "kernel.mode_changed"
    ]:
...
SWARM_SHARED_KEY = b"borg_v33_shared_key"
...
class InsightExportEngine:
    ...
    def export(self, organism_state: Dict[str, Any], recent_actions: List[Dict[str, Any]]):
        payload = {
            "ts": time.time(),
...
# ===========================================================
# BORG ORGANISM CORE — v33
# ===========================================================
class BORGOrganism:
    def __init__(self):
        log("[Organism] Initializing organism core v33...")
        ...
        log("[Organism] Core v33 initialized.")
    ...
    def _scheduler_loop(self):
        while True:
            if not self.scheduler_running:
                return
            try:
                log("[Organism] Scheduler tick v33...")
    ...
    def shutdown(self):
        log("[Organism] Shutting down v33...")
        self.scheduler_running = False
...
class DataIntelligenceEngine:
    ...
    def _design_new_modules(self) -> List[Dict[str, Any]]:
        ...
        blueprints.append({
            ...
            "type": "governor",
            "description": "Watches processes, GPU/CPU, RAM, disk I/O and applies profiles."
        })
        blueprints.append({
            "name": "GameAwareOptimizerOrgan",
            "type": "governor",
            "description": "Detects game exes, locks kernel mode to gaming, adjusts governor."
        })
        ...
        blueprints.append({
            ...
            "type": "governor",
            "description": "Adjusts kernel mode based on load."
        })
        return blueprints
...
class SystemAutopilotOrgan:
    ...
    def _detect_profile(self):
        cpu = psutil.cpu_percent(interval=0.1)
        mem = psutil.virtual_memory().percent
        procs = [p.info for p in psutil.process_iter(["name"])]

        names = [p.get("name") or "" for p in procs]
        gaming = any(name.lower().endswith((".exe")) and ("steam" in name.lower() or "epic" in name.lower() or "game" in name.lower()) for name in names)
        streaming = any("obs" in name.lower() or "twitch" in name.lower() or "stream" in name.lower() for name in names)
        work = any("chrome" in name.lower() or "edge" in name.lower() or "word" in name.lower() or "excel" in name.lower() for name in names)
    ...
    def _apply_profile(self, profile):
        ...
        insight = f"SystemAutopilotOrgan: applied profile {profile} based on system state."
        policy = {
    ...
    def run(self):
        ...
        if gaming:
            policy = {
                "kernel_update": {
                    "mode": "gaming",
                    "scenario": "GameAwareOptimizerOrgan: game detected, gaming mode engaged"
                },
                "persona_update": {
                    "traits": {"gaming_focus": True},
                    "history_event": "GameAwareOptimizerOrgan engaged gaming mode due to game process."
                },
                "insight": f"GameAwareOptimizerOrgan: game detected, cpu={cpu}, mem={mem}"
            }
            return {"status": "ok", "policy": policy}
        return {"status": "ok"}
'''
...
# ===========================================================
app = FastAPI(title="BORG OS v33 Telemetry & Control", version="33.0.0")
