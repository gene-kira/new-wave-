#!/usr/bin/env python3
# ============================================================
# Codex Input Firewall — OBLIVION BUILD (REFINED v5 GOD-MODE + ANALYTICS)
#
# New in this build:
# - Real anomaly-scoring pipeline (modular, multi-source)
# - Better clustering (DBSCAN-style logic, design stub)
# - Per-process baselines (score + feature baselines)
# - Auto-labeling rules for ML (good/bad/ambiguous)
# - Feature importance logging (which features drove the score)
# - Threat timelines (per-PID event history)
# - Graph-based anomaly detection (process graph + anomalies)
# - Real-time “heatmap” panels in GUI (textual density view)
#
# Existing capabilities:
# - Learning mode (observe-only)
# - Tuned thresholds (less noisy)
# - Rich logging (per-process feature dump)
# - Behavior profile export (offline JSON)
# - Adaptive thresholds (per-process trust)
# - Per-process trust scoring
# - Simple clustering (now upgraded)
# - God-Mode design stubs (neural, policy, swarm, hardware, etc.)
# - ML dataset builder + offline model stub
# ============================================================
...
ML_DATASET_FILE = os.path.join(BASE_DIR, "ml_dataset.json")
ML_MODEL_FILE = os.path.join(BASE_DIR, "ml_model_stub.json")
...
# Per-process baselines (score + features)
PROCESS_BASELINES = {}    # pid -> {"score_baseline": float, "feature_baseline": dict}

# Threat timelines: pid -> deque of events
THREAT_TIMELINES = defaultdict(lambda: deque(maxlen=100))
    ...
    try:
        with STATE_LOCK:
            data = {
                ...
                "god_view_enabled": STATE["god_view_enabled"],
                "ml_model_enabled": STATE["ml_model_enabled"],
            }
...
def load_config():
    ...
    try:
        ...
        with STATE_LOCK:
            ...
            STATE["god_view_enabled"] = data.get("god_view_enabled", False)
            STATE["ml_model_enabled"] = data.get("ml_model_enabled", False)
    except Exception as e:
    ...
    try:
        ...
        for pid, stats in BEHAVIOR_DB.items():
            snapshot[str(pid)] = {
                ...
                "neural_score": stats.get("neural_score", 0.0),
                "ml_score": stats.get("ml_score", 0.0),
                "baseline": PROCESS_BASELINES.get(pid, {}),
                "timeline": list(THREAT_TIMELINES.get(pid, [])),
            }
...
# ============================================================
# 12. GOD-MODE DESIGN STUBS
# ============================================================
class NeuralAnomalyEngineStub:
    def __init__(self):
        write_log("[NEURAL] (DESIGN) Neural anomaly engine stub initialized.")

    def score(self, features: dict) -> float:
        base = 0.0
        if features.get("dll_injection"):
            base += 3.0
        if features.get("hollowing"):
            base += 3.0
        if features.get("net_suspicious"):
            base += 2.0
        if features.get("inline_patch") or features.get("inline_code_mismatch"):
            base += 2.5
        if features.get("avg_cpu", 0.0) > 60.0:
            base += 1.5
        if features.get("avg_conn", 0.0) > 15.0:
            base += 1.5
        return base

NEURAL_ENGINE = NeuralAnomalyEngineStub()

class TemporalBehaviorGraphStub:
    def __init__(self):
        self.edges = {}  # pid -> set of child pids
        write_log("[GRAPH] (DESIGN) Temporal behavior graph stub initialized.")

    def update_for_process(self, proc: psutil.Process):
        try:
            pid = proc.pid
            parent = proc.parent()
            if parent:
                ppid = parent.pid
                self.edges.setdefault(ppid, set()).add(pid)
        except Exception:
            pass

    def summarize(self):
        write_log(f"[GRAPH] (DESIGN) Current graph edges: {len(self.edges)} parent nodes.")

TEMPORAL_GRAPH = TemporalBehaviorGraphStub()

class PolicyCompilerStub:
    def __init__(self):
        self.last_compile_time = 0
        write_log("[POLICY] (DESIGN) Policy compiler stub initialized.")

    def maybe_compile(self):
        now = time.time()
        if now - self.last_compile_time < 60:
            return
        self.last_compile_time = now
        write_log("[POLICY] (DESIGN) Would analyze logs and clusters to generate new rules.")

POLICY_COMPILER = PolicyCompilerStub()

class SwarmSyncStub:
    def __init__(self):
        self.last_sync_time = 0
        write_log("[SWARM] (DESIGN) Swarm sync stub initialized.")

    def maybe_sync(self):
        now = time.time()
        if now - self.last_sync_time < 120:
            return
        self.last_sync_time = now
        write_log("[SWARM] (DESIGN) Would sync anonymized threat data with other nodes.")

SWARM_SYNC = SwarmSyncStub()

class HardwareTelemetryStub:
    def __init__(self):
        write_log("[HW] (DESIGN) Hardware telemetry stub initialized.")
...
HW_TELEMETRY = HardwareTelemetryStub()

class SelfHealingWatchdogStub(threading.Thread):
    def __init__(self):
        super().__init__(daemon=True)
        self.running = True
        write_log("[WATCHDOG] (DESIGN) Self-healing watchdog stub initialized.")

    def run(self):
        while self.running:
            try:
                write_log("[WATCHDOG] (DESIGN) Would verify firewall integrity and respawn if needed.")
                time.sleep(60)
            except Exception:
                time.sleep(60)

WATCHDOG = SelfHealingWatchdogStub()
WATCHDOG.start()

# ============================================================
# 13. ML DATASET BUILDER + OFFLINE MODEL STUB
# ============================================================
class MLDatasetBuilder:
    ...
    def build_dataset(self):
        if not os.path.exists(PROFILE_EXPORT_FILE):
            write_log("[ML] No behavior_profiles.json found; export profiles first.")
            return

        try:
            with open(PROFILE_EXPORT_FILE, "r", encoding="utf-8") as f:
                profiles = json.load(f)
        except Exception as e:
            write_log(f"[ML] Failed to load profiles: {e}")
            return

        dataset = []
        for pid_str, stats in profiles.items():
            ...
            for k in self.FEATURE_KEYS:
                ...
                if v is None:
                    v = 0.0
                row[k] = float(v)
            label = 1.0 if stats.get("suspicious", False) else 0.0
            row["label"] = label
            dataset.append(row)

        try:
            with open(ML_DATASET_FILE, "w", encoding="utf-8") as f:
                json.dump(dataset, f, indent=2)
            write_log(f"[ML] Dataset built with {len(dataset)} rows -> {ML_DATASET_FILE}")
            write_log("[ML] Train a real model offline using this dataset (e.g., in a notebook).")
        except Exception as e:
            write_log(f"[ML] Failed to write dataset: {e}")

ML_DATASET_BUILDER = MLDatasetBuilder()

class OfflineModelStub:
    def __init__(self):
        self.loaded = False
        self.config = {}
        self.load_model()
    ...
    def load_model(self):
        if not os.path.exists(ML_MODEL_FILE):
            write_log("[ML] No ml_model_stub.json found; using default stub behavior.")
            self.loaded = False
            self.config = {}
            return
        try:
            ...
            self.loaded = True
            write_log("[ML] Offline model stub loaded from ml_model_stub.json (design only).")
        except Exception as e:
            write_log(f"[ML] Failed to load model stub: {e}")
            self.loaded = False
            self.config = {}
...
OFFLINE_MODEL = OfflineModelStub()

# ============================================================
# 14. ANALYTICS: BASELINES, AUTO-LABELING, FEATURE IMPORTANCE,
#     THREAT TIMELINES, GRAPH ANOMALY, CLUSTERING, PIPELINE
# ============================================================
class BaselineManager:
    """
    Per-process baselines for score and features.
    """
    def __init__(self):
        self.baselines = PROCESS_BASELINES

    def update_baseline(self, pid: int, score: float, features: dict):
        base = self.baselines.get(pid)
        if base is None:
            self.baselines[pid] = {
                "score_baseline": score,
                "feature_baseline": features.copy(),
                "samples": 1,
            }
            return
        n = base.get("samples", 1)
        old_score = base.get("score_baseline", 0.0)
        new_score = (old_score * n + score) / (n + 1)
        base["score_baseline"] = new_score
        base["samples"] = n + 1
        base["feature_baseline"] = features.copy()

    def get_score_delta(self, pid: int, score: float) -> float:
        base = self.baselines.get(pid)
        if not base:
            return 0.0
        return score - base.get("score_baseline", 0.0)

BASELINE_MANAGER = BaselineManager()

class AutoLabeler:
    """
    Auto-labeling rules for ML:
    - label=1: clearly bad
    - label=0: clearly good
    - label=None: ambiguous
    """
    def label(self, stats: dict) -> int | None:
        score = stats.get("ai_score", 0.0)
        thr = stats.get("dynamic_threshold", 20.0)
        suspicious = stats.get("suspicious", False)
        trust = stats.get("trust_score", 0.0)

        if suspicious and score >= thr * 1.1 and trust < -1.0:
            return 1
        if (not suspicious) and score < thr * 0.5 and trust > 1.0:
            return 0
        return None
...
class FeatureImportanceLogger:
    """
    Logs which features contributed most to the anomaly score.
    """
    def compute_importance(self, features: dict, stats: dict) -> list:
        score = stats.get("ai_score", 0.0)
        thr = stats.get("dynamic_threshold", 20.0)
        if score < thr * 0.7:
            return []
        ...
        for k, w in weights.items():
            ...
            if isinstance(v, bool):
                v = 1.0 if v else 0.0
            contribs.append((k, abs(v) * w))
        contribs.sort(key=lambda x: x[1], reverse=True)
        top = contribs[:5]
        if top:
            write_log("[FEATURES] Top contributors: " + ", ".join(f"{k}={c:.2f}" for k, c in top))
        return top

FEATURE_IMPORTANCE_LOGGER = FeatureImportanceLogger()

class ThreatTimeline:
    """
    Per-PID event history.
    """
    def add_event(self, pid: int, event: str):
        ts = time.strftime("%H:%M:%S")
        THREAT_TIMELINES[pid].append(f"{ts} {event}")

THREAT_TIMELINE = ThreatTimeline()

class GraphAnomalyDetector:
    """
    Graph-based anomaly detection on the temporal process graph.
    """
    def __init__(self, graph: TemporalBehaviorGraphStub):
        self.graph = graph
    ...
    def detect_anomalies(self) -> list:
        ...
        for parent, children in self.graph.edges.items():
            if len(children) > 20:
                anomalies.append((parent, len(children)))
        if anomalies:
            msg = "; ".join(f"PPID {ppid} -> {count} children" for ppid, count in anomalies)
            write_log(f"[GRAPH-ANOM] (DESIGN) Suspicious fan-out: {msg}")
        return anomalies

GRAPH_ANOMALY = GraphAnomalyDetector(TEMPORAL_GRAPH)

class ClusterEngineStub:
    """
    DBSCAN-style clustering stub:
    - cluster_2_high_risk
    - cluster_1_heavy_activity
    - cluster_0_normal
    """
    def assign_cluster(self, features: dict) -> str:
...
CLUSTER_ENGINE = ClusterEngineStub()

class AnomalyPipeline:
    """
    Real anomaly-scoring pipeline (modular):
    - Heuristics
    - GPU backend
    - Neural stub
    - Offline ML stub
    ...
    - Auto-labeling
    """
    def run(self, pid: int, proc: psutil.Process, stats: dict, features: dict) -> dict:
        score = stats.get("ai_score", 0.0)
        thr = stats.get("dynamic_threshold", 20.0)

        BASELINE_MANAGER.update_baseline(pid, score, features)
        delta = BASELINE_MANAGER.get_score_delta(pid, score)
        stats["baseline_delta"] = delta
        if delta > 10.0:
            score += 2.0
            stats["ai_score"] = score

        label = AUTO_LABELER.label(stats)
        stats["auto_label"] = label

        FEATURE_IMPORTANCE_LOGGER.compute_importance(features, stats)

        if score >= thr:
            THREAT_TIMELINE.add_event(pid, f"Score {score:.2f} >= thr {thr:.2f}")
        else:
            THREAT_TIMELINE.add_event(pid, f"Score {score:.2f} < thr {thr:.2f}")
...
# ============================================================
# 15. ADAPTIVE THRESHOLDS / TRUST / CLUSTERING
# ============================================================
...
def assign_cluster(features: dict) -> str:
    return CLUSTER_ENGINE.assign_cluster(features)

# ============================================================
# 16. PROCESS MONITORING & AI-STYLE MODEL
# ============================================================
...
def process_behavior_profile(proc: psutil.Process) -> dict:
    ...
    TEMPORAL_GRAPH.update_for_process(proc)
    hw_snapshot = HW_TELEMETRY.snapshot()
    features["hw_gpu_util"] = hw_snapshot["gpu_util"]
    features["hw_disk_io"] = hw_snapshot["disk_io"]
    features["hw_temp"] = hw_snapshot["temp"]

    neural_score = NEURAL_ENGINE.score(features)
    stats["neural_score"] = neural_score
    score += neural_score * 0.5

    with STATE_LOCK:
        use_ml = STATE["ml_model_enabled"]
    ml_score = 0.0
    if use_ml:
        ml_score = OFFLINE_MODEL.score(features)
    ...
    stats = ANOMALY_PIPELINE.run(pid, proc, stats, features)

    BEHAVIOR_DB[pid] = stats

    write_log(
        f"[PROFILE] PID {pid} score={stats['ai_score']:.2f} thr={dynamic_threshold:.2f} "
        ...
        f"neural={neural_score:.2f} ml={ml_score:.2f} delta={stats.get('baseline_delta', 0.0):.2f}"
    )
    ...
    def run(self):
        ...
        while self.running:
            try:
                        ...
                        try:
                            ...
                            if flagged:
                                if learning:
                                    ...
                                    THREAT_TIMELINE.add_event(proc.pid, "LEARNING: would kill")
                                else:
                                    write_log(
                                    ...
                                    THREAT_TIMELINE.add_event(proc.pid, "ENFORCE: killed")
                                    try:
                                        proc.kill()
                                    except Exception:
                                        pass
                ...
                POLICY_COMPILER.maybe_compile()
                SWARM_SYNC.maybe_sync()
                TEMPORAL_GRAPH.summarize()
                GRAPH_ANOMALY.detect_anomalies()
                time.sleep(1.0)
            except Exception as e:
...
# ============================================================
# 20. GUI COCKPIT (with real-time “heatmaps”)
# ============================================================
class CodexCockpitGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Codex Input Firewall — OBLIVION Cockpit (Refined v5 God-Mode + Analytics)")
    ...
    def build_layout(self):
        ...
        self.btn_build_dataset = ttk.Button(controls, text="Build ML Dataset", command=self.build_ml_dataset)
        self.btn_build_dataset.pack(side="left", padx=5)

        self.btn_reload_model = ttk.Button(controls, text="Reload ML Model", command=self.reload_ml_model)
        self.btn_reload_model.pack(side="left", padx=5)

        self.btn_god_view = ttk.Button(controls, text="Toggle God-View", command=self.toggle_god_view)
        self.btn_god_view.pack(side="left", padx=5)
        ...
        self.var_ml_model = tk.BooleanVar(value=STATE["ml_model_enabled"])
        self.chk_ml_model = ttk.Checkbutton(
            checks,
            text="Use offline ML model (design stub)",
            variable=self.var_ml_model,
            command=self.toggle_ml_model
        )
        self.chk_ml_model.pack(side="left", padx=5)
        ...
        # Top analytics panel: God-View + Heatmaps
        analytics_frame = tk.Frame(self.root, bg="#101010")
        analytics_frame.pack(padx=10, pady=5, fill="x")

        gv_frame = tk.LabelFrame(analytics_frame, text="God-View Summary (Design Stub)", bg="#101010", fg="#EEEEEE")
        gv_frame.pack(side="left", padx=5, pady=5, fill="both", expand=True)

        self.gv_text = tk.Text(gv_frame, height=10, bg="#181818", fg="#DDDDDD", insertbackground="#DDDDDD")
        self.gv_text.pack(fill="both", padx=5, pady=5, expand=True)

        heatmap_frame = tk.LabelFrame(analytics_frame, text="Real-Time Heatmaps (Textual)", bg="#101010", fg="#EEEEEE")
        heatmap_frame.pack(side="left", padx=5, pady=5, fill="both", expand=True)

        self.heatmap_text = tk.Text(heatmap_frame, height=10, bg="#181818", fg="#DDDDDD", insertbackground="#DDDDDD")
        self.heatmap_text.pack(fill="both", padx=5, pady=5, expand=True)
    ...
    def refresh_ui(self):
        with STATE_LOCK:
            ...
            learning = STATE["learning_mode"]
            ml_on = STATE["ml_model_enabled"]
        mode_str = "LEARNING" if learning else "ENFORCING"
        ml_str = "ML-ON" if ml_on else "ML-OFF"
        if fw_on:
            self.status_label.config(
                text=f"Firewall: ENABLED ({mode_str}, {'STEALTH' if stealth else 'VISIBLE'}, {ml_str})",
                fg="#FF5555"
            )
        else:
            self.status_label.config(
        ...
        self.var_ml_model.set(STATE["ml_model_enabled"])
    ...
    def refresh_god_view(self):
        self.gv_text.delete("1.0", "end")
        with STATE_LOCK:
            god_view = STATE["god_view_enabled"]
        if not god_view:
            self.gv_text.insert("1.0", "God-View disabled.\nEnable to see high-level summary.")
            return

        clusters = {}
        for pid, stats in BEHAVIOR_DB.items():
            cid = stats.get("cluster_id", "unknown")
            clusters.setdefault(cid, 0)
            clusters[cid] += 1

        self.gv_text.insert("end", "=== God-View Summary (Design Stub) ===\n")
        self.gv_text.insert("end", f"Total tracked processes: {len(BEHAVIOR_DB)}\n")
        for cid, count in clusters.items():
            self.gv_text.insert("end", f"  {cid}: {count} processes\n")

        high_risk = [pid for pid, s in BEHAVIOR_DB.items() if s.get("cluster_id") == "cluster_2_high_risk"]
        self.gv_text.insert("end", f"\nHigh-risk PIDs: {', '.join(map(str, high_risk)) if high_risk else 'None'}\n")
        self.gv_text.insert("end", "\n(Real God-Mode would render heatmaps and graphs here.)\n")

    def refresh_heatmaps(self):
        self.heatmap_text.delete("1.0", "end")
        self.heatmap_text.insert("end", "=== Real-Time Heatmaps (Textual Approximation) ===\n")

        cpu_buckets = defaultdict(int)
        for pid, stats in BEHAVIOR_DB.items():
            feats = stats.get("features", {})
            cpu = feats.get("avg_cpu", 0.0)
            bucket = int(cpu // 10) * 10
            cpu_buckets[bucket] += 1

        self.heatmap_text.insert("end", "\nCPU Activity Buckets:\n")
        for b in sorted(cpu_buckets.keys()):
            count = cpu_buckets[b]
            bar = "#" * min(count, 40)
            self.heatmap_text.insert("end", f"  {b:02d}-{b+9:02d}% : {bar} ({count})\n")

        conn_buckets = defaultdict(int)
        for pid, stats in BEHAVIOR_DB.items():
            feats = stats.get("features", {})
            conn = feats.get("avg_conn", 0.0)
            bucket = int(conn // 5) * 5
            conn_buckets[bucket] += 1

        self.heatmap_text.insert("end", "\nConnection Count Buckets:\n")
        for b in sorted(conn_buckets.keys()):
            count = conn_buckets[b]
            bar = "#" * min(count, 40)
            self.heatmap_text.insert("end", f"  {b:02d}-{b+4:02d} conns : {bar} ({count})\n")

        self.heatmap_text.insert("end", "\nThreat Timeline Samples:\n")
        for pid, timeline in list(THREAT_TIMELINES.items())[:5]:
            self.heatmap_text.insert("end", f"  PID {pid}:\n")
            for ev in list(timeline)[-3:]:
                self.heatmap_text.insert("end", f"    {ev}\n")

    def schedule_log_refresh(self):
        self.refresh_log_box()
        self.refresh_ui()
        self.refresh_god_view()
    ...
    def toggle_ml_model(self):
        with STATE_LOCK:
            STATE["ml_model_enabled"] = self.var_ml_model.get()
        save_config()
        write_log(f"[COCKPIT] ML model usage set to {STATE['ml_model_enabled']} (design stub).")
        self.refresh_ui()

    def toggle_god_view(self):
        with STATE_LOCK:
            STATE["god_view_enabled"] = not STATE["god_view_enabled"]
        save_config()
        write_log(f"[COCKPIT] God-View set to {STATE['god_view_enabled']}.")
        self.refresh_ui()
        self.refresh_god_view()
    ...
    def build_ml_dataset(self):
        MLDATASET_BUILDER = ML_DATASET_BUILDER
        MLDATASET_BUILDER.build_dataset()
        self.refresh_log_box()

    def reload_ml_model(self):
        OFFLINE_MODEL.load_model()
        self.refresh_log_box()

# ============================================================
# 21. STEALTH MODE ENTRY
# ============================================================
