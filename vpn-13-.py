...
"""
Borg Hybrid Overmind VPN Daemon (System + Browser VPN, Multi‑Signal Detection)
=============================================================================
...
NEW: Multi-signal browser VPN detection:
...
# Known DoH endpoints (for DNS behavior hints)
DOH_HOSTS = {
    "cloudflare-dns.com",
...
def _peek_tls_client_hello(sock, max_bytes=512):
    ...
    try:
        ...
        if not data:
            return None
        # Very rough: hash first 64 bytes as "fingerprint"
        fp = hash(data[:64])
...
class ProxyHandler(socketserver.StreamRequestHandler):
    ...
    def handle_connect(self, host, port, domain):
        ...
        # Peek TLS ClientHello from client side to fingerprint
...
class BorgGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Borg VPN Overmind - System + Browser VPN (Multi-Signal)")
    ...
    def _update_vpn_status(self):
        ...
        any_active = sys_active or browser_active
        server = VPN.get_current_vpn_server_name()
        ip = VPN.get_current_exit_ip()
...
def main():
    log("=== Borg VPN Overmind starting (System + Browser VPN, Multi-Signal Detection) ===")
