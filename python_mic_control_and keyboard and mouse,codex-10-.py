...
# ============================================================
# 9. MEMORY FORENSICS & DLL INJECTION / HOLLOWING HEURISTICS
# ============================================================
        ...
        for m in proc.memory_maps():
            ...
            if path.endswith(".dll"):
                if any(marker in path for marker in SUSPICIOUS_DLL_MARKERS):
                    return True
                if any(p in path for p in ["\\temp\\", "\\appdata\\local\\temp\\", "\\users\\"]):
                    return True
    except:
        return False
    return False
...
def detect_process_hollowing(proc: psutil.Process) -> bool:
    ...
    try:
        ...
        for m in proc.memory_maps():
            ...
            if not path:
                continue
            if path.endswith(".exe") and path != exe:
                if any(p in path for p in ["\\temp\\", "\\appdata\\local\\temp\\", "\\users\\"]):
                    suspicious_images += 1
...
def detect_debugging(proc: psutil.Process) -> bool:
    """
    User-mode heuristic: check for typical debugger processes and
    suspicious parent/child relationships.
    """
    debugger_names = [
        "ollydbg.exe", "x64dbg.exe", "ida.exe", "ida64.exe",
        "windbg.exe", "gdb.exe", "immunitydebugger.exe"
    ]
    try:
        parent = proc.parent()
...
def detect_vm_environment() -> bool:
    """
    Heuristic: check for common VM strings in system info.
    """
    try:
...
# ============================================================
# 12. PROCESS MONITORING & AI-STYLE ANOMALY MODEL
# ============================================================
...
def process_behavior_profile(proc: psutil.Process) -> dict:
    ...
    # AI-style scoring: combine multiple signals into a single score
    score = 0.0
    score += min(avg_cpu / 10.0, 5.0)          # CPU weight
    score += min(avg_conn / 2.0, 5.0)          # connections weight
    if net["suspicious"]:
        score += 5.0
    if net["blocked_hit"]:
        score += 10.0
    ...
    if scan_memory_maps_for_injection(proc):
        score += 8.0
    if scan_memory_for_suspicious_strings(proc):
        score += 4.0
    if detect_process_hollowing(proc):
        score += 8.0

    if detect_debugging(proc):
        score += 3.0

    if detect_vm_environment():
        # Not necessarily malicious, but raise sensitivity
        score += 1.0

    stats["ai_score"] = score
    stats["suspicious"] = score >= 10.0
...
# ============================================================
# 16. CODEX COCKPIT GUI
# ============================================================
class CodexCockpitGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Codex Input Firewall — Apex Cockpit")
