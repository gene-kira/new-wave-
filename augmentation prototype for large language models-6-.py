...
def auto_quant_mode(sys_tel):
    # If GPU memory is high, go more aggressive (int8)
    if sys_tel["gpu_mem_pct"] > 80 or sys_tel["gpu_util"] > 90:
        return "int8"
    return "fp8"


def auto_tile_size(sys_tel):
    # If GPU is very busy, use larger tiles to reduce overhead
...
def adaptive_skip_scale(sys_tel):
    # Scale skip threshold based on load, memory, and temperature
    load_factor = 1.0 + (sys_tel["gpu_util"] / 100.0) * 0.5
    mem_factor = 1.0 + (sys_tel["gpu_mem_pct"] / 100.0) * 0.5
    temp_factor = 1.0
    if sys_tel["gpu_temp"] > 60:
        temp_factor += (sys_tel["gpu_temp"] - 60) / 40.0
...
def gui_tkinter():
    ...
    info_label = tk.Label(
        top_frame,
        text=f"CUDA: {HAS_CUDA}, GPUs: {NUM_GPUS}",
        font=("Consolas", 11)
    )
    ...
    def update_status():
        ...
        status_var.set(f"Model: {CURRENT_MODEL_NAME}  |  Mode: {mode}  |  QUANT_MODE: {QUANT_MODE}  |  TILE: {TILE_ROWS}x{TILE_COLS}")
    ...
    def on_gen():
        ...
        def run_gen():
            try:
                text, stats, layer_stats = generate_with_forklift(prompt, max_new_tokens=128, collect_router_data=True)
        ...
        with MULTINODE_LOCK:
            for nid, tdata in MULTINODE_TELEMETRY.items():
                multinode_box.insert(
                    tk.END,
                    f"{nid}: GPU {tdata.get('gpu_util', 0):5.1f}%, "
