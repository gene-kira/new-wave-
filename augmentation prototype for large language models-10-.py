...
# GPU-resident cache toggle
GPU_TILE_CACHE = True if HAS_CUDA else False
...
# =========================
# Distributed tile cache hints + prefetch hooks
# =========================
...
def find_remote_nodes_with_tile(key_str: str):
    now = time.time()
    candidates = []
    with DISTRIBUTED_CACHE_LOCK:
        for nid, tiles in DISTRIBUTED_TILE_HINTS.items():
            ts = tiles.get(key_str, None)
            if ts is not None and now - ts < 30.0:
                candidates.append(nid)
    return candidates


# Simple TCP-based tile RPC (request/response)
TILE_RPC_PORT = 6001

def tile_rpc_server_loop(cache_ref):
    srv = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    srv.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    srv.bind(("0.0.0.0", TILE_RPC_PORT))
    srv.listen(8)
    print(f"[TileRPC] Listening on 0.0.0.0:{TILE_RPC_PORT}")
    while True:
...
def request_tile_from_remote(key):
    key_str = tile_key_to_str(key)
    nodes = find_remote_nodes_with_tile(key_str)
    if not nodes:
        return None
    # naive: just try localhost broadcast peers by hostname (assuming same subnet)
    # in real deployment you'd have node->ip mapping
    for nid in nodes:
        ...
        try:
            ...
            if resp.get("ok", False):
                import numpy as np
                q_tile_np = np.array(resp["q_tile"], dtype="int8")
                scale_np = np.array(resp["scale"], dtype="float32")
                q_tile = torch.from_numpy(q_tile_np).to(DEFAULT_DEVICE)
                scale = torch.from_numpy(scale_np).to(DEFAULT_DEVICE)
                return q_tile, scale
        except Exception:
            continue
        finally:
...
# =========================
# FP8 bit-packing helpers (E4M3/E5M2 style emulation)
# =========================

def float_to_fp8_e4m3(x: torch.Tensor):
    # simple emulation: clamp exponent range and quantize mantissa
    # not bit-perfect, but closer to real FP8 than naive int8
    x = x.clamp(-448.0, 448.0)  # rough FP8 E4M3 dynamic range
    scale = x.abs().amax(dim=1, keepdim=True).clamp(min=1e-8) / 240.0
    q = torch.round(x / scale).clamp(-240, 240)
    return q.to(torch.int16), scale.squeeze(1)


def fp8_e4m3_to_float(q: torch.Tensor, scale: torch.Tensor):
    return q.to(torch.float32) * scale.unsqueeze(1)


def float_to_fp8_e5m2(x: torch.Tensor):
    x = x.clamp(-57344.0, 57344.0)  # rough FP8 E5M2 dynamic range
    scale = x.abs().amax(dim=1, keepdim=True).clamp(min=1e-8) / 240.0
    q = torch.round(x / scale).clamp(-240, 240)
    return q.to(torch.int16), scale.squeeze(1)


def fp8_e5m2_to_float(q: torch.Tensor, scale: torch.Tensor):
    return q.to(torch.float32) * scale.unsqueeze(1)
...
def quantize_per_channel_fp8(tile: torch.Tensor, mode="e4m3"):
    if tile.numel() == 0:
        scale = torch.ones(tile.size(0), device=tile.device, dtype=torch.float32)
        q = torch.zeros_like(tile, dtype=torch.int16)
        return q, scale
    flat = tile
    if mode == "e4m3":
        q, scale = float_to_fp8_e4m3(flat)
    else:
        q, scale = float_to_fp8_e5m2(flat)
    return q, scale


def dequantize_per_channel_fp8(q_tile: torch.Tensor, scale: torch.Tensor, mode="e4m3"):
    if mode == "e4m3":
        return fp8_e4m3_to_float(q_tile, scale)
    else:
        return fp8_e5m2_to_float(q_tile, scale)
...
# =========================
# Triton matmul + quant kernels (optional)
# =========================
...
# =========================
# KV-cache compression + fused op stub
# =========================
...
class KVCacheManager:
    ...
    def flashattention_fused(self, q, k, v, mask=None):
        """
        FlashAttention-style fused KV op stub.
        In a real implementation, this would call a CUDA kernel.
        Here we just do standard attention but keep the API.
        """
        d = q.size(-1)
...
# =========================
# Tile importance router (batched)
# =========================
...
# =========================
# Forklift executor with batched router inference
# =========================
...
class ForkliftExecutor:
    ...
    def _batch_router_scores(self, feats_list: List[torch.Tensor]):
        if not feats_list:
            return []
        feats = torch.stack(feats_list, dim=0).to(DEFAULT_DEVICE)
        with torch.no_grad():
            scores = self.router(feats)
    ...
    def linear(self, layer_name: str, weight: torch.Tensor, bias: torch.Tensor, x: torch.Tensor, layer_depth: int = 0):
        ...
        # First pass: collect features for all tiles
        tile_meta = []
        feats_list = []
        for tr, tc in compute_tile_indices(W.shape, TILE_ROWS, TILE_COLS):
            ...
            tile_meta.append((tr, tc, rs, cs, X_sub, norm))
            feats_list.append(feats)

        # Batched router inference
        scores = self._batch_router_scores(feats_list)

        # Second pass: apply decisions
        for idx, (tr, tc, rs, cs, X_sub, norm) in enumerate(tile_meta):
            ...
            if cached is None:
                # try remote prefetch
                remote = request_tile_from_remote(key)
                if remote is not None:
                    q_tile, scale = remote
                else:
                    tile = W[rs, cs].contiguous()
                    if GPU_TILE_CACHE and tile.device.type == "cuda":
                        tile = tile.to(DEFAULT_DEVICE)
            ...
            if tile.device != X_sub.device:
                tile = tile.to(X_sub.device)
...
class CUDAGraphWrapper:
    ...
    def capture(self, example_inputs):
        ...
        with torch.cuda.stream(stream):
            static_inputs = {k: v.clone().detach() for k, v in example_inputs.items()}
            for v in static_inputs.values():
                v.requires_grad = False
            g = torch.cuda.CUDAGraph()
            torch.cuda.synchronize()
            with torch.cuda.graph(g):
                out = self.model(**static_inputs)
        torch.cuda.synchronize()
        self.graph = g
    ...
    def run(self, inputs):
        ...
        for k in self.static_inputs.keys():
            self.static_inputs[k].copy_(inputs[k])
        self.graph.replay()
...
def generate_with_forklift(prompt: str, max_new_tokens: int = 64, collect_router_data=False):
    ...
    global CUDA_GRAPH_WRAPPER
    if CUDA_GRAPH_WRAPPER is None and HAS_CUDA:
        CUDA_GRAPH_WRAPPER = CUDAGraphWrapper(model)
        try:
            CUDA_GRAPH_WRAPPER.capture(inputs)
        except Exception:
    ...
    with torch.no_grad():
        if hasattr(model, "generate"):
            # CUDA graphs don't play nicely with HF generate, so we use normal path here
            ...
            if CUDA_GRAPH_WRAPPER is not None:
                out = CUDA_GRAPH_WRAPPER.run(inputs)
            else:
                out = model(**inputs)
            logits = out.logits
...
def train_router_on_collected_data(steps=ROUTER_TRAIN_STEPS):
    ...
    for step in range(steps):
        ...
        xb = X[idx].to(DEFAULT_DEVICE)
        yb = y[idx].to(DEFAULT_DEVICE)
...
# =========================
# Router distillation from teacher model (stub)
# =========================

def distill_router_from_teacher(teacher_model, prompts: List[str], max_new_tokens=32):
    """
    High-level idea:
    - Run teacher model with full dense matmuls (no skipping).
    - Measure per-tile contribution norms.
    - Train router to match teacher's tile importance.
    This is a stub entry point; full implementation would require
    instrumenting teacher forward with tile hooks.
    """
    print("[Distill] Router distillation stub called (not fully implemented).")
    # You can extend this by wrapping teacher with a similar ForkliftExecutor
    # in "observation" mode and using its contrib norms as labels.
...
# =========================
# Benchmark vs HF baseline
# =========================

def benchmark_vs_hf_baseline(prompt: str, max_new_tokens: int = 64, runs: int = 5):
    """
    Compare:
    - HF baseline (no Forklift wrapping)
    - Forklift engine
    """
    print("[Benchmark] Loading baseline model...")
    baseline_tok = AutoTokenizer.from_pretrained(PRIMARY_MODEL_NAME, use_fast=True)
    ...
    def run_baseline():
        inputs = baseline_tok(prompt, return_tensors="pt")
        if HAS_CUDA:
            first_device = next(baseline_model.parameters()).device
            inputs = {k: v.to(first_device) for k, v in inputs.items()}
        start = time.time()
        with torch.no_grad():
            out_ids = baseline_model.generate(
                **inputs,
                max_new_tokens=max_new_tokens,
                do_sample=False,
                use_cache=True,
    ...
    print("[Benchmark] Running baseline...")
    baseline_times = [run_baseline() for _ in range(runs)]
    baseline_avg = sum(baseline_times) / len(baseline_times)

    print("[Benchmark] Running Forklift...")
    forklift_times = []
    forklift_stats = None
    forklift_text = None
    for _ in range(runs):
        start = time.time()
        stats, text = run_forklift()
        end = time.time()
        forklift_times.append((end - start) * 1000.0)
        forklift_stats = stats
        forklift_text = text
    forklift_avg = sum(forklift_times) / len(forklift_times)

    print("\n=== Benchmark Results ===")
    print(f"Baseline avg latency:  {baseline_avg:.2f} ms")
    print(f"Forklift avg latency:  {forklift_avg:.2f} ms")
    print(f"Speedup:               {baseline_avg / forklift_avg:.2f}x")
    print("\nForklift stats:")
    for k, v in forklift_stats.items():
        print(f"  {k}: {v}")
    print("\nSample output:")
    print(forklift_text)
...
def gui_tkinter():
    ...
    # Tile importance heatmap (simple textual)
    heatmap_frame = tk.LabelFrame(right_frame, text="Tile Importance Heatmap (per-layer)", padx=5, pady=5)
    heatmap_frame.pack(fill=tk.BOTH, expand=False, pady=(5, 0))
    heatmap_box = scrolledtext.ScrolledText(heatmap_frame, height=6)
    heatmap_box.pack(fill=tk.BOTH, expand=True)
    ...
    def heatmap_log(text):
        heatmap_box.insert(tk.END, text + "\n")
        heatmap_box.see(tk.END)
        ...
        def run_gen():
            try:
                ...
                for name, flops, bytes_moved, skipped, fpb, skip_ratio in layer_stats[:30]:
                    ...
                    # simple heatmap: more skipped -> more '#'
                    bar_len = int(skip_ratio * 40)
                    heatmap_log(f"{name[:40]:40s} | " + "#" * bar_len + "-" * (40 - bar_len))
            except Exception as e:
...
if __name__ == "__main__":
    ...
    if "--headless-node" in sys.argv:
        ...
        benchmark_vs_hf_baseline("The future of AI acceleration is", max_new_tokens=64, runs=3)
    else:
        gui_tkinter()
