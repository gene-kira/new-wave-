...
# -*- coding: utf-8 -*-
"""
AI Security Bridge (GUI-first + Auto-elevation + Auto WAN/LAN detect + Auto-start bridge + Crash-proof subsystems + Diagnostics)
...
- Auto-detect WAN vs LAN mode using interface IP ranges
- Auto-select WAN_IN, WAN_OUT, LAN_IN, LAN_OUT (force mode fallback)
...
WAN_IN_IFACE = None    # From ISP / modem
WAN_OUT_IFACE = None   # To UDM Pro WAN port

LAN_IN_IFACE = None    # From main system
LAN_OUT_IFACE = None   # To UDM Pro LAN
...
def run_diagnostics_once():
    # Npcap presence (best-effort)
    try:
        # crude check: look for npcap directory
        npcap_dir = os.path.join(os.environ.get("ProgramFiles", "C:\\Program Files"), "Npcap")
        if os.path.isdir(npcap_dir):
            DIAG_STATE["npcap_present"] = "yes"
        else:
            DIAG_STATE["npcap_present"] = "maybe"
    except Exception as e:
        DIAG_STATE["npcap_present"] = "unknown"
        diag_add_error(f"Npcap check error: {e}")

    # Raw socket capability test
    try:
        s = socket(AF_INET, SOCK_STREAM)
        s.close()
    ...
    # Promiscuous mode test is heuristic here
    DIAG_STATE["promisc_ok"] = "maybe"
...
class AISecurityBridgeApp:
    ...
    def build_fw_tab(self):
        lbl = tk.Label(
            self.tab_fw,
            text=(
                "Firewall Engine: DPI (HTTP + TLS + signatures), deception, persona, ad-block.\n"
    ...
    def build_diag_tab(self):
        ...
        self.diag_npcap_label = tk.Label(top, text="Npcap: unknown")
        self.diag_npcap_label.pack(side="left", padx=5)
    ...
    def refresh_diag(self):
        ...
        self.diag_npcap_label.config(text=f"Npcap: {state.get('npcap_present', 'unknown')}")
