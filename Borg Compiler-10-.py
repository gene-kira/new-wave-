...
GLOBAL_CONFIG = {
    ...
    "router_ckpt": os.environ.get("BORG_ROUTER_CKPT", "router_net_v83.pt"),
}
...
class CodeGenerator:
    ...
    # ---------- AST transforms (same as v8.2, shortened comments) ----------
    ...
    def ml_ranking(self, tree: ast.AST) -> ast.AST:
        ...
        comment = ast.Expr(
            value=ast.Constant(
                value=f"# ML router ranking active (v8.3 RL): {summary}, model={stats.get('router_model')}"
            )
        )
        if isinstance(tree, ast.Module):
            tree.body.insert(0, comment)
...
class BorgCompilerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Borg Compiler v8.3 - RL Router + Autopilot")
...
class ForkliftExecutor:
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        out["router_model"] = "RouterNet-MLP-v8.3" if self.router_net is not None else "none"
...
def self_diagnostic_report(args):
    print("\n[Diagnostics] Borg v8.3 self-check")
...
def safe_argparse():
    parser = argparse.ArgumentParser(
        description="Borg Compiler v8.3 + Forklift Node - Universal Autopilot"
    )
