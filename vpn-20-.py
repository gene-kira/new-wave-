...
def _fetch_ipinfo(ip):
    """
    ipinfo with 429 backoff + simple cache.
    """
    ...
    try:
        with IPINFO_CACHE_LOCK:
            ...
            if now - IPINFO_CACHE["last_429_time"] < IPINFO_CACHE["backoff_seconds"]:
                # Still in backoff window, return cached or empty
                if IPINFO_CACHE["last_ip"] == ip and IPINFO_CACHE["last_data"]:
                    return dict(IPINFO_CACHE["last_data"])
                return {}
...
def register_tls_fingerprint(domain, fp):
    ...
    with PUBLIC_IP_LOCK:
        ...
        if old is None:
            ...
            fps[domain] = fp
    # Also persist per-domain in site store for replay
    if domain:
...
def _is_vpn_asn(asn, isp):
    """
    Fixed: normalize ASN/ISP to strings before .startswith / .lower.
    """
    if asn is None:
        asn = ""
    else:
...
def _prewarm_exit_ip(exit_ip, timeout=2.0):
    """
    Pre-warm socket by opening a dummy TCP connection to exit_ip:443.
    No TLS handshake, just connect+close.
    """
...
def _parse_set_cookie_headers(raw_response_bytes):
    """
    Very simple Set-Cookie parser: looks for 'Set-Cookie:' lines in the raw response.
    """
    try:
...
class ProxyHandler(socketserver.StreamRequestHandler):
    ...
    def _apply_fingerprint_headers(self, domain, headers_list):
        ...
        # Session continuity: if no Cookie header, inject stored cookies from VPN session
    ...
    def handle_connect(self, host, port, domain):
        # Ghost tunnel pre-warm: if we have cached exit IP for this domain, prewarm
    ...
    def handle_http(self, method, target, headers, raw_headers, domain):
        ...
        # Ghost tunnel pre-warm for HTTP as well
        profile = SITE_STORE.get_site(domain) if domain else None
        if profile:
            exit_ip = profile.get("last_exit_ip")
            if exit_ip:
        ...
        # Capture response to extract Set-Cookie for session continuity
            ...
            try:
                while True:
                    ...
                    if direction == "in" and dom:
                        # accumulate a limited amount of response for cookie parsing
                        if len(buf) < 65536:
                            buf += data
            except Exception:
                pass
...
class BorgGUI:
    ...
    def _update_domains_table(self):
        for row in self.domains_tree.get_children():
            self.domains_tree.delete(row)
...
def main():
    log("=== Borg VPN Overmind starting (GPU Threat + Swarm Telemetry + LLM Anomaly + RT Browser VPN Saving + Ghost Tunnel) ===")
