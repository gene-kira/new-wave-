...
# psutil (critical but may be missing on fresh systems)
try:
    import psutil
except ImportError:
    psutil = None
...
class WatchdogOrgan:
    ...
    def check_processes(self):
        if psutil is None:
            log_info("[WATCHDOG] psutil not available, skipping process checks.")
            return
        for role, pid in list(self.processes.items()):
            try:
                ...
                log_error(f"[WATCHDOG] Error checking pid {pid}: {e}")
    ...
    def do_POST(self):
        ...
        if self.path == "/jobs/submit":
            ...
            try:
                cur = DB.execute(
                    "INSERT INTO jobs (created_ts, status, model, input, output, target_node) VALUES (?, ?, ?, ?, ?, ?)",
                    (datetime.utcnow().isoformat(), "queued", model, text, None, None)
                )
                DB.commit()
