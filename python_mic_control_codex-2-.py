...
ensure_admin()
print("[Codex] Mic Control starting...")

# ============================================================
# Tier‑4 Codex Autoloader (GUI-safe)
# ============================================================
...
try:
    ...
    print("[Codex] Critical: PyQt5 or psutil not available.")
    print("Run:  python -m pip install pyqt5 psutil")
    sys.exit(1)
    ...
    def run(self):
        while True:
            try:
                ...
                self.log_signal.emit(write_log(f"[WATCHDOG ERROR] {e}"))
                time.sleep(2)
    ...
    def run(self):
        while True:
            try:
                ...
                self.log_signal.emit(write_log(f"[DEVICE-INTERCEPTOR ERROR] {e}"))
                time.sleep(1.0)
    ...
    def __init__(self):
        ...
        if os.path.exists(LOG_FILE):
            try:
                ...
                self.log_box.append(f"[Log Load Error] {e}")
