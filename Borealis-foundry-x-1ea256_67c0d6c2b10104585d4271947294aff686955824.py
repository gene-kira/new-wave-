...
# Name: Borealis-foundry-x-1ea256
# Constraints: self_healing, rapid_deploy, silent, fail_safe, modular, transparent
# Params: scale=51.3481, power_budget=0.9561, tol=0.003, mutation_rate=0.8
# Scores: final=0.8019 novelty=0.7983 utility=0.9777 impact=0.7000 curiosity=0.4861
...
class Config:
    def __init__(self):
        self.name = 'Borealis-foundry-x-1ea256'
...
def main():
    ...
    p = Parameters(scale=51.3481, power_budget=0.9561, tolerance=0.003, mutation_rate=0.8)
    errs = validate_params(p)
    if errs:
        logging.error('Validation errors: %s', errs); sys.exit(1)
    core = ProjectCore(p, 'materials_weave-quantized_transport-orbital_kite-damping', ['self_healing', 'rapid_deploy', 'silent', 'fail_safe', 'modular', 'transparent'], 'damping')
