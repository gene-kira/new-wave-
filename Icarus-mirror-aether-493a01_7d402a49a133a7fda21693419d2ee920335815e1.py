...
# Name: Icarus-mirror-aether-493a01
# Constraints: zero_emissions, modular, self_healing, scalable, fail_safe, low_mass, transparent
# Params: scale=50.476, power_budget=0.1, tol=0.003, mutation_rate=0.8
# Scores: final=0.7607 novelty=0.7272 utility=0.6741 impact=1.0000 curiosity=0.5311
...
class Config:
    def __init__(self):
        self.name = 'Icarus-mirror-aether-493a01'
...
def main():
    ...
    p = Parameters(scale=50.476, power_budget=0.1, tolerance=0.003, mutation_rate=0.8)
    errs = validate_params(p)
    if errs:
        logging.error('Validation errors: %s', errs); sys.exit(1)
    core = ProjectCore(p, 'atmosphere-gradient_control-beam_array-sensing', ['zero_emissions', 'modular', 'self_healing', 'scalable', 'fail_safe', 'low_mass', 'transparent'], 'sensing')
