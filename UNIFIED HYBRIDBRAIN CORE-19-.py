#!/usr/bin/env python3
"""
HybridBrain v8.0 – True Hybrid (v6.3 + v7.0 merged)
...
Monolithic, OS-adaptive, swarm-ready AI kernel with:
  ...
  - Swarm coordination (TCP JSON, UDP multicast discovery, topology learning, ZeroMQ bus stub)
...
# ============================================================
# GPU PREDICTION ORGAN (stubbed NN)
# ============================================================
...
class SwarmTopology:
    ...
    def update_link(self, src: str, dst: str, latency: float, success: bool):
        key = (src, dst)
        info = self.links.get(key, {"latency_ewma": latency, "success_rate": 0.5, "count": 0})
        alpha = 0.3
        info["latency_ewma"] = (1 - alpha) * info["latency_ewma"] + alpha * latency
        info["count"] += 1
        sr = info["success_rate"]
        sr = (1 - alpha) * sr + alpha * (1.0 if success else 0.0)
        info["success_rate"] = sr
        self.links[key] = info

    def best_neighbors(self, node_id: str, top_k: int = 3) -> List[Tuple[str, float, float]]:
        candidates = []
        for (src, dst), info in self.links.items():
            if src == node_id:
                candidates.append((dst, info["latency_ewma"], info["success_rate"]))
    ...
    def snapshot(self) -> Dict[str, Any]:
        ...
        for (src, dst), info in self.links.items():
            out.append({
                "src": src,
                "dst": dst,
                "latency_ewma": info["latency_ewma"],
...
# ============================================================
# SWARM COORDINATOR (planner/executor)
# ============================================================

class SwarmCoordinator:
    def __init__(self, core: "HybridBrainCore", node_id: str, zmq_endpoint: str = "tcp://*:9100"):
        self.core = core
        self.node_id = node_id
...
class HybridBrainCore:
    def __init__(self, cfg_dict: Optional[Dict[str, Any]] = None):
        ...
        self.swarm_coordinator = SwarmCoordinator(self, self.node_id)
    ...
    def cognition_step(self) -> OrganOutput:
        ...
        out.reasoning_tail["stance"] = stance
        out.reasoning_tail["meta_state"] = meta_state.name
        out.reasoning_tail["risk_score"] = risk.risk_score
        out.reasoning_tail["potential"] = potential
...
def launch_tk_gui(core: HybridBrainCore):
    ...
    root = tk.Tk()
    root.title("HybridBrain v8.0 – True Hybrid")
...
def parse_args(argv: List[str]) -> Dict[str, Any]:
    parser = argparse.ArgumentParser(description="HybridBrain v8.0 – True Hybrid")
...
def main():
    ...
    if args["swarm_executor"]:
        ...
        try:
            core.swarm_coordinator.autonomous_loop(interval=args["planner_interval"])
        except KeyboardInterrupt:
            pass
    else:
        if args["gui"] and launch_tk_gui(core):
            return
        if args["web"]:
            launch_web_gui(core, host=args["web_host"], port=args["web_port"])
        else:
            run_headless(core)
