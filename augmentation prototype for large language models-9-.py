...
# Cross-node tile RPC
TILE_RPC_PORT = 6001  # separate from text RPC
TILE_RPC_TIMEOUT = 0.2  # seconds
...
def tile_key_from_str(s: str):
    ...
    tc = int(parts[2])
    mode = parts[3]
    tile_rows = int(parts[4])
    tile_cols = int(parts[5])
    return (layer_name, tr, tc, mode, tile_rows, tile_cols)
...
def _tile_rpc_server_loop(cache_ref, host="0.0.0.0", port=TILE_RPC_PORT):
    """
    Simple JSON-over-TCP tile server:
    - Client sends: {"key": "layer|tr|tc|mode|rows|cols"}
    - Server responds: {"ok": true, "q_tile": [...], "scale": [...], "shape": [m,n]} or {"ok": false}
    """
...
def _handle_tile_rpc_client(conn, addr, cache_ref):
    try:
        ...
        resp = {
            ...
            "q_tile": q_tile.cpu().tolist(),
            "scale": scale.cpu().tolist(),
        }
        conn.sendall((json.dumps(resp) + "\n").encode())
    except Exception:
        try:
            conn.sendall(json.dumps({"ok": False, "error": "exception"}).encode())
        except Exception:
            pass
    finally:
        conn.close()
...
def try_fetch_tile_from_remote(key) -> Optional[Tuple[torch.Tensor, torch.Tensor]]:
    """
    Try to fetch a quantized tile from any node that claims to have it.
    """
    key_str = tile_key_to_str(key)
    now = time.time()
    with DISTRIBUTED_CACHE_LOCK:
        hints = dict(DISTRIBUTED_TILE_HINTS)

    # naive: try all nodes that claim this tile, newest first
    candidates = []
    for nid, tiles in hints.items():
        ts = tiles.get(key_str)
        if ts is not None and now - ts < 30.0:  # 30s freshness
            candidates.append((ts, nid))
    candidates.sort(reverse=True)

    for _, nid in candidates:
        # assume node hostname is encoded in NODE_ID-like string; here we just use nid as host
        host = nid.replace("node-", "")
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.settimeout(TILE_RPC_TIMEOUT)
            s.connect((host, TILE_RPC_PORT))
            req = {"key": key_str}
            ...
            if not resp.get("ok", False):
                continue
            q_tile_list = resp["q_tile"]
            scale_list = resp["scale"]
            q_tile = torch.tensor(q_tile_list, dtype=torch.int8, device=DEFAULT_DEVICE)
            scale = torch.tensor(scale_list, dtype=torch.float32, device=DEFAULT_DEVICE)
            return q_tile, scale
        except Exception:
            continue
        finally:
...
# =========================
# Real FP8 quantization helpers (E4M3-style approximation)
# =========================

def _fp32_to_e4m3(x: torch.Tensor) -> torch.Tensor:
    """
    Approximate FP8 E4M3 by clamping exponent/mantissa ranges.
    We emulate via scaling + clamping, not bit-accurate, but closer than plain int8.
    """
    # avoid log(0)
    eps = 1e-8
    x_clamped = torch.clamp(x, -448.0, 448.0)  # rough E4M3 dynamic range
    # scale to roughly use 3 mantissa bits -> quantization step ~ 1/8
    scale = 8.0
    q = torch.round(x_clamped * scale)
    q = torch.clamp(q, -448 * scale, 448 * scale)
    return q / scale


def _fp32_to_e5m2(x: torch.Tensor) -> torch.Tensor:
    """
    Approximate FP8 E5M2: wider range, fewer mantissa bits.
    """
    x_clamped = torch.clamp(x, -57344.0, 57344.0)
    scale = 4.0
    q = torch.round(x_clamped * scale)
    q = torch.clamp(q, -57344 * scale, 57344 * scale)
    return q / scale


def quantize_per_channel_fp8(tile: torch.Tensor, mode: str = "e4m3"):
    """
    Per-channel FP8-ish quantization:
    - compute per-row scale
    - apply E4M3/E5M2-like rounding
...
# =========================
# Tile importance router (optimized)
# =========================
...
class ForkliftExecutor:
    def __init__(self, cache: TileCache, router: TileRouter):
        ...
        # for heatmap: per-layer tile scores (2D grid)
        self.tile_scores: Dict[str, Dict[Tuple[int, int], float]] = {}
    ...
    def linear(self, layer_name: str, weight: torch.Tensor, bias: torch.Tensor, x: torch.Tensor, layer_depth: int = 0):
        ...
        # per-layer tile score map
        if layer_name not in self.tile_scores:
            self.tile_scores[layer_name] = {}
        ...
        for tr, tc in compute_tile_indices(W.shape, TILE_ROWS, TILE_COLS):
            ...
            self.tile_scores[layer_name][(tr, tc)] = score
            ...
            if cached is None:
                # try remote fetch before local quantization
                remote = try_fetch_tile_from_remote(key)
    ...
    def get_tile_heatmap_for_layer(self, layer_name: str):
        """
        Returns a 2D grid (list of lists) of scores for the given layer,
        with shape [num_tr][num_tc], missing tiles filled with 0.
        """
        if layer_name not in self.tile_scores:
            return []
        scores = self.tile_scores[layer_name]
        if not scores:
            return []
        max_tr = max(tr for (tr, tc) in scores.keys())
        max_tc = max(tc for (tr, tc) in scores.keys())
        grid = []
        for tr in range(max_tr + 1):
            row = []
            for tc in range(max_tc + 1):
                row.append(scores.get((tr, tc), 0.0))
            grid.append(row)
        return grid
...
# =========================
# Router training curriculum (E)
# =========================

def train_router_curriculum():
    """
    Multi-phase curriculum:
    1) Warmup on all collected samples
    2) Hard-negative mining (tiles with high norm but low label)
    3) Refinement on high-confidence positives
    """
    X, y = EXECUTOR.get_router_dataset()
    if X is None or y is None:
        print("No router data collected; skipping curriculum.")
        return

    print("[Curriculum] Phase 1: Warmup on full dataset")
    _train_router_on_dataset(X, y, steps=ROUTER_TRAIN_STEPS // 2)

    with torch.no_grad():
        ROUTER.eval()
        preds = ROUTER(X)

    # Phase 2: hard negatives (pred high, label low)
    hard_mask = (preds > 0.5) & (y < 0.2)
    if hard_mask.any():
        X_hard = X[hard_mask]
        y_hard = y[hard_mask]
        print(f"[Curriculum] Phase 2: Hard negatives ({X_hard.size(0)} samples)")
        _train_router_on_dataset(X_hard, y_hard, steps=ROUTER_TRAIN_STEPS // 3)
    else:
        print("[Curriculum] Phase 2: No hard negatives found.")

    # Phase 3: high-confidence positives
    pos_mask = (y > 0.7)
    if pos_mask.any():
        X_pos = X[pos_mask]
        y_pos = y[pos_mask]
        print(f"[Curriculum] Phase 3: High-confidence positives ({X_pos.size(0)} samples)")
        _train_router_on_dataset(X_pos, y_pos, steps=ROUTER_TRAIN_STEPS // 3)
    else:
        print("[Curriculum] Phase 3: No high-confidence positives found.")

    print("[Curriculum] Router curriculum complete.")


def _train_router_on_dataset(X, y, steps: int):
    ROUTER.train()
    optimizer = torch.optim.Adam(ROUTER.parameters(), lr=ROUTER_LR)
    loss_fn = nn.MSELoss()
    dataset_size = X.size(0)
    ...
    for step in range(steps):
        ...
        if (step + 1) % max(1, steps // 4) == 0:
            print(f"[Curriculum] step {step+1}/{steps}, loss={loss.item():.6f}")
    ROUTER.eval()
...
def _handle_rpc_client(conn, addr):
    try:
        ...
        text, stats, _ = generate_with_forklift(prompt, max_new_tokens=max_new_tokens, collect_router_data=False)
...
# =========================
# Tkinter Cockpit (with router heatmap)
# =========================
...
def gui_tkinter():
    ...
    curriculum_btn = tk.Button(gen_btn_frame, text="Train Router Curriculum", width=22)
    curriculum_btn.pack(side=tk.LEFT, padx=2)
    ...
    # Router Visualizer (with heatmap)
    ...
    heatmap_frame = tk.Frame(router_frame)
    heatmap_frame.pack(fill=tk.X, pady=3)
    tk.Label(heatmap_frame, text="Heatmap Layer:").pack(side=tk.LEFT)
    heatmap_layer_var = tk.StringVar()
    heatmap_layer_entry = tk.Entry(heatmap_frame, textvariable=heatmap_layer_var, width=40)
    heatmap_layer_entry.pack(side=tk.LEFT, padx=5)
    heatmap_btn = tk.Button(heatmap_frame, text="Show Heatmap")
    heatmap_btn.pack(side=tk.LEFT, padx=2)
    ...
    def on_train_curriculum():
        log("Running router training curriculum...")
        root.update_idletasks()

        def run_curr():
            try:
                train_router_curriculum()
                log("Router curriculum complete.")
            except Exception as e:
                log(f"Router curriculum failed: {e}")

        t = threading.Thread(target=run_curr, daemon=True)
        t.start()

    def on_show_heatmap():
        layer_name = heatmap_layer_var.get().strip()
        router_box.delete("1.0", tk.END)
        if not layer_name:
            router_log("No layer name provided for heatmap.")
            return
        grid = EXECUTOR.get_tile_heatmap_for_layer(layer_name)
        if not grid:
            router_log(f"No heatmap data for layer '{layer_name}'.")
            return
        router_log(f"=== Tile Importance Heatmap for {layer_name} ===")
        # normalize scores for ASCII
        flat = [v for row in grid for v in row]
        if not flat:
            router_log("Empty heatmap.")
            return
        mn = min(flat)
        mx = max(flat)
        rng = max(mx - mn, 1e-6)
    ...
    curriculum_btn.config(command=on_train_curriculum)
    heatmap_btn.config(command=on_show_heatmap)
...
def run_swarm_node():
    ...
    tile_rpc_thread = threading.Thread(target=_tile_rpc_server_loop, args=(GLOBAL_CACHE,), daemon=True)
