...
GGUF_CONFIG = {
    ...
    "n_ctx": 4096,
    "n_gpu_layers_if_cuda": 35,
    "n_gpu_layers_if_cpu": 0,
    "n_threads": 8,
    "temperature": 0.7,
    "max_tokens": 128,
}
...
class ModelTierManager:
    ...
    def _init_gguf_llm(self):
        ...
        try:
            ...
            n_gpu_layers = GGUF_CONFIG["n_gpu_layers_if_cuda"] if RUNTIME.has_cuda else GGUF_CONFIG["n_gpu_layers_if_cpu"]
    ...
    def run_whisper(self, audio_path: str) -> Dict[str, Any]:
        ...
        if self.whisper_backend == "onnx" and self.whisper_onnx_session:
            ...
            # Real pipeline: load audio, compute mel, feed to ONNX.
            # Here we just demonstrate call structure.
            input_name = MODEL_PATHS["whisper_onnx_input_name"]
            dummy = [[0.0]]  # replace with real features
            outputs = self.whisper_onnx_session.run(None, {input_name: dummy})
