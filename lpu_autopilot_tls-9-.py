...
# -*- coding: utf-8 -*-
"""
LPU Autopilot Kernel - FULLY UNIFIED

Single-file autonomous kernel with:

- Autopilot core (state machine, safety)
- LPU (NLP → validated autopilot commands)
- Hardware abstraction layer + async hardware polling
- GPU-ready local LLM backend (llama.cpp hook)
- Whisper offline STT (lazy, optional, crash-safe)
- PySide6 GUI (control, logs, telemetry, metrics, swarm GPS map, 3D view)
- Voice control (Whisper + optional Google STT + TTS)
- Plugin system with sandbox isolation + hot-reload
- Async distributed event bus (TCP pub/sub)
- Swarm visualization on GPS map (with optional map tiles)
- TLS + JWT IPC server (using auto-cert engine with SAN)
- OTA update hooks (real server endpoint)
- REST API (async, aiohttp-guarded)
- MQTT bridge
- Matrix bridge (optional, nio-based)
- Multi-node swarm peers propagation
"""
...
MATRIX_USER_ID = "@lpu-bot:example.org"
MATRIX_ACCESS_TOKEN = "YOUR_ACCESS_TOKEN"
MATRIX_STORE_PATH = str(BASE_DIR / "matrix_store")
MATRIX_ROOM_COMMANDS_REAL = "!realroom:example.org"
MATRIX_ROOM_COMMANDS_SIM = "!simroom:example.org"
MATRIX_ROOM_TELEMETRY_REAL = "!realtele:example.org"
MATRIX_ROOM_TELEMETRY_SIM = "!simtele:example.org"
MATRIX_ROOM_ALERTS = "!alerts:example.org"

# =============================================================================
# Ensure working directory & directories
# =============================================================================
...
try:
    ...
    pass
try:
    nio = importlib.import_module("nio")
except Exception:
    nio = None
try:
...
MATRIX_ENABLED = CONFIG.get("matrix_enabled", False)
MATRIX_E2EE_ENABLED = CONFIG.get("matrix_e2ee_enabled", True)
MATRIX_HOMESERVER = CONFIG.get("matrix_homeserver", MATRIX_HOMESERVER)
MATRIX_USER_ID = CONFIG.get("matrix_user_id", MATRIX_USER_ID)
MATRIX_ACCESS_TOKEN = CONFIG.get("matrix_access_token", MATRIX_ACCESS_TOKEN)
...
# =============================================================================
# Hardware integration layer + async wrapper
# =============================================================================
...
# TODO: swap this with your real hardware implementation
HARDWARE: HardwareInterface = MockHardware()

class AsyncHardwareWrapper:
    def __init__(self, autopilot_core: AutopilotCore, interval: float = 1.0):
...
def validate_autopilot_message(
    ...
    if msg_type == "NavigationCommand":
        ...
        if current_mode == VehicleMode.EMERGENCY_STOP:
            return False, "Navigation blocked in EMERGENCY_STOP"

        return True, "OK"

    if msg_type == "SystemQuery":
        return True, "OK"
...
class MQTTBridge:
    ...
    async def _handle_command(self, text: str):
        try:
            result = await self.lpu.process_text(text, source="mqtt")
            await EVENT_BUS.publish("mqtt_result", result)
        except Exception as e:
...
class MatrixBridge:
    def __init__(self):
        if nio is None or not MATRIX_ENABLED:
            self.client = None
            return
        ...
        self.RoomMessageText = RoomMessageText

        cfg = AsyncClientConfig(
            store_sync_tokens=True,
            encryption_enabled=MATRIX_E2EE_ENABLED,
        )
        self.client = AsyncClient(
            MATRIX_HOMESERVER,
            MATRIX_USER_ID,
            config=cfg,
            store_path=MATRIX_STORE_PATH if MATRIX_E2EE_ENABLED else None,
        )
    ...
    async def start(self):
        if self.client is None:
            logger.info("[Matrix] Disabled.")
            return

        RoomMessageText = self.RoomMessageText

        logger.info("[Matrix] Starting Matrix bridge...")
        try:
            await self.client.sync(timeout=3000, full_state=True)
            logger.info("[Matrix] Initial sync complete.")
        except Exception as e:
            logger.error(f"[Matrix] Initial sync failed: {e}")

        EVENT_BUS.subscribe("telemetry", self._on_telemetry_sync)
        EVENT_BUS.subscribe("mode_change", self._on_mode_changed_sync)
        EVENT_BUS.subscribe("emergency", self._on_emergency_sync)
                    ...
                    for room_id, room in resp.rooms.join.items():
                        for ev in room.timeline.events:
                            if isinstance(ev, RoomMessageText):
                                await self._handle_message(room_id, ev)
                except Exception as e:
                    logger.error(f"[Matrix] recv_loop error: {e}")
        ...
        await asyncio.gather(recv_loop(), heartbeat_loop())
    ...
    async def _handle_message(self, room_id, ev):
        body = ev.body.strip()
        sender = ev.sender
        logger.info(f"[Matrix] <{sender}> in {room_id}: {body}")

        if room_id == MATRIX_ROOM_COMMANDS_REAL:
            channel = "real"
        elif room_id == MATRIX_ROOM_COMMANDS_SIM:
            channel = "sim"
        else:
            return
        ...
        await EVENT_BUS.publish("matrix_text_command", envelope)
    ...
    def _on_telemetry_sync(self, snapshot: dict):
        asyncio.get_event_loop().create_task(self._on_telemetry(snapshot))

    async def _on_telemetry(self, snapshot: dict):
        text = (
            f"[TELEMETRY] mode={snapshot.get('mode','?')} "
            f"speed={snapshot.get('speed','?')} "
            f"lat={snapshot.get('gps', ['?','?'])[0]} "
            f"lon={snapshot.get('gps', ['?','?'])[1]}"
        )
        await self._send_text(MATRIX_ROOM_TELEMETRY_REAL, text)

    def _on_mode_changed_sync(self, event: dict):
        asyncio.get_event_loop().create_task(self._on_mode_changed(event))

    async def _on_mode_changed(self, event: dict):
        mode = event.get("mode")
        text = f"[MODE] {mode}"
        await self._send_text(MATRIX_ROOM_TELEMETRY_REAL, text)

    def _on_emergency_sync(self, event: dict):
        asyncio.get_event_loop().create_task(self._on_emergency(event))

    async def _on_emergency(self, event: dict):
        text = (
            f"🚨 EMERGENCY: {event.get('event','unknown')}"
        )
        await self._send_text(MATRIX_ROOM_ALERTS, text)
...
# =============================================================================
# PySide6 GUI with telemetry, metrics, swarm GPS map, 3D view
# =============================================================================
...
async def main_async_server():
    autopilot_core = AutopilotCore()
    lpu = LanguageProcessingUnit(autopilot_core, unit_id="LPU-Autopilot-Kernel")
    ...
    if MATRIX_ENABLED and MATRIX_BRIDGE.client is not None:
        tasks.append(MATRIX_BRIDGE.start())
...
def launch_gui():
    ...
    autopilot_core = AutopilotCore()
    gui = LPUClientGUI(autopilot_core)
    gui.show()
    sys.exit(app.exec())
