#!/usr/bin/env python3
"""
UNIFIED_MEMORY_SYSTEM_BORG_NEURAL_OS_GOVERNOR_v6.py

v6 CHANGES (high level):
- Added neural brain persistence (save/load weights + replay-safe training)
- Added PPO agent persistence
- Added stronger safety guards around CUDA/OpenCL/psutil/subprocess
- Tightened reward clipping and numeric stability
- Improved VRAM fragmentation + more “real” GPU compaction within our abstraction
- Multi-GPU aware telemetry and fragmentation stats
- Watchdog-safe policy restart and HUD launch
- Minor performance optimizations (reduced allocations, cached shapes)
- Architecture notes:
  - Subsystems rated (in comments) and bottlenecks identified
  - v6 keeps monolithic file but is ready for modularization
"""
...
_torch_available = _try_import("torch")
if _torch_available:
    import torch
...
HUD_STATE_FILE = "hud_state.json"
NEURAL_BRAIN_WEIGHTS = "neural_brain_v6.pt"
PPO_AGENT_WEIGHTS = "ppo_agent_v6.pt"
...
class HardwareGenome:
    """
    Subsystem rating (v6):
    - Profiling quality: 8/10
    - Bottleneck: disk speed proxy is trivial; GPU perf not measured, only VRAM.
    """
    def __init__(self):
        self.genome = {}
        self._profile()
...
# -----------------------------
# Accelerator abstraction (LPU / NPU / DPU hooks)
# -----------------------------

class AcceleratorManager:
    """
    Rating:
    - Abstraction clarity: 7/10
    - Bottleneck: no real LPU/DPU detection; flags are heuristic.
    """
    def __init__(self):
        self.has_lpu = False
        self.has_npu = _torch_available and torch.cuda.is_available()
        self.has_dpu = BACKEND.cuda and BACKEND.numba

    def can_run_neural(self):
        return self.has_npu

    def can_run_compaction(self):
        return self.has_dpu
...
class UnifiedBuffer:
    """
    Rating:
    - Abstraction power: 9/10
    - Bottleneck: managed arrays limit fine-grained multi-GPU control.
    """
    ...
    def _init_cuda_unified(self):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to allocate CUDA unified memory: {e}")
...
# -----------------------------
# CUDA kernels (including compaction)
# -----------------------------
...
if BACKEND.cuda and BACKEND.numba:
    ...
    @cuda.jit
    def prefix_sum_kernel(input_arr, output_arr):
        shared = cuda.shared.array(1024, dtype=cuda.float32)
        tid = cuda.threadIdx.x
        i = cuda.grid(1)
        if i < input_arr.size:
            shared[tid] = input_arr[i]
        cuda.syncthreads()
        stride = 1
        ...
        while stride > 0:
            idx = (tid + 1) * stride * 2 - 1
            if idx + stride < cuda.blockDim.x:
                shared[idx + stride] += shared[idx]
            stride //= 2
            cuda.syncthreads()
        if i < input_arr.size:
            output_arr[i] = shared[tid]

    @cuda.jit
    def scatter_compact_kernel(src, dst, offsets, base_offset):
        i = cuda.grid(1)
        if i < src.size:
            new_idx = int(offsets[i]) + base_offset
            if 0 <= new_idx < dst.size:
                dst[new_idx] = src[i]
...
# -----------------------------
# VRAM fragmentation model + GPU allocator compaction sketch (v6)
# -----------------------------
...
class VRAMFragmentationModel:
    ...
    - Bottleneck: still abstract; v6 adds more realistic compaction within UnifiedBuffer world.
    """
    def __init__(self):
        self.regions = []
        self.next_addr = 0
    ...
    def gpu_compact(self, manager_buffers):
        """
        v6: More realistic compaction:
        - Build sizes array
        - Prefix-sum to compute offsets
        - Allocate a single large UnifiedBuffer on GPU
        - Scatter-copy each buffer into the compact pool
        - Update fragmentation model to reflect single region
        """
        if not (BACKEND.cuda and BACKEND.numba and ACCEL.can_run_compaction()):
            return

        from numba import cuda

        gpu_buffers = [b for b in manager_buffers if b.mode == "cuda_unified"]
        if not gpu_buffers:
            return

        sizes = np.array([b.size for b in gpu_buffers], dtype=np.float32)
        if sizes.size == 0:
            return

        try:
            d_in = cuda.to_device(sizes)
            d_out = cuda.device_array_like(d_in)

            threads_per_block = 1024
            blocks = (sizes.size + threads_per_block - 1) // threads_per_block
            prefix_sum_kernel[blocks, threads_per_block](d_in, d_out)
            cuda.synchronize()

            offsets = d_out.copy_to_host()
            total_elems = int(offsets[-1])
            if total_elems <= 0:
                return

            compact_buf = UnifiedBuffer(total_elems, dtype=np.float32, mode="cuda_unified", tag="gpu_compact")

            # Scatter each buffer into compact_buf
            base_offset = 0
            for i, b in enumerate(gpu_buffers):
                # local offsets for this buffer
                local_size = b.size
                local_offsets = np.arange(local_size, dtype=np.float32)
                d_local_offsets = cuda.to_device(local_offsets)

                threads_per_block = 256
                blocks = (local_size + threads_per_block - 1) // threads_per_block
                scatter_compact_kernel[blocks, threads_per_block](b.gpu, compact_buf.gpu, d_local_offsets, base_offset)
                cuda.synchronize()
                base_offset += local_size

            # Mark compaction in fragmentation model
            self.clear()
            self.next_addr = compact_buf.bytes()
            self.register_buffer(compact_buf)
        except Exception:
            # Safety: never crash the governor due to compaction failure
            return
...
class AdvancedPredictor:
    ...
    Rating:
    - Robustness: 7/10
    - Bottleneck: purely heuristic; no online learning.
    """
    def __init__(self):
...
class AlteredStatesEngine:
    ...
    Rating:
    - Expressiveness: 9/10
    - Bottleneck: thresholds need auto-tuning (handled by PolicyModule).
    """
    def __init__(self):
        self.state = "FLOW"
        self.workload_profile = "generic"
        self.panic_threshold = 0.97
        self.alert_threshold = 0.9
        self.dream_threshold = 0.35

    def set_workload_profile(self, profile):
        if profile in ("gaming", "llm", "generic"):
            self.workload_profile = profile

    def update_thresholds(self, panic_th, alert_th, dream_th):
        self.panic_threshold = panic_th
        self.alert_threshold = alert_th
        self.dream_threshold = dream_th
    ...
    def update_state(self, ram_used, ram_total, vram_used, vram_total,
        ...
        high_future = False
        if pred_ram and ram_total and pred_ram / ram_total > self.alert_threshold:
            high_future = True
        if pred_vram and vram_total and pred_vram / vram_total > self.alert_threshold:
            high_future = True

        if ram_ratio > self.panic_threshold or vram_ratio > self.panic_threshold:
            self.state = "PANIC"
        elif ram_ratio > self.alert_threshold or vram_ratio > self.alert_threshold or high_future:
            self.state = "ALERT"
        elif ram_ratio < self.dream_threshold and vram_ratio < self.dream_threshold:
            self.state = "DREAM"
        else:
...
# -----------------------------
# Water-physics engine (Navier-Stokes style)
# -----------------------------
...
class WaterPhysicsEngine:
    ...
    Rating:
    - Creativity: 10/10
    - Bottleneck: parameters need feedback tuning (v6 adds self_rewrite_from_outcomes).
    """
    def __init__(self):
        ...
        self.pressure_history = deque(maxlen=64)
    ...
    def compute_hints(self, ram_ratio, vram_ratio, frag_score, state):
        ...
        grad_p = pressure_gpu - pressure_cpu
        velocity = grad_p * (1.0 - self.viscosity)

        flow_gpu_to_cpu = max(0.0, velocity)
        flow_cpu_to_gpu = max(0.0, -velocity)

        self.pressure_history.append(max(pressure_gpu, pressure_cpu))
        if len(self.pressure_history) > 4:
            p_arr = np.array(self.pressure_history, dtype=np.float32)
            turbulence = self.turbulence_weight * float(np.std(p_arr) + 0.5 * np.std(p_arr[-4:]))
        else:
    ...
    def self_rewrite_from_outcomes(self, avg_turbulence, avg_pressure):
        if avg_turbulence > 0.3:
            ...
            self.viscosity = min(0.3, self.viscosity + 0.01)
        else:
            self.turbulence_weight *= 0.995
            self.viscosity = max(0.05, self.viscosity - 0.005)
...
# -----------------------------
# Deeper neural network brain with replay buffer (v6 with persistence)
# -----------------------------
...
class NeuralMemoryBrain:
    ...
    Rating:
    - Power: 9/10
    - Bottlenecks: no persistence (fixed in v6), reward shaping, batch stability.
    """
    def __init__(self, lr=1e-3, replay_capacity=5000, batch_size=32):
        self.dim = 16
        self.batch_size = batch_size
        self.last_pressure = None

        if not (_torch_available and ACCEL.can_run_neural()):
            self.model = None
            self.optimizer = None
            self.replay = None
            return
        ...
        self.model = DeepMLP(input_dim=self.dim, hidden_dim=64, output_dim=2).to(self.device)
        self.optimizer = optim.Adam(self.model.parameters(), lr=lr)
        self.replay = ReplayBuffer(capacity=replay_capacity)

        # v6: load persisted weights if available
        self._load_weights()
    ...
    def _features_for_buffer(self, meta, frag_score, ram_ratio, vram_ratio, now):
        ...
        layer_id = buf_meta.get("layer_id", 0)
        layer_norm = np.tanh(layer_id / 64.0)

        mode_cpu = 1.0 if meta["mode"] == "cpu" else 0.0
        mode_gpu = 1.0 if meta["mode"] == "cuda_unified" else 0.0
        ...
        x = np.array([
            ...
            mode_gpu,
            1.0,
            frag_score * ram_ratio,
            frag_score * vram_ratio,
            ram_ratio - vram_ratio,
            1.0,
        ], dtype=np.float32)
        return x

    def _train_step(self):
        if self.model is None or self.replay is None:
            return
        ...
        with torch.no_grad():
            ...
            target_q = rewards + 0.95 * max_next_q

        q_values = self.model(states)
        q_selected = q_values.gather(1, actions.unsqueeze(1)).squeeze(1)

        loss = ((q_selected - target_q) ** 2).mean()
        ...
        # v6: persist weights after each train step (cheap for small net)
        self._save_weights()

    def _save_weights(self):
        try:
            torch.save(self.model.state_dict(), NEURAL_BRAIN_WEIGHTS)
        except Exception:
            pass

    def _load_weights(self):
        if not os.path.exists(NEURAL_BRAIN_WEIGHTS):
            return
        try:
            state = torch.load(NEURAL_BRAIN_WEIGHTS, map_location=self.device)
    ...
    def decide_and_learn(self, state, hints, buffer_meta, ram_ratio, vram_ratio, frag_score):
        ...
        for i, meta in enumerate(buffer_meta):
            mode = meta["mode"]
            x_np = self._features_for_buffer(meta, frag_score, ram_ratio, vram_ratio, now)
            x = torch.from_numpy(x_np).to(self.device).unsqueeze(0)
            with torch.no_grad():
                scores = self.model(x)[0]
            score_cpu = float(scores[0].item())
            score_gpu = float(scores[1].item())
            ...
            next_pressure = pressure * (0.98 if target == "cpu" and vram_ratio > ram_ratio else 0.99)
            reward = (self.last_pressure - next_pressure) * 10.0
            reward = float(np.clip(reward, -5.0, 5.0))  # v6 safety: clip reward
...
# -----------------------------
# PPO / Actor-Critic RL system (v6 with persistence)
# -----------------------------

class ActorCritic(nn.Module):
    def __init__(self, state_dim=4, hidden_dim=64, n_actions=4):
        super().__init__()
...
class PPOAgent:
    """
    Rating:
    - Policy expressiveness: 8/10
    - Bottleneck: small state space; persistence added in v6.
    """
    def __init__(self, n_actions=4, state_dim=4, lr=3e-4, gamma=0.99, clip_eps=0.2):
        if not (_torch_available and ACCEL.can_run_neural()):
            self.model = None
            return
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model = ActorCritic(state_dim=state_dim, hidden_dim=64, n_actions=n_actions).to(self.device)
        self.optimizer = optim.Adam(self.model.parameters(), lr=lr)
        self.gamma = gamma
    ...
    def select_action(self, state_vec):
        if self.model is None:
            return 0, 0.0, 0.0
        s = torch.from_numpy(state_vec.astype(np.float32)).to(self.device).unsqueeze(0)
        with torch.no_grad():
    ...
    def update(self):
        if self.model is None or not self.memory:
            self.memory = []
            return

        states = torch.tensor([m[0] for m in self.memory], dtype=torch.float32, device=self.device)
        actions = torch.tensor([m[1] for m in self.memory], dtype=torch.long, device=self.device)
        old_log_probs = torch.tensor([m[2] for m in self.memory], dtype=torch.float32, device=self.device)
        rewards = [m[3] for m in self.memory]
        values = torch.tensor([m[4] for m in self.memory], dtype=torch.float32, device=self.device)
        ...
        critic_loss = ((returns - new_values.squeeze()) ** 2).mean()
    ...
    def _save_weights(self):
        try:
            torch.save(self.model.state_dict(), PPO_AGENT_WEIGHTS)
        except Exception:
            pass

    def _load_weights(self):
        if not os.path.exists(PPO_AGENT_WEIGHTS):
            return
        try:
            state = torch.load(PPO_AGENT_WEIGHTS, map_location=self.device)
            self.model.load_state_dict(state)
        except Exception:
            pass
...
class RLMemoryBrain:
    """
    PPO / Actor-Critic RL agent:
    - Actions: balanced, gpu_pref, cpu_pref, aggressive_compact
    """
    def __init__(self):
        self.actions = ["balanced", "gpu_pref", "cpu_pref", "aggressive_compact"]
        self.agent = PPOAgent(n_actions=len(self.actions), state_dim=4)
        self.last_pressure = None
        self.last_frag = None
    ...
    def update(self, ram_ratio, vram_ratio, frag_score, state):
        ...
        reward = 0.0
        reward += (self.last_pressure - pressure) * 15.0
        reward += (self.last_frag - frag_score) * 8.0
        if state == "PANIC":
            reward -= 10.0
        elif state == "ALERT":
            reward -= 5.0
        ...
        state_vec = np.array([ram_ratio, vram_ratio, frag_score, 1.0 if state == "PANIC" else 0.0], dtype=np.float32)

        if self.agent.model is None:
            action_idx = 0
        else:
            action_idx, log_prob, value = self.agent.select_action(state_vec)
            self.agent.store(state_vec, action_idx, log_prob, reward, value)
...
# -----------------------------
# Policy module (self-rewriting)
# -----------------------------

class PolicyModule:
    """
    Rating:
    - Meta-policy quality: 8/10
    - Bottleneck: only looks at PANIC/ALERT rates, not full state distribution.
    """
    def __init__(self):
        self.params = {
    ...
    def rewrite(self):
        ...
        self.params["dream_threshold"] = float(np.clip(self.params["dream_threshold"], 0.1, 0.5))

    def apply_to_states(self, states_engine: AlteredStatesEngine):
        states_engine.update_thresholds(
            self.params["panic_threshold"],
            self.params["alert_threshold"],
            self.params["dream_threshold"],
        )
...
class GamingVRAMAllocator:
    """
    Rating:
    - Heuristic quality: 8/10
    - Bottleneck: no frame timing feedback; purely age + flags.
    """
...
class LLMTensorResidencyEngine:
    ...
    Rating:
    - LLM-awareness: 9/10
    - Bottleneck: no per-layer access frequency stats; uses age + layer position.
    """
...
class PolicyEngine:
    """
    v6 architecture:
    - PredictiveEngine + AlteredStatesEngine + WaterPhysicsEngine
    - NeuralMemoryBrain (per-buffer decisions, persistent)
    - RLMemoryBrain (global PPO policy, persistent)
    - GamingVRAMAllocator + LLMTensorResidencyEngine
    - VRAMFragmentationModel with more realistic compaction
    """
    ...
    def apply_policies(self):
        ...
        self.policy_module.apply_to_states(self.states)
        ...
        if hints["need_compaction"]:
            self.manager.vram_frag_model.gpu_compact(self.manager.buffers)
...
class UnifiedMemoryManager:
    ...
    - Bottleneck: monolithic file; ready for modular split in v7.
    """
    def __init__(self):
...
def print_ascii_telemetry(manager: UnifiedMemoryManager):
    ...
    print("=" * 110)
    print("UNIFIED MEMORY LIVE STATUS (NEURAL GOVERNOR v6)")
...
def write_hud_state(state, profile, ram_used, ram_total, vram_used, vram_total):
    try:
        data = {
            "state": state,
            "profile": profile,
...
# -----------------------------
# Cluster sharding + multi-node tensor migration
# -----------------------------

class ClusterNode:
    """
    Rating:
    - Distributed awareness: 8/10
    - Bottleneck: conceptual tensor migration; no actual data transfer.
    """
        ...
        def server_loop():
            ...
            while self.running:
                try:
                    ...
                    elif msg.get("type") == "tensor_migrate":
                        pass
                    conn.close()
                except Exception:
                    continue
    ...
    def _local_state(self):
        ...
        llm_bytes = sum(b.bytes() for b in self.manager.buffers if b.meta.get("is_llm_tensor"))
        return {
            "buffers": [
                {
                    "size": b.size,
                    ...
                    "tag": b.tag,
                    "shard_id": b.shard_id,
                    "is_llm": b.meta.get("is_llm_tensor", False),
                }
                for b in self.manager.buffers
            ],
            "total_bytes": total_bytes,
    ...
    def _node_score(self, node_id):
        ...
        llm_bytes = st.get("llm_bytes", 0)

        load_penalty = total_bytes / (1024**3)
        gpu_penalty = gpu_ratio
        llm_penalty = llm_bytes / (1024**3)
        latency_penalty = 0.0
        ...
        score = -0.4 * load_penalty - 0.3 * gpu_penalty - 0.2 * latency_penalty - 0.1 * llm_penalty
        return score

    def _consistent_hash_index(self, key, n):
        h = 0
        for c in key:
    ...
    def suggest_shard_id(self, key):
        ...
        if not nodes:
            return self.node_id
        base_idx = self._consistent_hash_index(key, len(nodes))
    ...
    def rebalance_shards(self):
        ...
        if not nodes:
            return
        for b in self.manager.buffers:
            if not b.meta.get("is_llm_tensor"):
                continue
            desired = self.suggest_shard_id(f"{b.tag}:{b.size}")
            if b.shard_id != desired:
                b.shard_id = desired
...
class SystemGovernor:
    ...
    Rating:
    - Adaptivity: 8/10
    - Bottleneck: only uses static hardware genome; could incorporate runtime stats.
    """
...
class Watchdog:
    ...
    Rating:
    - Safety: 9/10
    - Bottleneck: no backoff; always restarts immediately.
    """
...
# -----------------------------
# HUD process (compact, black text)
# -----------------------------
...
class MiniMemoryHUD:
    ...
    Rating:
    - UX: 8/10
    - Bottleneck: single-process Tkinter; no remote control.
    """
    def __init__(self):
        ...
        self.state_label = tk.Label(top_frame, text="State: N/A", font=self.font_small)
        self.state_label.pack(side="left", padx=4)

        self.profile_label = tk.Label(top_frame, text="Profile: N/A", font=self.font_small)
        self.profile_label.pack(side="left", padx=4)
        ...
        self.ram_label = tk.Label(self.main_frame, text="", font=self.font_main, anchor="w", fg="black")
        self.ram_label.pack(fill="x", pady=3, padx=4)

        self.vram_label = tk.Label(self.main_frame, text="", font=self.font_main, anchor="w", fg="black")
        self.vram_label.pack(fill="x", pady=3, padx=4)

        self.cpu_gpu_label = tk.Label(self.main_frame, text="", font=self.font_main, anchor="w", fg="black")
    ...
    def read_hud_state(self):
        if not os.path.exists(HUD_STATE_FILE):
            return None
        try:
            with open(HUD_STATE_FILE, "r") as f:
                return json.load(f)
        except Exception:
            return None
    ...
    def update(self):
        ...
        hud_state = self.read_hud_state()
        if hud_state:
            state = hud_state.get("state", "N/A")
            profile = hud_state.get("profile", "N/A")
            self.state_label.config(text=f"State: {state}")
            self.profile_label.config(text=f"Profile: {profile}")

            if state == "PANIC":
                self.root.configure(bg="#ff4040")
            elif state == "ALERT":
                self.root.configure(bg="#ffff80")
            elif state == "DREAM":
                self.root.configure(bg="#80aaff")
            else:
                self.root.configure(bg="#f0f0f0")

        if not _psutil_available:
            self.ram_label.config(text="RAM: psutil not available", fg="black")
            self.cpu_gpu_label.config(text="CPU: N/A   GPU Temp: N/A", fg="black")
            ...
            if vram_total > 0:
                ...
                self.vram_label.config(text="VRAM: No NVIDIA GPU detected", fg="black")
...
def run_live_system(use_hud=True, log_history=False, start_cluster=False,
    ...
    def restart_policy():
        print("[WATCHDOG] Restarting policy engine...")
        mgr.policy_engine = PolicyEngine(mgr)
    watchdog.add_restart_callback(restart_policy)
    threading.Thread(target=watchdog.monitor_loop, daemon=True).start()
    ...
    def telemetry_loop():
        while True:
            try:
                ...
                state = mgr.policy_engine.states.state
                profile = mgr.policy_engine.states.workload_profile

                write_hud_state(state, profile, ram_used, ram_total, vram_used, vram_total)

                print_ascii_telemetry(mgr)
                if logger and ram_used is not None:
