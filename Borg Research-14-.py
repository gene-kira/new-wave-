# ============================================================
# Mythic Swarm Engine — Ultra Unified Swarm Cluster
# Real-physics sims + real LLM integration + distributed mesh + GUI
# Single-file mega-engine
# ============================================================
...
autoload(["numpy", "psutil", "torch", "openai"])

# --- Imports & globals ---
import os, json, time, random, hashlib, math, zlib, logging, datetime, threading, queue, textwrap, traceback, socket
...
def compose_idea(seed_noise: float = 0.15, learned: dict = None) -> dict:
    def pick(cat):
        base_list = PRIMITIVES[cat]
        choice = random.choice(base_list)
        if learned and "top_constraints" in learned and cat == "constraints":
            # bias toward learned constraints
            choice = random.choice(learned["top_constraints"] or base_list)
        if random.random() < seed_noise and cat != "constraints":
    ...
    if learned and "top_constraints" in learned:
        constraints = list(set(constraints + learned["top_constraints"]))[:7]
...
def mutate_idea_safe(idea: dict) -> dict:
    m = mutate_idea(idea)
    m["constraints"] = list(dict.fromkeys(m["constraints"]))[:7]
    return clamp_params(m)

def idea_to_text(idea: dict) -> str:
    return json.dumps(ensure_idea_keys(idea), sort_keys=True)

# ============================================================
# Evaluation + real-ish physics simulators (CPU + GPU)
# ============================================================
...
def novelty_score(candidate_text: str, archive_texts: list) -> float:
    if not archive_texts:
        return 1.0 + compressibility_score(candidate_text)
    if HAS_NUMPY:
        distances = []
...
class OrbitalComfortSimulator(Simulator):
    ...
    def score(self, idea: dict):
        ...
        diag = {
            ...
            "coriolis_penalty": coriolis_penalty,
            "power_factor": power_ok
        }
        return utility, diag
...
class ThermalSimulator(Simulator):
    ...
    def score(self, idea: dict):
        ...
        # simple conduction model: ΔT ~ P / (k*A)
        area = max(1.0, scale * 10.0)
        k_eff = 50.0  # W/mK effective
        temp_rise = power / max(1.0, k_eff * area)
        penalty = min(1.0, temp_rise / 40.0)
        utility = max(0.0, 1.0 - penalty)
        return utility, {"temp_rise": temp_rise, "penalty": penalty}

class StructuralSimulator(Simulator):
    name = "structural"
    def score(self, idea: dict):
        p = ensure_idea_keys(idea)["params"]
        scale = p["scale"]
        tol = p["tolerance"]
        # log stress vs scale, tolerance as fragility
        stress = math.log10(max(1.0, scale)) * (1.0 + tol*5.0)
        penalty = min(1.0, stress / 5.0)
        utility = max(0.0, 1.0 - penalty)
        return utility, {"stress": stress, "penalty": penalty}

class EnergyRoutingSimulator(Simulator):
    name = "energy_routing"
    def score(self, idea: dict):
        p = ensure_idea_keys(idea)["params"]
        power = p["power_budget"]
        mut = p["mutation_rate"]
        efficiency = max(0.1, 1.0 - math.log10(max(1.0, power))/6.0 - mut*0.3)
        utility = max(0.0, efficiency)
        return utility, {"efficiency": efficiency}

class AIBehaviorSimulator(Simulator):
    name = "ai_behavior"
    def score(self, idea: dict):
        p = ensure_idea_keys(idea)["params"]
        mut = p["mutation_rate"]
        tol = p["tolerance"]
        stability = max(0.0, 1.0 - mut*0.8 - tol*0.5)
        exploration = min(1.0, mut*1.2 + tol*0.3)
        utility = 0.5*stability + 0.5*exploration
        return utility, {"stability": stability, "exploration": exploration}

class GPUSimulator(Simulator):
    name = "gpu_physics"
    def score(self, idea: dict):
        if not (HAS_TORCH and torch.cuda.is_available()):
            return 0.0, {"gpu": "unavailable"}
        p = ensure_idea_keys(idea)["params"]
        scale = p["scale"]
        power = p["power_budget"]
        try:
            device = torch.device("cuda")
            x = torch.tensor([scale, power], dtype=torch.float32, device=device)
            w = torch.tensor([[0.3, -0.1], [-0.2, 0.4]], dtype=torch.float32, device=device)
            y = torch.matmul(w, x)
            score = float(torch.sigmoid(y.mean()).item())
            return score, {"gpu_score": score}
        except Exception:
            return 0.0, {"gpu": "error"}

SIMULATORS = [
    OrbitalComfortSimulator(),
    AtmosphereRoutingSimulator(),
    ThermalSimulator(),
    StructuralSimulator(),
    EnergyRoutingSimulator(),
    AIBehaviorSimulator(),
    GPUSimulator()
]
...
def impact_score(idea: dict) -> float:
    cons = set(ensure_idea_keys(idea).get("constraints", []))
    score = 0.0
...
def evaluate_idea_full(idea: dict, archive_texts: list, cfg: dict,
    ...
    idea = clamp_params(ensure_idea_keys(idea))
    ...
    w = {
        "novelty_weight": cfg.get("novelty_weight", 0.25),
    ...
    final_score = (w["novelty_weight"]*novelty +
    ...
    return {
        ...
        "sims": results,
        "hash": idea_hash(text),
        "name": autonomous_project_name(idea, cfg.get("node_name","local")),
        "objective": final_score
    }

# ============================================================
# Strategies (versioned + diff log)
# ============================================================
STRATEGY_PATH = os.path.join(os.getcwd(), "strategies.py")
STRATEGY_LOG_PATH = os.path.join(os.getcwd(), "strategies_log.jsonl")

def ensure_strategy_file(cfg: dict):
    if os.path.exists(STRATEGY_PATH):
        return
    nw, uw, iw, cw = (
        cfg.get("novelty_weight",0.25),
        cfg.get("utility_weight",0.35),
        cfg.get("impact_weight",0.25),
        cfg.get("curiosity_weight",0.15)
    )
...
def load_strategy():
    try:
        ...
        logging.exception("Failed to load strategies module.")
        return None

def log_strategy_mutation(before_text: str, after_text: str, cfg: dict, source: str = "local"):
    try:
        entry = {
            "timestamp": time.time(),
        ...
        logging.exception("Failed to log strategy mutation.")
...
def mutate_strategy(cfg: dict):
    try:
        ...
        def jitter(val: float) -> float:
            return max(0.0, val + random.uniform(-scale, scale))
        def mutate_line(line: str) -> str:
            for base in ["0.05","0.03","0.04","0.02"]:
                line = line.replace(base, f"{jitter(float(base)):.3f}")
            return line
        ...
        log_strategy_mutation(before, mutated, cfg, source="local_mutation")
    except Exception:
        logging.exception("Failed to mutate strategy file.")

def overwrite_strategy_from_llm(new_text: str, cfg: dict):
    try:
        with open(STRATEGY_PATH, "r", encoding="utf-8") as f:
            before = f.read()
    except Exception:
    ...
    try:
        with open(STRATEGY_PATH, "w", encoding="utf-8") as f:
            f.write(new_text)
        importlib.invalidate_caches()
        log_strategy_mutation(before, new_text, cfg, source="llm")
    except Exception:
        logging.exception("Failed to overwrite strategy from LLM.")
    ...
    if os.path.exists(ap):
        try:
            ...
            if not isinstance(data, dict):
                raise ValueError("Archive not dict")
            ...
            stats.setdefault("storage_dir", paths.get("storage_dir", ""))
            stats.setdefault("learned", {})
            return data
        except Exception:
...
def enforce_archive_limits(archive: dict, cfg: dict):
    max_size = int(cfg.get("max_archive_size", 1000))
    ideas = archive.get("ideas", {})
    if len(ideas) > max_size:
        def score_of(item):
            try:
                ev = item[1].get("eval") or item[1]
                return ev.get("final_score", ev.get("objective", 0.0))
            except Exception:
                return 0.0
...
# ============================================================
# Re-ingestion: learn from generated projects
# ============================================================
def reingest_archive_patterns(archive: dict):
    ideas = archive.get("ideas", {})
    if not ideas:
        return
    constraint_counts = {}
    scale_vals, power_vals, tol_vals, mut_vals = [], [], [], []
    for item in ideas.values():
        try:
            idea = item.get("idea") or {}
            cons = idea.get("constraints", [])
            for c in cons:
                constraint_counts[c] = constraint_counts.get(c, 0) + 1
            p = idea.get("params", {})
            if "scale" in p: scale_vals.append(float(p["scale"]))
            if "power_budget" in p: power_vals.append(float(p["power_budget"]))
            if "tolerance" in p: tol_vals.append(float(p["tolerance"]))
            if "mutation_rate" in p: mut_vals.append(float(p["mutation_rate"]))
        except Exception:
            continue
    top_constraints = sorted(constraint_counts.items(), key=lambda x: x[1], reverse=True)
    top_constraints = [c for c, _ in top_constraints[:5]]
    def rng(vals):
        if not vals:
            return (None, None)
    ...
    learned = {
        ...
        "param_ranges": {
            "scale": rng(scale_vals),
            "power_budget": rng(power_vals),
            "tolerance": rng(tol_vals),
            "mutation_rate": rng(mut_vals),
        }
    }
    archive["stats"]["learned"] = learned
    logging.info("[Reingest] Updated learned patterns: %s", learned)

# ============================================================
# Metrics & entropy (CPU + GPU)
# ============================================================
class SystemMetrics:
    def __init__(self):
...
def system_entropy(sys_metrics: dict) -> float:
    ...
    gpu = sys_metrics.get("gpu_util", 0.0) / 100.0
    return min(2.5, 0.5 * cpu + 0.001 * procs + 0.002 * conns + 0.1 * net + 0.1 * disk + 0.4 * gpu)
...
def feed_entropy(feeds: FeedRegistry) -> float:
    try:
        snap = feeds.snapshot()
        items = float(snap.get("items", 0))
        sources = float(snap.get("sources", 0))
...
class Throttle:
    def __init__(self, cfg: dict):
        ...
        self.cfg = {
            "sleep_min_seconds": cfg.get("sleep_min_seconds", 0.025),
    ...
    def adjust(self):
        if psutil is None:
            self.sleep_s = min(self.cfg["sleep_max_seconds"], self.sleep_s * 1.15 + 0.03)
            return
        try:
            cpu = psutil.cpu_percent(interval=0.15) / 100.0
            target = self.cfg["cpu_target_util"]
            if cpu > target:
                self.sleep_s = min(self.cfg["sleep_max_seconds"], self.sleep_s * 1.35 + 0.06)
            else:
                self.sleep_s = max(self.cfg["sleep_min_seconds"], self.sleep_s * 0.92 - 0.01)
        except Exception:
            pass
    def wait(self):
...
class StatusBus:
    ...
    def emit(self, event: str, payload: dict = None):
        try:
            self.q.put_nowait({"event": event, "payload": payload or {}})
        except Exception:
            pass
    def get_nowait(self):
        try:
...
class BorgMesh:
    ...
    def snapshot(self):
        with self.lock:
            return {
                u: {"snippet": v["snippet"], "links": list(v["links"]), "state": v["state"]}
...
class CodeBuilder:
    ...
    def add_header(self):
        hdr = [
            ...
            f"# Scores: final={self.evals.get('final_score', self.evals.get('objective', 0.0)):.4f} "
            f"novelty={self.evals['novelty']:.4f} utility={self.evals['utility']:.4f} "
            f"impact={self.evals['impact']:.4f} curiosity={self.evals['curiosity']:.4f}",
            "",
            '"""',
...
# ============================================================
# Multi-node distributed evolution (swarm cluster)
# ============================================================
class SwarmNode:
                ...
                with _archive_lock:
                    ...
                    for e in elites:
                        ...
                        if hid not in arch["ideas"]:
                            ...
                            arch["top"].append({
                                "id": hid,
                                "objective": ev.get("final_score", ev.get("objective", 0.0)),
                                "novelty": ev.get("novelty", 0.0),
                                "utility": ev.get("utility", 0.0)
                            })
        except Exception:
            pass
        finally:
...
# ============================================================
# Evolution loop (wired to StatusBus + re-ingestion + LLM chaos)
# ============================================================
def select_parents(archive: dict, k: int, seed_noise: float, learned: dict) -> list:
    ...
    for _ in range(k):
        if top_ids and random.random() < 0.7:
            ...
            pool.append(compose_idea(seed_noise, learned=learned))
    return pool
    ...
    while not _shutdown_event.is_set():
        try:
            for step in range(cfg.get("iterations_per_tick", 10)):
                with _archive_lock:
                    archive = archive_ref["ref"]
                    learned = archive["stats"].get("learned", {})
                chosen_dir = choose_storage_dir(cfg)
                _set_storage_dir(chosen_dir)
                with _archive_lock:
                    archive["stats"]["storage_dir"] = chosen_dir
                ...
                parents = select_parents(archive, k=max(5, cfg["batch_size"]//4),
                                         seed_noise=cfg["seed_noise"], learned=learned)
                candidates = []
                active_names = []
                for _ in range(cfg["batch_size"]):
                    base = random.choice(parents)
                    idea = mutate_idea_safe(base) if random.random() < 0.8 else clamp_params(compose_idea(cfg["seed_noise"], learned=learned))
                ...
                if (time.time() - last_reingest) > cfg.get("reingest_interval_seconds", 60.0):
                    with _archive_lock:
                        reingest_archive_patterns(archive)
                    last_reingest = time.time()
                ...
                if no_improve > cfg.get("patience", 50):
                    ...
                    for _ in range(chaos_batch_size):
                        idea = compose_idea(seed_noise=0.2 + 0.2*entropy_sys, learned=learned)
                        if llm_constraints:
                            idea["constraints"] = list(set(idea.get("constraints", []) + llm_constraints))[:7]
                        idea["params"]["mutation_rate"] = round(min(0.9, idea["params"]["mutation_rate"] * (1.0 + 0.4*entropy_feed + 0.4*entropy_sys)), 4)
                        ...
                        evals = evaluate_idea_full(idea, archive_texts, cfg, perf=perf, weights_override=None)
...
class AdvancedGUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.lbl_summary = ttk.Label(topbar, text="Starting...", anchor="w")
        ...
        self.lbl_storage = ttk.Label(topbar, text=f"Storage: {get_storage_dir()}", anchor="w")
        self.lbl_storage.pack(side="left", padx=8)
        ...
        self.notebook.add(self.tab_active, text="Active")
        self.active_list = tk.Listbox(self.tab_active)
        self.active_list.pack(fill="both", expand=True, padx=10, pady=10)
        self.tab_finished = ttk.Frame(self.notebook)
        self.notebook.add(self.tab_finished, text="Finished")
        self.finished_list = tk.Listbox(self.tab_finished)
        self.finished_list.pack(fill="both", expand=True, padx=10, pady=10)
        btns = ttk.Frame(self.tab_finished)
        btns.pack(fill="x")
        ...
        self.txt_metrics = tk.Text(self.tab_metrics, height=12)
        self.txt_metrics.pack(fill="both", expand=True, padx=10, pady=10)
    ...
    def start_evolver(self, llm_bridge):
        if self.evolver is None:
            self.evolver = threading.Thread(
                target=evolve,
                args=(self.cfg, self.archive_ref, self.feeds, self.sysmon, self.status, self.mesh, self.scanner_in, self.ops_q, llm_bridge),
                daemon=True
            )
            self.evolver.start()
    ...
    def _poll_status_bus(self):
        ...
        if evt:
            ...
            self.lbl_storage.config(text=f"Storage: {stats.get('storage_dir', get_storage_dir())}")
            self.active_list.delete(0, tk.END)
            [self.active_list.insert(tk.END, n) for n in stats.get("active_project_names", [])]
            self.finished_list.delete(0, tk.END)
            [self.finished_list.insert(tk.END, n) for n in stats.get("finished_project_names", [])]
            ...
            if event == "completed":
                self.txt_metrics.insert(tk.END, f"\nCompleted: {', '.join(payload.get('names', []))}")
            elif event == "chaos":
...
# ============================================================
# LLM Bridge + strategy optimizer (real LLM integration)
# ============================================================
LLM_TOOLS_SCHEMA = [
    ...
    {"name": "optimize_strategy", "description": "Rewrite the strategy file based on archive trends",
     "parameters": {"type": "object", "properties": {}}},
    {"name": "suggest_constraints_for_chaos", "description": "Suggest extra constraints for chaos ideas",
     "parameters": {"type": "object", "properties": {}}}
]
...
class LLMBridge:
    ...
    def generate_project(self, params: dict = None) -> dict:
        ...
        idea = compose_idea(local_cfg.get("seed_noise", 0.15),
                            learned=self.archive_ref["ref"]["stats"].get("learned", {}))
    ...
    def optimize_strategy(self) -> dict:
        ...
        return {"summary": summary, "hint": "LLM should rewrite strategies.py using this summary."}
    ...
    def chat(self, messages, tools=None, model=None):
        if self.backend == "Stub":
            ...
            if wants_generate and tools and any(t["name"] == "generate_project" for t in tools):
                ...
                faux["choices"][0]["message"]["content"] = "No tools requested. Ask me to generate, get status, or optimize strategy."
            return faux
...
def run_llm_orchestrator(bridge: LLMBridge, tools_schema, cfg: dict, status_bus: StatusBus):
    ...
    messages = [{"role": "system", "content": "You control the Mythic Swarm Engine via tools. Improve strategy and evolution."}]
    last_error_time = 0.0
    while not _shutdown_event.is_set():
        try:
            ...
            if stats["completed_projects"] < 5:
                ...
                user_prompt = "Get status and, if stagnating, optimize strategy and suggest chaos constraints."
...
if __name__ == "__main__":
    ...
    cfg = {
        ...
        "reingest_interval_seconds": 60.0
    }
    ensure_strategy_file(cfg)
