#!/usr/bin/env python3
"""
HybridBrain v8.0 – Unified Hybrid Everything (v6.3 + v7.0)

Monolithic, OS-adaptive, swarm-ready AI kernel with:
  ...
  - Swarm coordination (TCP JSON) + UDP multicast discovery + ZeroMQ bus
...
class HybridWeightOrgan:
    ...
    def adjust(
        ...
        prev_total = (previous_weights.novelty_weight + previous_weights.utility_weight +
                      previous_weights.impact_weight + previous_weights.curiosity_weight) if previous_weights else 1.0
...
# ============================================================
# PLUGIN SANDBOX (run plugins/organs in subprocesses)
# ============================================================

class PluginSandbox:
    """
    Runs plugin organs in subprocesses.
    Protocol: JSON line on stdin/stdout: {"method": "...", "payload": {...}}
    """
...
# ============================================================
# SWARM DISCOVERY (UDP multicast)
# ============================================================

class SwarmDiscovery:
    """
    Simple UDP multicast-based discovery.
    Nodes broadcast their ID + TCP port; others track them.
    """
...
# ============================================================
# SWARM TOPOLOGY (simple link tracking)
# ============================================================
...
class SwarmTopology:
    ...
    def update_link(self, src: str, dst: str, latency: float, success: bool):
        key = (src, dst)
        info = self.links.get(key, {"latency": [], "success": 0, "total": 0})
        info["latency"].append(latency)
        if len(info["latency"]) > 50:
            info["latency"].pop(0)
        info["total"] += 1
        if success:
            info["success"] += 1
    ...
    def best_neighbors(self, node_id: str) -> List[str]:
        neighbors = {}
        for (src, dst), info in self.links.items():
            if src != node_id:
                continue
            avg_lat = sum(info["latency"]) / len(info["latency"]) if info["latency"] else 999.0
            success_rate = info["success"] / info["total"] if info["total"] > 0 else 0.0
            score = success_rate - 0.1 * avg_lat
            neighbors[dst] = score
        return sorted(neighbors.keys(), key=lambda n: neighbors[n], reverse=True)

    def snapshot(self) -> Dict[str, Any]:
        out = {}
        for (src, dst), info in self.links.items():
            avg_lat = sum(info["latency"]) / len(info["latency"]) if info["latency"] else None
            success_rate = info["success"] / info["total"] if info["total"] > 0 else None
            out[f"{src}->{dst}"] = {
                "avg_latency": avg_lat,
                "success_rate": success_rate,
                "samples": len(info["latency"]),
            }
        return out

# ============================================================
# SWARM BUS (ZeroMQ PUB/SUB) with address-in-use handling
# ============================================================
        ...
        try:
            if self.mode == "bind":
                try:
                    ...
                    # Address in use: fall back to connect
                    self.pub.connect(self.endpoint.replace("*", "127.0.0.1"))
                try:
                    self.sub.bind(self.endpoint.replace("*", "0.0.0.0"))
                except self.zmq.ZMQError:
                    self.sub.connect(self.endpoint.replace("*", "127.0.0.1"))
            else:
...
class SwarmCoordinator:
    ...
    def generate_plan_local(self, target: str = "autonomous_project") -> Dict[str, Any]:
        ...
        risk = float(self.core.last_risk_score if hasattr(self.core, "last_risk_score") else 0.0)
...
class HybridBrainCore:
    def __init__(self, cfg_dict: Optional[Dict[str, Any]] = None):
        ...
        self.plugin_sandbox = PluginSandbox(os.path.join(os.path.dirname(sys.argv[0]), "plugins.py"))
    ...
    def cognition_step(self) -> OrganOutput:
        ...
        self.last_risk_score = risk.risk_score
...
def launch_tk_gui(core: HybridBrainCore):
    ...
    root = tk.Tk()
    root.title("HybridBrain v8.0 – Unified Hybrid Everything")
...
def parse_args(argv: List[str]) -> Dict[str, Any]:
    parser = argparse.ArgumentParser(description="HybridBrain v8.0 – Unified Hybrid Everything")
...
def main():
    ...
    if args["swarm_executor"]:
        ...
        print("[SwarmExecutor] Ready to execute SelfBuildOrgan plans.")
        run_headless(core)
    elif args["swarm_planner"]:
        threading.Thread(target=core.swarm_coordinator.autonomous_loop, args=("autonomous_project", args["planner_interval"]), daemon=True).start()
        if args["gui"]:
