...
GLOBAL_CONFIG = {
    ...
    "router_ckpt": os.environ.get("BORG_ROUTER_CKPT", "router_net.pt"),
}
...
class CodeGenerator:
    ...
    def ml_ranking(self, tree: ast.AST) -> ast.AST:
        ...
        comment = ast.Expr(
            value=ast.Constant(
                value=f"# ML router ranking active: {summary}, model={stats.get('router_model')}"
            )
        )
        if isinstance(tree, ast.Module):
            tree.body.insert(0, comment)
...
class BorgCompilerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Borg Compiler v8.2 - RouterNet Edition")
...
# ============================================================
# Forklift executor (sparsity, FP8, sharding, router)
# ============================================================
class ForkliftExecutor:
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        out["router_model"] = "RouterNet-MLP" if self.router_net is not None else "none"
...
def self_diagnostic_report(args):
    print("\n[Diagnostics] Borg v8.2 self-check")
...
def safe_argparse():
    parser = argparse.ArgumentParser(
        description="Borg Compiler v8.2 + Forklift Node - Universal Autopilot"
    )
    ...
    parser.add_argument("--router-save", type=str, default=None,
                        help="Path to save router checkpoint after run.")
    parser.add_argument("--router-load", type=str, default=None,
                        help="Path to load router checkpoint before run.")
    parser.add_argument("--router-train-only", action="store_true",
                        help="Run router-only training loop and exit.")
