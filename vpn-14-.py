...
"""
Borg Hybrid Overmind VPN Daemon (System + Browser VPN, Multi‑Signal Detection + Process Socket Mapping + QUIC Detection)
=============================================================================
...
- QUIC tunnel detection (UDP 443 from browser processes)
...
# Known VPN ASNs / IP hints (simplified; extend as needed)
...
# =========================
# Process-socket mapping + QUIC detection (Windows)
# =========================
...
def _parse_netstat():
    ...
    for line in out.splitlines():
        ...
        if proto not in ("tcp", "udp"):
            continue
        local = parts[1]
        remote = parts[2] if proto == "tcp" else parts[2]
        pid = parts[-1]
        try:
            pid_int = int(pid)
        except ValueError:
    ...
    while True:
        ...
        try:
            ...
            for proto, local, remote, pid in tcp + udp:
                ...
                if proto == "udp" and rport == "443":
                    quic_hits.append(hit_detail)

                if any(h in rhost.lower() for h in KNOWN_VPN_HOST_HINTS):
            ...
            if browser_hits:
                log(f"[PROC] Browser VPN activity detected via process sockets: {browser_hits}")
            if quic_hits:
                log(f"[PROC] QUIC VPN activity detected via UDP 443: {quic_hits}")
...
class ProxyHandler(socketserver.StreamRequestHandler):
    ...
    def learn_domain(self, domain):
        ...
        any_vpn = VPN.is_any_vpn_active()
        if any_vpn:
            server_name = VPN.get_current_vpn_server_name() or "browser-vpn"
            exit_ip = VPN.get_current_exit_ip() or get_public_ip_state().get("current_ip") or "0.0.0.0"
            ...
            log(f"[LEARN] {domain} via {server_name} ({exit_ip}) [any_vpn={any_vpn}]")
        else:
            log(f"[LEARN] Skipping {domain}, no VPN (system or browser) active")
            BORG.guardian_on_vpn_drop()
...
class BorgGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Borg VPN Overmind - System + Browser VPN (Multi-Signal + Process/QUIC)")
...
def main():
    log("=== Borg VPN Overmind starting (System + Browser VPN, Multi-Signal + Process/QUIC Detection) ===")
