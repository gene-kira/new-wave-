...
# -*- coding: utf-8 -*-
"""
LPU Autopilot Kernel - Unified Ultimate Monolith
...
def validate_autopilot_message(
    ...
    if msg_type == "NavigationCommand":
        ...
        if current_mode == VehicleMode.EMERGENCY_STOP:
            return False, "Cannot execute navigation while in EMERGENCY_STOP"

        return True, "NavigationCommand valid"

    if msg_type == "SystemQuery":
        qtype = payload.get("query_type")
        if qtype != "status":
            return False, "SystemQuery 'query_type' must be 'status'"
        return True, "SystemQuery valid"
    ...
    if msg_type == "SystemEvent":
        ...
        if etype == "stop_requested":
            return True, "Emergency stop event valid"
        return True, "SystemEvent valid"
