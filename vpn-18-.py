...
+ Ghost‑Tunnel TLS Pre‑Warm + Session Continuity
"""
...
class BorgMemory:
    ...
    def set_tls_fp(self, domain, tls_fp):
        with self.lock:
            d = self.data["domains"].get(domain)
            if not d:
                d = {
...
def register_tls_fingerprint(domain, fp):
    ...
    with PUBLIC_IP_LOCK:
        ...
        if old is None:
            ...
            fps[domain] = fp
    BORG_MEMORY.set_tls_fp(domain, fp)
...
# =========================
# Ghost‑Tunnel + Session Cookie Store
# =========================

COOKIE_LOCK = threading.Lock()
COOKIE_STORE = {}

def store_cookies(domain, cookie_header):
    if not domain or not cookie_header:
        return
    with COOKIE_LOCK:
        COOKIE_STORE[domain] = cookie_header

def get_cookies(domain):
    with COOKIE_LOCK:
        return COOKIE_STORE.get(domain)

class GhostTunnelManager:
    def __init__(self):
        self.enabled = True
    ...
    def prewarm(self, domain, exit_ip, tls_fp, cookies):
        if not self.enabled or not domain:
            return
        now = time.time()
        with self.state_lock:
            self.state["last_domain"] = domain
            self.state["last_exit_ip"] = exit_ip
            self.state["last_tls_fp"] = tls_fp
        ...
        try:
            ...
            if tls_fp:
                headers[FINGERPRINT_HEADER] = tls_fp
            if cookies:
                headers["Cookie"] = cookies
            req = urllib.request.Request(url, headers=headers)
            log(f"[GHOST] Pre‑warm HTTPS for {domain} via cached exit_ip={exit_ip} tls_fp={tls_fp} cookies={'yes' if cookies else 'no'}")
            with urllib.request.urlopen(req, timeout=3) as resp:
                _ = resp.read(128)
        except Exception as e:
            log(f"[GHOST] Pre‑warm for {domain} failed: {e}")

GHOST_TUNNEL = GhostTunnelManager()
...
class ProxyHandler(socketserver.StreamRequestHandler):
    def handle(self):
        try:
            ...
            if method.upper() == "CONNECT":
                ...
                self._ghost_tunnel_handshake(domain)
    ...
    def _ghost_tunnel_handshake(self, domain):
        if not domain:
            return
        profile = SITE_STORE.get_site(domain) or {}
        server_name = profile.get("preferred_vpn_server") or profile.get("vpn_server_name") or ""
        exit_ip = profile.get("last_exit_ip") or get_public_ip_state().get("current_ip") or "0.0.0.0"
        tls_fp = BORG_MEMORY.get_tls_fp(domain) or get_public_ip_state()["tls_fingerprints"].get(domain)
        cookies = get_cookies(domain)
        GHOST_TUNNEL.prewarm(domain, exit_ip, tls_fp, cookies)
    ...
    def handle_http(self, method, target, headers, raw_headers, domain):
        ...
        if cookie_header and domain:
            store_cookies(domain, cookie_header)
...
def main():
    log("=== Borg VPN Overmind starting (GPU Threat + Swarm Telemetry + LLM Anomaly + RT Browser VPN Saving) ===")
