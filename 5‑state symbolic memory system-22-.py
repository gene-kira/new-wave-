...
"""
BORG OS v21 – One-Click Approval Window + 5-Minute Safe Mode + Full Game Protection
Gaming-Safe Unified Accelerator (CPU/GPU/NPU) + RL/DPI/Threat + Mesh + Command Center
...
# ============================================================
#  STORAGE + AUTO MIGRATION + SETTINGS
# ============================================================
...
# ============================================================
#  OS GOVERNOR (GAMING-SAFE, HARD NEVER-KILL FOR borg_games + ASK-BEFORE-KILL + 5-MIN WINDOW)
# ============================================================

class BorgOSGovernor:
    def __init__(self, logger=None, storage=None, event_bus=None,
                 persona=BorgPersona.PASSIVE, deep_rl=None,
        ...
        # Global approval window: after first user-confirmed kill, 5 minutes of safe mode
    ...
    def apply(self, states):
        ...
        try:
            ...
            # If we are inside the 5-minute approval window, we do NOT kill anything.
            # We only monitor and treat processes as safe/learned.
            ...
            for i, state in enumerate(states):
                ...
                for p in procs[start:end]:
                    try:
                        ...
                        # Global approval window: everything is treated as safe/monitored.
                        ...
                        # Hard NEVER-KILL for games and launchers
    ...
    def _kill_process(self, p, score, gpu_load, hard_protected, learned_safe, sigs, gpu_mem, overloaded):
        try:
            ...
            # User approved ONE kill: start 5-minute global approval window
...
class BorgCore:
    def __init__(self, size=10, logger=None, storage=None, governor=None, event_bus=None, persona=BorgPersona.PASSIVE):
...
# ============================================================
#  GLOBAL INSTANCES + GAME SCAN INTEGRATION
# ============================================================
...
governor = BorgOSGovernor(logger=lambda msg: print(msg), storage=storage, event_bus=event_bus,
                          persona=BorgPersona.PASSIVE, deep_rl=deep_rl,
...
core = BorgCore(size=10, logger=lambda msg: print(msg), storage=storage, governor=governor,
                event_bus=event_bus, persona=BorgPersona.PASSIVE)
...
if FASTAPI_AVAILABLE:
    app = FastAPI(title="Borg OS Node", version="21.0.0")
...
class BorgGUI:
    def __init__(self, root, core: BorgCore):
        ...
        root.title("BORG OS NODE v21 – Command Center (Accelerator + Threat + Deep RL + Mesh + DPI + Games + Safe Window)")
...
def run_cli():
    print("BORG OS NODE (CLI MODE, Autonomous v21, Accelerator + Threat + Deep RL + Mesh + DPI, Gaming-Safe + One-Time Scan + Incremental Detection + NEVER-KILL borg_games + ASK-BEFORE-KILL + 5-Min Safe Window)")
    print(f"OS: {OS_NAME}")
