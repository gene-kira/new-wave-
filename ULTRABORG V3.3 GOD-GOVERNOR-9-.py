#!/usr/bin/env python3
"""
ULTRABORG V5.x GOD-GOVERNOR + NODE RPC (HYBRID MERGE, LEADER/FOLLOWER CLUSTER)

- Neural uncertainty engine
- Water-state visualization hooks
- Multi-node hive-mind coordination
- Real-time entropy mapping
- Memory-based personality drift
- Borg RL governor (DQN + PPO + Oracle)
- Predictive model (Bernoulli risk)
- Security chain engine + global risk field
- Cluster manager with leader/follower roles
- HF text-generation node with ForkliftLinear patch
- TinyFallback model for safe fallback
"""
...
DEFAULT_CONFIG = {
    ...
    "cluster": {
        "enabled": False,  # SAFE_MODE: off by default
...
class ClusterNode:
    def __init__(self, cfg: Dict[str, Any]):
        ...
        if self.enabled and not SAFE_MODE:
            ...
            print("[CLUSTER] SAFE_MODE: cluster disabled")
    ...
    def loop(self):
        while self.enabled:
            try:
                ...
                self.hive_state[addr[0]] = payload
...
# =========================
# Borg cluster manager (leader/follower)
# =========================

class BorgClusterManager:
    def __init__(self, cluster: ClusterNode):
        self.cluster = cluster
        self.nodes_state: Dict[str, Dict[str, Any]] = {}
        self.lock = threading.Lock()
        self.last_leader_action: int = 0

    def update_local_node(
        self,
        node_id: str,
        phase: str,
        temp: float,
    ...
    def ingest_cluster_payloads(self):
        with self.lock:
            for addr, payload in self.cluster.hive_state.items():
                self.nodes_state[addr] = {
                    "phase": payload.get("phase", "neutral"),
                    "temp": float(payload.get("temp", 0.0)),
                    "fps": float(payload.get("fps", 0.0)),
                    "stress": 0.0,
                    "global_risk": float(payload.get("global_risk", 0.0)),
                    "uncertainty": float(payload.get("uncertainty", 0.0)),
                    "action_hint": int(payload.get("action_hint", 0)),
                    "ts": time.time(),
                }
                self.last_leader_action = int(payload.get("action_hint", self.last_leader_action))

    def cluster_consensus(self) -> Dict[str, Any]:
        with self.lock:
            ...
            n = len(self.nodes_state)
            phase_counts = {"flow": 0, "strain": 0, "recovery": 0, "neutral": 0}
            temp_sum = fps_sum = stress_sum = risk_sum = unc_sum = 0.0
            for _, st in self.nodes_state.items():
                ph = st.get("phase", "neutral")
                phase_counts[ph] = phase_counts.get(ph, 0) + 1
                temp_sum += st.get("temp", 0.0)
                fps_sum += st.get("fps", 0.0)
                stress_sum += st.get("stress", 0.0)
                risk_sum += st.get("global_risk", 0.0)
                unc_sum += st.get("uncertainty", 0.0)

            phase = max(phase_counts.items(), key=lambda kv: kv[1])[0]
            consensus = {
                "phase": phase,
                "avg_temp": temp_sum / n,
                "avg_fps": fps_sum / n,
                "avg_stress": stress_sum / n,
                "avg_global_risk": risk_sum / n,
                "avg_uncertainty": unc_sum / n,
            }
            LOGGER.log("cluster_consensus", consensus)
            return consensus

    def cluster_hint_for_local(self) -> Dict[str, Any]:
        cons = self.cluster_consensus()
        phase = cons["phase"]
        risk = cons["avg_global_risk"]
        unc = cons["avg_uncertainty"]
        ...
        hint = {
            ...
            "risk_boost": 0.3 * risk,
            "uncertainty_boost": 0.3 * unc,
        }
        LOGGER.log("cluster_hint", hint)
        return hint

    def get_leader_action_or_default(self, brain: HybridAgent, state: np.ndarray, epsilon: float) -> int:
        if self.cluster.role == "follower" and self.last_leader_action is not None:
            LOGGER.log("cluster_follow_action", {"action": self.last_leader_action})
            return self.last_leader_action
        return brain.act(state, epsilon=epsilon)

BORG_CLUSTER = BorgClusterManager(CLUSTER)

# =========================
# Consensus / security / risk
# =========================
...
class PredictiveModelRL(torch.nn.Module):
...
PREDICTIVE = PredictiveModelRL(STATE_DIM, ACTION_DIM)
...
# =========================
# Thermal / global risk / water state
# =========================
...
# =========================
# Borg main loop (leader/follower)
# =========================
...
def run_borg_loop():
    ...
    while True:
        ...
        if CONFIG["dream"]["enabled"] and step_count > 0 and step_count % CONFIG["dream"]["interval_steps"] == 0:
            if STATE_PHYSICS.should_dream() and CLUSTER.role == "leader":
        ...
        if CLUSTER.role == "leader":
            ...
            raw_action = BORG_CLUSTER.get_leader_action_or_default(brain, s, epsilon)
        ...
        global_risk_value = compute_global_risk_value()
        ...
        BORG_CLUSTER.update_local_node(
            node_id="local-node",
            phase=STATE_PHYSICS.phase,
            temp=snap.gpu_temp,
            fps=snap.fps,
            ...
            action_hint=raw_action,
        )
        BORG_CLUSTER.ingest_cluster_payloads()
        cluster_hint = BORG_CLUSTER.cluster_hint_for_local()
        stress += cluster_hint["phase_bias"]
        STATE_PHYSICS.update_phase(stress)
        global_risk_value += cluster_hint["risk_boost"]
        uncertainty_value += cluster_hint["uncertainty_boost"]
        ...
        if CLUSTER.role == "leader" and len(traj["s"]) >= CONFIG["rl"]["trajectory_len"]:
            stats = TEACHER.agent.ppo_update(traj)
            LOGGER.log("ppo_update_main", stats)
            traj = {"s": [], "a": [], "r": [], "logp": [], "v": []}
...
def train_policy_net_step(telemetry: dict, latency_ms: float):
    # stub: already training in Borg loop
    pass
...
def generate_text(prompt: str, max_new_tokens: int = 128, node_id: str = "node-1") -> Tuple[str, dict]:
    load_model()
        ...
        while True:
            ...
            if b"\n" in buf:
                ...
                try:
                    ...
                    cluster_cons = BORG_CLUSTER.cluster_consensus()

                    resp = {
                        "text": text,
                        "stats": stats,
                        "cluster_consensus": cluster_cons,
                    }
                except Exception as e:
...
def run_local_cli():
    ...
    while True:
        ...
        for k, v in stats.items():
            print(f"{k}: {v}")
        print("\n--- Cluster ---")
        print("consensus:", BORG_CLUSTER.cluster_consensus())
...
def main():
    ...
    if args.headless_node:
        print("[Node] Running in headless Borg+RPC mode.")
        try:
            while True:
                time.sleep(3600)
