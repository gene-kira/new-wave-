...
PEER_QUEENS = args.peer_queen  # list of "host:api_port"
...
DEFAULT_CONFIG = {
    ...
    "strictness": 1,
    "stress_enabled": True,
    "node_class": None,  # auto-detect if None: gpu | storage | general
    "autoscale": {
        "enabled": True,
        "cpu_high": 75,
        "cpu_low": 25,
    ...
    with CONFIG_LOCK:
        ...
        if os.path.exists(CONFIG_PATH):
            try:
                ...
                # shallow merge, then fix ml.per_class defaults
                cfg.update(user_cfg)
                if "ml" in user_cfg:
...
def detect_node_class():
    ...
    if HAS_GPU or HAS_AMD or HAS_INTEL_GPU:
        return "gpu"
    # if we have many drives or large disk, call it storage
...
ML_TRAINING_STATE = {
    ...
    "status": "idle",
    "per_class": {}  # class -> {last_model_hash, last_dataset_hash, status}
}
...
# ============================================================
#  METRICS + ML (Z-SCORE + DEEP + ISO FOREST)
# ============================================================

ML_MODEL = None
ML_MODEL_HASH = None
ML_MODEL_TYPE = None  # per-node-class effective type
...
def ml_load_model_if_needed():
    ...
    try:
        ...
        if model_type == "zscore" and mtype == "zscore":
            ...
            print(f"[{NAME}] Loaded z-score ML model {model_hash} for class {NODE_CLASS}")
        elif model_type == "iso_forest" and mtype == "iso_forest":
            ML_MODEL = obj
            ML_MODEL_HASH = model_hash
            ML_MODEL_TYPE = "iso_forest"
            print(f"[{NAME}] Loaded Isolation Forest model {model_hash} for class {NODE_CLASS}")
        elif model_type in ("mlp", "autoencoder", "lstm", "transformer") and mtype == "deep" and HAS_TORCH:
            arch = obj["arch"]
            keys = obj["keys"]
            weights = obj["weights"]
            if arch == "mlp":
            ...
            print(f"[{NAME}] Loaded deep ML model {model_hash} ({arch}) for class {NODE_CLASS}")
        else:
    ...
    try:
        if ML_MODEL_TYPE == "zscore":
            ...
            if not path_lengths:
                return
            import math
            score = math.exp(-avg_h / c_n) if (avg_h := sum(path_lengths) / len(path_lengths)) else 0.0

        elif ML_MODEL_TYPE in ("mlp", "autoencoder", "lstm", "transformer") and HAS_TORCH:
            arch = ML_MODEL["arch"]
            keys = ML_MODEL["keys"]
            net = ML_MODEL["net"]
            vals = [float(WORKER_STATE.get(k, 0)) for k in keys]
            if arch == "autoencoder":
                ...
                score = float(abs(mse))
            elif arch in ("lstm", "transformer"):
                x = torch.tensor([[vals]], dtype=torch.float32)  # (B=1, T=1, F)
                with torch.no_grad():
                    out = net(x).item()
                score = float(abs(out))
            else:  # mlp
...
# ============================================================
#  QUEEN JOB GENERATOR / AUTOSCALE / ML TRAINING
# ============================================================
...
def queen_train_ml_model_for_class(node_class):
    ...
    try:
        ...
        per_class_cfg = ml_cfg.get("per_class", {})
        class_cfg = per_class_cfg.get(node_class, {})
        model_type = class_cfg.get("model_type", ml_cfg.get("default_model_type", "zscore"))

        # ---- z-score or fallback ----
        if model_type == "zscore" or not HAS_TORCH:
            cols = len(keys)
            sums = [0.0] * cols
            sums2 = [0.0] * cols
            n = len(rows)
            ...
            model_bytes = json.dumps(model, indent=2).encode()
        ...
        # ---- Deep models (MLP / AE / LSTM / Transformer) ----
        else:
            ...
            if model_type != "zscore" and HAS_TORCH:
        ...
        with ML_LOCK:
            ...
            ML_TRAINING_STATE["per_class"][node_class]["status"] = "trained"
        print(f"[QUEEN {NAME}] ML model ({model_type}) trained for class {node_class} and stored as {model_hash}")
    except Exception:
        traceback.print_exc()
        with ML_LOCK:
