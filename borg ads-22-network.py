#!/usr/bin/env python3
"""
ULTRABORG V8.0 — GAMING-FIRST FULL SYSTEM GOVERNOR + WAMI/WADI
(RL + DEEP RL + WATCHDOG + ANOMALY + ADBLOCK + GUI + EVOLUTION + SUPERVISOR +
 ...
 AUTO GAME MODE + ADVANCED NETWORK SCANNER (AUTO CIDR, ARP, ICMP, TCP, WAMI PORTS 1–65535) +
 WAMI (Wide Area Motion/Data Imagery) TELEMETRY + WADI FUSION ENGINE +
...
DEFAULT_CONFIG = {
    ...
    "network": {
        ...
        "wami": {
            ...
            "low_density_ports": [22, 53, 80, 443],
            "full_scan_top_hosts": 4,
            "full_scan_interval_sec": 300
        }
    },
    "patch": {
    ...
    "telemetry": {
        "wami_frame_interval_sec": 10.0,
        "wadi_frame_interval_sec": 2.0
    }
}
...
def load_config() -> Dict[str, Any]:
    ...
    print("[CONFIG] Using embedded default config (gaming-first, V8.0 WAMI/WADI)")
...
def get_proc_info(proc):
    ...
    for old_pid, old_info in list(PROCESS_CACHE.items()):
        ...
        if old_name == name and old_path == path and old_pid != pid:
            resurrected = True
...
def cleanup_cache(current_pids: List[int]):
    ...
    for pid in list(PROCESS_CACHE.keys()):
        if pid not in current_pids:
            info = PROCESS_CACHE[pid]
            if now - info.get("last_seen", 0) > 60.0:
...
# ============================================================
# NETWORK: AUTO CIDR + ARP + ICMP + TCP + WAMI PORTS
# ============================================================
...
class NetworkScanner:
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # WAMI-style port sampling config
        wami_cfg = cfg.get("wami", {})
        self.wami_max_ports_per_host = wami_cfg.get("max_ports_per_host", 128)
        self.wami_high_density_threshold = wami_cfg.get("high_density_threshold", 0.7)
        self.wami_low_density_ports = wami_cfg.get("low_density_ports", [22, 53, 80, 443])
        self.wami_full_scan_top_hosts = wami_cfg.get("full_scan_top_hosts", 4)
        self.wami_full_scan_interval = wami_cfg.get("full_scan_interval_sec", 300)
        self.last_full_scan_ts = 0.0
    ...
    def start(self):
        print("[NET] NetworkScanner starting (WAMI-enabled)")
        self.thread.start()
    ...
    def _get_ports_for_ip(self, ip: str, prev_score: float) -> List[int]:
        """
        WAMI-style adaptive port sampling:
        - Low score: small set of canonical ports
        - High score: dense sampling including random ports
        - Occasionally: full 1–65535 scan for top hosts
        """
        ports = set(self.wami_low_density_ports)
        if prev_score >= self.wami_high_density_threshold:
            # High-threat host: add dense sampling
            while len(ports) < self.wami_max_ports_per_host:
                ports.add(random.randint(1, 65535))
        else:
            # Normal host: add a few extra ports from baseline list
            for p in self.tcp_ports:
                ports.add(p)
        return sorted(ports)

    def _scan_ip(self, ip: str, arp_mac: str = "", prev_score: float = 0.0) -> Dict[str, Any]:
        alive = self._ping(ip)
        open_ports = []
        if alive:
            ports_to_probe = self._get_ports_for_ip(ip, prev_score)
    ...
    def _score_device(self, dev: Dict[str, Any]) -> float:
        ...
        if len(ports) > 3:
            score += 0.3
        # WAMI: more open ports => higher motion/interest
        if len(ports) > 20:
            score += 0.2
        return min(1.0, score)

    def _full_port_scan_cycle(self, devices: List[Dict[str, Any]]):
        """
        WAMI full-port scan: pick top N hosts by score and scan 1–65535.
        This is throttled by wami_full_scan_interval to avoid hammering router.
        """
        now = time.time()
        if now - self.last_full_scan_ts < self.wami_full_scan_interval:
            return
        self.last_full_scan_ts = now
        if not devices:
            return
        sorted_devs = sorted(devices, key=lambda d: d.get("score", 0.0), reverse=True)
        top = sorted_devs[:self.wami_full_scan_top_hosts]
        for dev in top:
            ip = dev.get("ip", "")
            if not ip:
                continue
            print(f"[WAMI] Full-port scan for {ip}")
            open_ports = []
            for p in range(1, 65536):
                if self._probe_tcp(ip, p, timeout=0.05):
                    open_ports.append(p)
            dev["ports"] = sorted(set(dev.get("ports", [])) | set(open_ports))
            dev["score"] = self._score_device(dev)
            LOGGER.log("wami_full_scan", {"ip": ip, "open_ports": len(open_ports)})
    ...
    def loop(self):
        while self.running:
            try:
                ...
                prev_snapshot = {}
                with self.lock:
                    prev_snapshot = dict(self.devices)
                for ip in hosts:
                    prev_score = prev_snapshot.get(ip, {}).get("score", 0.0)
                    dev = self._scan_ip(ip, arp_mac=arp.get(ip, ""), prev_score=prev_score)
                    if dev:
                ...
                # WAMI full-port cycle
                self._full_port_scan_cycle(devices)
                with self.lock:
...
# ============================================================
# WADI FUSION ENGINE (Wide Area Data Imagery)
# ============================================================

class WADIFusionEngine:
    """
    Fuses all telemetry into a single frame:
    - Sensors
    - Network (WAMI devices)
    - RL state/action
    - Firewall decisions (via penalties)
    """
    def __init__(self, cfg: Dict[str, Any]):
        self.cfg = cfg
        self.interval = cfg.get("wadi_frame_interval_sec", 2.0)
        self.running = True
        self.thread = threading.Thread(target=self.loop, daemon=True)
    ...
    def start(self):
        print("[WADI] Fusion engine starting")
        self.thread.start()

    def build_frame(self, snap: SensorSnapshot, rl_reward: float, rl_action: str, net_info: Dict[str, float]):
        ...
        frame = {
            ...
            "temp": snap.gpu_temp,
            "fps": snap.fps,
            "frametime_var": snap.frametime_var,
            "vram": snap.vram_usage,
            "power": snap.gpu_power,
            "rl_reward": rl_reward,
            "rl_action": rl_action,
            "net_health": net_info.get("net_health", 1.0),
            "lan_stability": net_info.get("lan_stability", 1.0),
            "latency_score": net_info.get("latency_score", 1.0),
            "latency_jitter": net_info.get("latency_jitter", 0.0),
            "devices": devices
        }
        with self.lock:
            self.last_frame = frame
        LOGGER.log("wadi_frame", frame)
    ...
    def loop(self):
        # Passive loop: main loop will push frames; here we just keep interval heartbeat
        while self.running:
            time.sleep(self.interval)

WADI = WADIFusionEngine(CONFIG["telemetry"])

# ============================================================
# WAMI TELEMETRY SYSTEM (Wide Area Motion/Data Imagery)
# ============================================================

class WAMITelemetrySystem:
    """
    Builds wide-area snapshots of LAN + ports as a single WAMI frame.
    This is more "imagery-like": one frame = all hosts + port motion.
    """
    def __init__(self, cfg: Dict[str, Any]):
        self.cfg = cfg
        self.interval = cfg.get("wami_frame_interval_sec", 10.0)
        self.running = True
        self.thread = threading.Thread(target=self.loop, daemon=True)
        self.last_frame: Dict[str, Any] = {}
        self.lock = threading.Lock()
    ...
    def start(self):
        print("[WAMI] Telemetry system starting")
        self.thread.start()

    def build_frame(self):
        devices = NETWORK_SCANNER.get_snapshot()
        # Build a compact WAMI frame: each device with ports and score
        frame_devices = []
        for d in devices:
            frame_devices.append({
                "ip": d.get("ip", ""),
                "label": d.get("label", ""),
                "ports": d.get("ports", []),
        ...
        frame = {
            ...
            "lan_stability": NET_HISTORY.stability_score()
        }
        with self.lock:
            self.last_frame = frame
        LOGGER.log("wami_frame", frame)
    ...
    def loop(self):
        while self.running:
            try:
                ...
                print(f"[WAMI] Telemetry loop error: {e}")
            time.sleep(self.interval)

WAMI = WAMITelemetrySystem(CONFIG["telemetry"])
...
class BorgRLCore:
    ...
    def select_action(self, state: SensorSnapshot) -> Tuple[str, Dict[str, float], float]:
...
class MobileRemoteHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/telemetry":
            snap = SENSORS.snapshot()
            frame = WADI.get_last_frame()
            self._send_json({
                "cpu": snap.cpu_usage,
                "gpu": snap.gpu_usage,
                "temp": snap.gpu_temp,
                "fps": snap.fps,
                "net_health": frame.get("net_health", 1.0),
                "latency_score": frame.get("latency_score", 1.0),
                "latency_jitter": frame.get("latency_jitter", 0.0)
            })
        elif self.path == "/wami":
            frame = WAMI.get_last_frame()
...
class BorgGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("ULTRABORG V8.0 — Gaming-First System Governor (WAMI/WADI)")
    ...
    def _build_ui(self):
        ...
        self.net_tree.column("ports", width=220, anchor="w")
        ...
        labels = {
            ...
            "rl": ttk.Label(tel_frame, text="RL Reward: "),
            "act": ttk.Label(tel_frame, text="RL Action: "),
        }
        row = 0
        ...
        notebook.add(net_frame, text="Network (WAMI)")
        notebook.add(tel_frame, text="Telemetry (WADI)")
    ...
    def _telemetry_loop(self):
        ...
        while True:
            try:
                ...
                self.telemetry_labels["rl"].configure(text=f"RL Reward: {frame.get('rl_reward', 0.0):.2f}")
                self.telemetry_labels["act"].configure(text=f"RL Action: {frame.get('rl_action', 'noop')}")
            except Exception:
                pass
            time.sleep(3.0)
...
def borg_main_loop():
    print("[BORG] Main loop starting (gaming-first, V8.0 WAMI/WADI)")
    KERNEL_DRIVER.init()
    UPDATER.start()
    WATCHDOG.start()
    ...
    SUPERVISOR.register("wadi", WADI.thread)
    SUPERVISOR.register("wami", WAMI.thread)
    SUPERVISOR.start()
    ...
    while True:
        try:
            ...
            action, net_info, reward = RL_CORE.select_action(snap)
            RL_CORE.apply_action(action, snap)

            # WADI fusion frame
            WADI.build_frame(snap, reward, action, net_info)
