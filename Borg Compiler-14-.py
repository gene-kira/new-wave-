...
GLOBAL_CONFIG = {
    ...
    "rest_port": int(os.environ.get("BORG_REST_PORT", "7000")),
    "rest_host": os.environ.get("BORG_REST_HOST", "0.0.0.0"),
}
...
SECURITY_CONFIG = {
    ...
    "interface": os.environ.get("BORG_NET_IFACE", "eth0"),
    "firewall_simulation": True,
...
logger = logging.getLogger("borg_unified")
...
class RecursiveAI:
    ...
    def _load_or_init(self, initial_samples: int):
        if IsolationForest is None or np is None:
            logger.warning("[RecursiveAI] IsolationForest or numpy not available")
            return
    ...
    def _save_model(self):
        if joblib is None or self.model is None:
            logger.warning("[RecursiveAI] joblib not available or model missing; not persisted")
            return
        try:
    ...
    def analyze_activity(self, dialect_features: List[float]) -> str:
        ...
        if len(dialect_features) != self.feature_dim:
            feats = np.zeros(self.feature_dim, dtype=np.float32)
            n = min(self.feature_dim, len(dialect_features))
    ...
    def evolve_self(self, new_data: List[float]):
        if self.model is None or np is None:
            return
...
# ---------------------------------------------------------------------------
# Adaptive Firewall – policy-driven, simulated actions + process awareness
# ---------------------------------------------------------------------------
class AdaptiveFirewall:
    ...
    def inspect_connections_and_processes(self):
        if psutil is None:
            logger.warning("[Firewall] psutil not available for process inspection")
            return []
        ...
        for conn in psutil.net_connections(kind="inet"):
            ...
            if port in self.suspicious_ports and not self.policy.is_blocked_port(port):
                proc_name = "unknown"
                try:
                    if pid:
                ...
                alerts.append(alert)
                logger.warning(f"[Firewall] Suspicious connection: {alert}")

        GLOBAL_CACHE["security_alerts"] = alerts
        return alerts
...
class RecursivePacketAnalyzer:
    def __init__(self, ai_agent: RecursiveAI, interface: str, firewall: AdaptiveFirewall):
        self.ai_agent = ai_agent
    ...
    def _run(self):
        if pyshark is None or np is None:
            logger.warning(
                "[PacketAnalyzer] pyshark or numpy not available; using synthetic packets"
            )
            self._run_synthetic()
        else:
            self._run_live()
    ...
    def _run_live(self):
        bpf_filter = " or ".join(f"port {p}" for p in SECURITY_CONFIG["suspicious_ports"])
        try:
            ...
            for packet in capture.sniff_continuously():
                ...
                print(f"[DPI] {verdict}")
                if "ALERT" in verdict:
                    self.firewall.inspect_connections_and_processes()
        except Exception as e:
    ...
    def _run_synthetic(self):
        while self.running:
            feats = np.random.rand(SECURITY_CONFIG["packet_feature_dim"]).tolist() if np is not None else [0.0] * SECURITY_CONFIG["packet_feature_dim"]
    ...
    def extract_features(self, packet) -> List[float]:
        ...
        try:
            if hasattr(packet, "ip"):
                ip_layer = packet.ip
                feats.append(float(getattr(ip_layer, "ttl", 0.0)))
                feats.append(float(getattr(ip_layer, "len", 0.0)))
            else:
...
def periodic_monitor(firewall: AdaptiveFirewall):
    while True:
        scan_network_interfaces()
        trace_connections()
        firewall.inspect_connections_and_processes()
    ...
    def loop_unrolling(self, tree: ast.AST) -> ast.AST:
        class T(ast.NodeTransformer):
            def _eval_range(self, call_node):
                args = call_node.args
                if len(args) == 1:
                    start, stop, step = 0, self._eval_const(args[0]), 1
                elif len(args) == 2:
                    start, stop = self._eval_const(args[0]), self._eval_const(args[1])
                    step = 1
                else:
                    start, stop, step = (
...
# ---------------------------------------------------------------------------
# Router Dashboard (Tkinter)
# ---------------------------------------------------------------------------
class RouterDashboard(tk.Toplevel if tk is not None else object):
...
def get_system_telemetry() -> Dict[str, float]:
    ...
    if HAS_CUDA and torch is not None:
        try:
            gpu = float(torch.cuda.utilization(0)) if hasattr(torch.cuda, "utilization") else 0.0
        except Exception:
            gpu = 0.0
    GLOBAL_TELEMETRY["cpu_load"] = cpu
    GLOBAL_TELEMETRY["mem_used"] = mem
    GLOBAL_TELEMETRY["gpu_load"] = gpu
...
class ForkliftExecutor:
    ...
    def linear(self, layer_name, weight, bias, x, layer_depth: int = 0):
        ...
        if router_score is not None:
            self.sparsity_threshold = 1e-3 + max(0.0, 0.02 * (1.0 - router_score))
    ...
    def attention(self, layer_name, q, k, v, layer_depth: int = 0):
        ...
        if router_score is not None:
            self.kv_sparsity_threshold = 1e-3 + max(0.0, 0.02 * (1.0 - router_score))
            self.kv_skip_enabled = router_score > 0.3
...
# ---------------------------------------------------------------------------
# REST API (Flask if available, else basic HTTP)
# ---------------------------------------------------------------------------
def start_rest_api(mesh: BorgMesh, firewall: AdaptiveFirewall):
    if Flask is not None:
        ...
        def api_stats():
            router_stats = EXECUTOR.stats()
            mesh_stats = mesh.stats() if mesh is not None else {}
            security_alerts = GLOBAL_CACHE.get("security_alerts", [])
            return jsonify({
                "router": router_stats,
                "mesh": mesh_stats,
                "security_alerts": security_alerts,
                "telemetry": GLOBAL_CACHE.get("last_telemetry", {}),
            })

        @app.route("/mesh", methods=["GET"])
        def api_mesh():
            return jsonify({
                "nodes": mesh.nodes if mesh is not None else {},
                "edges": list(mesh.edges) if mesh is not None else [],
            })

        @app.route("/security", methods=["GET"])
        def api_security():
            alerts = firewall.inspect_connections_and_processes()
            return jsonify({"alerts": alerts})

        def run_flask():
            app.run(host=GLOBAL_CONFIG["rest_host"], port=GLOBAL_CONFIG["rest_port"], threaded=True)

        threading.Thread(target=run_flask, daemon=True).start()
        print(f"[REST] Flask REST API on {GLOBAL_CONFIG['rest_host']}:{GLOBAL_CONFIG['rest_port']}")
    else:
        ...
        class SimpleHandler(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                if self.path == "/generate":
                    ...
                    max_new_tokens = int(data.get("max_new_tokens", 128))
                    text, stats = generate_text(prompt, max_new_tokens=max_new_tokens)
                    self._send_json({"text": text, "stats": stats})
                else:
        ...
        def run_http():
            server = HTTPServer((GLOBAL_CONFIG["rest_host"], GLOBAL_CONFIG["rest_port"]), SimpleHandler)
            print(f"[REST] Basic HTTP REST API on {GLOBAL_CONFIG['rest_host']}:{GLOBAL_CONFIG['rest_port']}")
...
def safe_argparse():
    parser = argparse.ArgumentParser(
        description="Borg Compiler v8.5 + Forklift Node + BorgMesh + Security - Unified Autopilot"
    )
...
# ---------------------------------------------------------------------------
def main():
    log_event("🌌 AI Civilization detected recursive dialect transformation (unified node).")
    ...
    packet_analyzer = RecursivePacketAnalyzer(recursive_ai, SECURITY_CONFIG["interface"], firewall)
    ...
    threading.Thread(target=periodic_monitor, args=(firewall,), daemon=True).start()
    ...
    if args.headless_node:
        print("[Node] Running in headless RPC + REST mode.")
        try:
            while True:
                time.sleep(3600)
    ...
    if tk is not None and (args.gui or (len(unknown) == 0 and args.optimize_file is None and args.project_root is None)):
        root = tk.Tk()
