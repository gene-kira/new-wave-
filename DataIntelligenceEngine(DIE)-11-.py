# ===========================================================
# BORG OS v34 — Full Autopilot Evolution, Meta-Optimizer, Intent Engine
...
PROFILE_PATH = os.path.join(BASE_DIR, "intelligence_profile_v34.json")
...
INSIGHT_EXPORT_PATH = os.path.join(BASE_DIR, "insight_export.json")
DECISION_GRAPH_PATH = os.path.join(BASE_DIR, "decision_graph.json")
...
def log(msg: str):
    ...
    print(line)
    try:
        with open(os.path.join(LOG_DIR, "borg_v34.log"), "a", encoding="utf-8") as fp:
            fp.write(line + "\n")
    except Exception:
        pass
...
def _subscribe_memory_graph():
    for et in [
        ...
        "autopilot.profile_applied", "kernel.mode_changed", "intent.changed"
    ]:
...
# ===========================================================
# DECISION GRAPH — Why BORG Did What It Did
# ===========================================================
class DecisionGraph:
    def __init__(self):
    ...
    def _load(self):
        if os.path.exists(DECISION_GRAPH_PATH):
            try:
                with open(DECISION_GRAPH_PATH, "r", encoding="utf-8") as fp:
                    data = json.load(fp)
                self.decisions = data.get("decisions", [])
                log("[DecisionGraph] Loaded decision graph.")
            except Exception as e:
                log(f"[DecisionGraph] Failed to load decision graph: {e}")

    def _save(self):
        try:
            with self.lock:
                data = {"decisions": self.decisions[-1000:]}
            with open(DECISION_GRAPH_PATH, "w", encoding="utf-8") as fp:
                json.dump(data, fp, indent=4)
            log("[DecisionGraph] Saved decision graph.")
        except Exception as e:
            log(f"[DecisionGraph] Failed to save decision graph: {e}")

    def record_decision(self, source: str, action: str, evidence: Dict[str, Any], confidence: float, alternatives: List[Dict[str, Any]]):
        entry = {
            "ts": time.time(),
            "source": source,
            "action": action,
        ...
        self._save()
        EVENT_BUS.emit("decision.recorded", {"entry": entry})
...
SWARM_SHARED_KEY = b"borg_v34_shared_key"
    ...
    def _save_state(self):
        try:
            with open(RL_STATE_PATH, "w", encoding="utf-8") as fp:
                json.dump({
                    "experiences": self.experiences[-500:],
...
class KernelState:
    ...
    def set_intent(self, intent: str):
        ...
        self._save()
        EVENT_BUS.emit("intent.changed", {"intent": intent})
...
# ===========================================================
# INSIGHT EXPORT ENGINE
# ===========================================================
class InsightExportEngine:
    def __init__(self):
        self.last_export: Dict[str, Any] = {}

    def export(self, organism_state: Dict[str, Any], recent_actions: List[Dict[str, Any]], decisions: List[Dict[str, Any]]):
        payload = {
            "ts": time.time(),
        ...
        try:
            with open(INSIGHT_EXPORT_PATH, "w", encoding="utf-8") as fp:
                json.dump(payload, fp, indent=4)
            log("[Insight] Exported insight summary.")
        except Exception as e:
            log(f"[Insight] Failed to export insight: {e}")


INSIGHT_ENGINE = InsightExportEngine()


# ===========================================================
# INTENT ENGINE — User Intent & Context
# ===========================================================
class IntentEngine:
    def __init__(self):
        self.intent = KERNEL_ENGINE.state.get("intent", "performance")
        self.lock = threading.Lock()

    def set_intent(self, intent: str):
        with self.lock:
            self.intent = intent
        KERNEL_ENGINE.set_intent(intent)

    def get_intent(self) -> str:
        with self.lock:
            return self.intent

    def map_intent_to_profile(self) -> str:
...
# ===========================================================
# BORG ORGANISM CORE — v34
# ===========================================================
class BORGOrganism:
    def __init__(self):
        log("[Organism] Initializing organism core v34...")
        ...
        self.module_lineage: Dict[str, List[str]] = {}
        ...
        log("[Organism] Core v34 initialized.")
    ...
    def apply_policy(self, policy: Dict[str, Any], source: str = "module", confidence: float = 0.7, evidence: Dict[str, Any] = None, alternatives: List[Dict[str, Any]] = None):
        ...
        if len(self.recent_actions) > 200:
            self.recent_actions.pop(0)

        DECISION_GRAPH.record_decision(
            source=source,
            action=str(policy),
        ...
        EVENT_BUS.emit("organism.policy_applied", {"policy": policy, "source": source, "confidence": confidence})
    ...
    def _update_fitness(self, name: str, output: Dict[str, Any], duration: float):
        ...
        if isinstance(output, dict):
            ...
            if "policy" in output:
                score += 3.0
                evidence = {"output": output, "duration": duration}
                self.apply_policy(output["policy"], source=name, confidence=0.8, evidence=evidence)
            if "error" in output:
                score -= 3.0
        if duration > 1.0:
            score -= (duration - 1.0)
    ...
    def _scheduler_loop(self):
        while True:
            if not self.scheduler_running:
                return
            try:
                log("[Organism] Scheduler tick v34...")
    ...
    def shutdown(self):
        log("[Organism] Shutting down v34...")
        self.scheduler_running = False
...
class DataIntelligenceEngine:
    ...
    def _design_new_modules(self) -> List[Dict[str, Any]]:
        ...
        tm = self.unified_dataset.get("threat_matrix", {})
        blueprints.append({
            "name": "SystemAutopilotOrgan",
            "type": "governor",
            "description": "Smarter profiles based on clusters, behavior, time-of-day, intent."
        })
        blueprints.append({
            "name": "GameAwareOptimizerOrgan",
            "type": "governor",
            "description": "Bulletproof game detection using processes + behavior."
        })
        blueprints.append({
        ...
        blueprints.append({
            ...
            "type": "governor",
            "description": "Adjusts kernel mode based on load and intent."
        })
        blueprints.append({
            "name": "MetaOptimizerOrgan",
            "type": "meta",
            "description": "Tunes other organs based on fitness and decisions."
        })
        blueprints.append({
            "name": "AutopilotConfidenceOrgan",
            "type": "meta",
            "description": "Computes confidence for autopilot decisions."
        })
        return blueprints
    ...
    def _render_module_code(self, name: str, bp: Dict[str, Any]) -> str:
        if name == "SystemAutopilotOrgan":
            return '''import psutil
import time
import os
import datetime

EXCLUDED_PROCESSES = ["Steam.exe", "steam.exe", "EpicGamesLauncher.exe", "Teams.exe", "ms-teams.exe", "Copilot.exe", "copilot.exe"]

class SystemAutopilotOrgan:
    def __init__(self, organism):
        self.organism = organism
        self.current_profile = "idle"

    def _cluster_processes(self):
        ...
        for p in procs:
            ...
            if any(ex.lower() == name for ex in EXCLUDED_PROCESSES):
                clusters["background"].append(p)
                continue
            if "obs" in name or "twitch" in name or "stream" in name:
                clusters["streaming"].append(p)
            elif "steam" in name or "epic" in name or "game" in name or "riot" in name or "battle.net" in name:
                clusters["gaming"].append(p)
            elif "chrome" in name or "edge" in name or "word" in name or "excel" in name or "powerpoint" in name:
                clusters["work"].append(p)
            else:
                if cpu > 5.0 or mem > 2.0:
                    clusters["background"].append(p)
        return clusters

    def _behavior_signature(self):
        cpu = psutil.cpu_percent(interval=0.1)
        mem = psutil.virtual_memory().percent
    ...
    def _detect_profile(self):
        clusters = self._cluster_processes()
        behavior = self._behavior_signature()
        intent_profile = self.organism.kernel.get("intent", "performance")
        time_profile = self._time_of_day_profile()

        gaming_active = len(clusters["gaming"]) > 0
        streaming_active = len(clusters["streaming"]) > 0
        work_active = len(clusters["work"]) > 0

        cpu = behavior["cpu"]
        mem = behavior["mem"]

        if intent_profile == "performance" or time_profile == "gaming":
            if gaming_active or cpu > 60:
                return "gaming"
        if intent_profile == "quiet":
            if cpu < 30 and mem < 50:
                return "idle"
        if intent_profile == "streaming":
            if streaming_active:
                return "streaming"
        if intent_profile == "work" or time_profile == "work":
            if work_active:
                return "work"
    ...
    def _apply_profile(self, profile):
        ...
        scenario = f"SystemAutopilotOrgan: profile={profile}"
        kernel_update = {"mode": profile, "scenario": scenario}
        persona_update = {
            "traits": {"autopilot_profile": profile},
            "history_event": f"SystemAutopilotOrgan applied profile {profile}."
        }
        insight = f"SystemAutopilotOrgan: applied profile {profile} based on clusters, behavior, intent."
        policy = {
    ...
    def run(self):
        ...
        policy = self._apply_profile(profile)
        if policy:
            return {"status": "ok", "policy": policy}
        return {"status": "ok"}
'''
        if name == "GameAwareOptimizerOrgan":
            return '''import psutil
import time

GAME_HINTS = ["steam.exe", "epicgameslauncher.exe", "battle.net.exe", "riotclient.exe", "eldenring.exe", "fortnite.exe", "cs2.exe"]

class GameAwareOptimizerOrgan:
    def __init__(self, organism):
        self.organism = organism
    ...
    def run(self):
        sig = self._behavior_signature()
        gaming = self._is_game_running()
        cpu = sig["cpu"]
        mem = sig["mem"]
        if gaming or (cpu > 50 and mem > 50):
            policy = {
                "kernel_update": {
                    "mode": "gaming",
                    "scenario": "GameAwareOptimizerOrgan: game/behavior detected, gaming mode engaged"
                },
                "persona_update": {
                    "traits": {"gaming_focus": True},
                    "history_event": "GameAwareOptimizerOrgan engaged gaming mode due to game/behavior."
                },
                "insight": f"GameAwareOptimizerOrgan: gaming behavior detected, cpu={cpu}, mem={mem}"
            }
            return {"status": "ok", "policy": policy}
        return {"status": "ok"}
'''
...
class SystemGovernorOrgan:
    ...
    def run(self):
        ...
        intent = self.organism.kernel.get("intent", "performance")
        new_mode = mode

        if intent == "quiet":
            if cpu < 40 and mem < 60:
                new_mode = "idle"
        elif intent == "performance":
            if cpu > 70 or mem > 80:
                new_mode = "performance"
        elif intent == "multitask":
            new_mode = "work"
        ...
        if new_mode != mode:
            policy = {
                "kernel_update": {
                    "mode": new_mode,
                    "scenario": "SystemGovernorOrgan: adjusted kernel mode based on load+intent"
                },
                "persona_update": {
                    "traits": {"system_governor": True},
                    "history_event": "SystemGovernorOrgan changed kernel mode."
                },
                "insight": "SystemGovernorOrgan: kernel mode adjusted for system health and intent."
            }
            return {"status": "ok", "policy": policy}
        return {"status": "ok"}
'''
        if name == "MetaOptimizerOrgan":
            return '''class MetaOptimizerOrgan:
    def __init__(self, organism):
        self.organism = organism

    def run(self):
        fitness = self.organism.module_fitness
        errors = self.organism.module_error_count
        adjustments = {}
        for name, score in fitness.items():
            err = errors.get(name, 0)
            if score < 0 and err > 0:
                adjustments[name] = {"priority": "low"}
            elif score > 5 and err == 0:
                adjustments[name] = {"priority": "high"}
        if adjustments:
            insight = f"MetaOptimizerOrgan: adjusted module priorities: {adjustments}"
            policy = {
                "persona_update": {
                    "traits": {"meta_optimizer": True},
                    "history_event": "MetaOptimizerOrgan tuned module priorities."
                },
                "insight": insight
            }
            return {"status": "ok", "policy": policy}
        return {"status": "ok"}
'''
        if name == "AutopilotConfidenceOrgan":
            return '''class AutopilotConfidenceOrgan:
    def __init__(self, organism):
        self.organism = organism

    def run(self):
        last_actions = self.organism.recent_actions[-10:]
        if not last_actions:
            return {"status": "ok"}
        correct = 0
        total = 0
        for a in last_actions:
            if a.get("type") == "policy_applied":
                total += 1
        ...
        insight = f"AutopilotConfidenceOrgan: estimated confidence={confidence:.2f} over last {total} decisions."
        policy = {
            "persona_update": {
                "traits": {"autopilot_confidence": confidence},
                "history_event": "AutopilotConfidenceOrgan evaluated autopilot decisions."
            },
            "insight": insight
        }
        return {"status": "ok", "policy": policy}
...
# ===========================================================
app = FastAPI(title="BORG OS v34 Telemetry & Control", version="34.0.0")
...
def get_state():
    ...
    return {
        ...
        "intent": INTENT_ENGINE.get_intent()
    }
...
@app.get("/insights")
def get_insights():
    if os.path.exists(INSIGHT_EXPORT_PATH):
        try:
            with open(INSIGHT_EXPORT_PATH, "r", encoding="utf-8") as fp:
                data = json.load(fp)
            return data
        except Exception:
            return JSONResponse({"error": "Failed to read insight export"}, status_code=500)
    return {"status": "no_insight_yet"}

@app.get("/decisions")
def get_decisions(limit: int = 200):
    return {"decisions": DECISION_GRAPH.decisions[-limit:]}
...
def set_kernel_mode(mode: str):
    ...
    if GLOBAL_ORGANISM:
        ...
        GLOBAL_ORGANISM.recent_actions.append({
            "ts": time.time(),
            "type": "kernel_mode_set",
            "mode": mode
        })
        EVENT_BUS.emit("autopilot.profile_applied", {"profile": mode, "source": "api"})
    return {"status": "ok", "mode": mode}

@app.post("/intent")
def set_intent(intent: str):
    INTENT_ENGINE.set_intent(intent)
    profile = INTENT_ENGINE.map_intent_to_profile()
    set_kernel_mode(profile)
    return {"status": "ok", "intent": intent, "mapped_profile": profile}

@app.post("/profiles/gaming")
def set_profile_gaming():
    return set_kernel_mode("gaming")

@app.post("/profiles/work")
def set_profile_work():
    return set_kernel_mode("work")

@app.post("/profiles/idle")
def set_profile_idle():
    return set_kernel_mode("idle")

@app.post("/profiles/streaming")
def set_profile_streaming():
    return set_kernel_mode("streaming")

@app.post("/autopilot/boost")
def autopilot_boost():
    INTENT_ENGINE.set_intent("performance")
    profile = INTENT_ENGINE.map_intent_to_profile()
    return set_kernel_mode(profile)

@app.post("/autopilot/calm")
def autopilot_calm():
    INTENT_ENGINE.set_intent("quiet")
    profile = INTENT_ENGINE.map_intent_to_profile()
    return set_kernel_mode(profile)

@app.post("/autopilot/focus")
def autopilot_focus():
    INTENT_ENGINE.set_intent("work")
    profile = INTENT_ENGINE.map_intent_to_profile()
    return set_kernel_mode(profile)

@app.post("/autopilot/diagnose")
def autopilot_diagnose():
    return {
        "intent": INTENT_ENGINE.get_intent(),
        "kernel": KERNEL_ENGINE.state,
        "recent_decisions": DECISION_GRAPH.decisions[-20:]
    }
